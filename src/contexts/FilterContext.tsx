...
export const filterUtils = {
  ...
  filterData: <T extends Record<string, any>>(
    ...
    return data.filter(item => {
      ...
      // 科目筛选
      if (filterState.selectedSubjects.length > 0) {
        if (!filterState.selectedSubjects.includes(item[subjectField])) return false;
      }
  ...
  getFilterDescription: (filterState: FilterState): string => {
    ...
    if (filterState.dateRange) {
      ...
      parts.push(`时间: ${start.toLocaleDateString()} - ${end.toLocaleDateString()}`);
    }
  ...
  isEqual: (state1: FilterState, state2: FilterState): boolean => {
    return (
      ...
      JSON.stringify(state1.selectedClasses.sort()) === JSON.stringify(state2.selectedClasses.sort()) &&
      JSON.stringify(state1.selectedSubjects.sort()) === JSON.stringify(state2.selectedSubjects.sort()) &&
