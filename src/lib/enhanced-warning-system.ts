...
export interface WarningCondition {
  type: 'score_threshold' | 'score_trend' | 'attendance_rate' | 'behavioral_score' | 'custom';
  field: string;
  operator: 'lt' | 'lte' | 'gt' | 'gte' | 'eq' | 'neq' | 'between' | 'consecutive';
...
export interface WarningAction {
  type: 'notify_teacher' | 'notify_parent' | 'create_task' | 'send_email' | 'log_event';
  recipients?: string[];
  template?: string;
...
export class EnhancedWarningSystem {
  ...
  // 评估学生是否触发预警
  static async evaluateStudent(studentId: string, rules: EnhancedWarningRule[]): Promise<WarningAlert[]> {
    ...
    try {
      ...
      for (const rule of rules.filter(r => r.isActive)) {
        const evaluation = await this.evaluateRule(rule, student, grades || [], classGrades || []);
  ...
  private static async evaluateRule(
    ...
    grades: any[],
    classGrades: any[]
  ): Promise<{ triggered: boolean; description: string; data: Record<string, any> }> {
    ...
    try {
      ...
      for (const condition of rule.conditions) {
        const conditionResult = await this.evaluateCondition(condition, student, grades, classGrades);
        
        if (rule.triggers[0]?.conditions === 'all') {
          triggered = triggered === false ? conditionResult.met : triggered && conditionResult.met;
        } else {
  ...
  private static async evaluateCondition(
    ...
    switch (condition.type) {
      case 'score_threshold':
        ...
        const belowThreshold = recentScores.filter(g => g.score < (condition.value as number));
        
        if (condition.operator === 'consecutive' && condition.consecutiveCount) {
          ...
          met = this.compareValues(latestScore, condition.operator, condition.value as number);
          if (met) {
      ...
      case 'score_trend':
        if (grades.length >= 2) {
          ...
          met = this.compareValues(change, condition.operator, condition.value as number);
          if (met) {
      ...
      case 'custom':
        if (condition.field === 'avg_score_gap') {
          // 计算学生平均分与班级平均分的差距
          const studentAvg = grades.reduce((sum, g) => sum + g.score, 0) / grades.length;
          const classAvgBySubject = this.calculateClassAverages(classGrades);
          const classOverallAvg = Object.values(classAvgBySubject).reduce((sum: number, avg: any) => sum + avg, 0) / Object.keys(classAvgBySubject).length;
          const gap = studentAvg - classOverallAvg;
          
          met = this.compareValues(gap, condition.operator, condition.value as number);
          if (met) {
          ...
          met = this.compareValues(maxGap.gap, condition.operator, condition.value as number);
          if (met) {
  ...
  // 工具方法：比较值
  private static compareValues(actual: number, operator: string, expected: number | [number, number]): boolean {
    switch (operator) {
      case 'lt': return actual < (expected as number);
      case 'lte': return actual <= (expected as number);
      case 'gt': return actual > (expected as number);
      case 'gte': return actual >= (expected as number);
      case 'eq': return actual === (expected as number);
      case 'neq': return actual !== (expected as number);
      case 'between':
  ...
  // 工具方法：计算班级各科目平均分
  private static calculateClassAverages(classGrades: any[]): Record<string, number> {
    const subjectGroups = classGrades.reduce((groups, grade) => {
  ...
  // 工具方法：计算科目间最大差距
  private static calculateMaxSubjectGap(subjectScores: Record<string, number[]>): {
    gap: number;
    highSubject: string;
    lowSubject: string;
    highScore: number;
  ...
  // 批量评估所有学生
  static async evaluateAllStudents(rules: EnhancedWarningRule[]): Promise<WarningAlert[]> {
    try {
      // 获取所有学生
  ...
  // 获取预警分析数据
  static async getWarningAnalytics(timeRange: 'week' | 'month' | 'quarter' = 'month'): Promise<WarningAnalytics> {
    try {
      const now = new Date();
      ...
      analytics.trendAnalysis.thisWeek = warnings.filter(w => 
        new Date(w.created_at) >= oneWeekAgo
      ).length;

      analytics.trendAnalysis.lastWeek = warnings.filter(w => 
        new Date(w.created_at) >= twoWeeksAgo && new Date(w.created_at) < oneWeekAgo
      ).length;

      analytics.trendAnalysis.changeRate = analytics.trendAnalysis.lastWeek > 0
        ? ((analytics.trendAnalysis.thisWeek - analytics.trendAnalysis.lastWeek) / analytics.trendAnalysis.lastWeek) * 100
        : 0;

      // 高风险学生统计
      const studentWarningCounts = warnings.reduce((counts, warning) => {
        ...
        if (severityOrder[warning.severity as keyof typeof severityOrder] > 
            severityOrder[counts[key].highestSeverity as keyof typeof severityOrder]) {
      ...
      if (resolvedWarnings.length > 0) {
        const totalResolutionTime = resolvedWarnings.reduce((total, warning) => {
