...
export class EnhancedAnalyzer {
  ...
  // 趋势分析 - 分析学生成绩趋势
  static analyzeTrend(scores: number[], timePoints: string[]): TrendAnalysisResult {
    if (scores.length < 2) {
      return {
        trend: 'stable',
        trendStrength: 0,
  ...
  // 异常检测 - 检测异常成绩
  static detectAnomalies(scores: number[], studentAverage?: number): AnomalyDetectionResult[] {
    if (scores.length < 3) {
      return [];
    }
    ...
    scores.forEach((score, index) => {
      ...
      if (studentAverage && Math.abs(score - studentAverage) > 15) {
        isAnomaly = true;
        anomalyScore = Math.max(anomalyScore, Math.abs(score - studentAverage) / 50);
  ...
  // 性能洞察分析
  static generatePerformanceInsights(gradeRecords: GradeRecord[]): PerformanceInsight[] {
    ...
    // 按科目分组
    const subjectGroups = gradeRecords.reduce((groups, record) => {
    ...
    Object.entries(subjectGroups).forEach(([subject, records]) => {
      ...
      if (scores.length >= 3) {
        ...
        const earlyAverage = earlyScores.reduce((a, b) => a + b, 0) / earlyScores.length;
        ...
        if (recentAverage > earlyAverage + 5) {
          insights.push({
            ...
            data: { recentAverage, earlyAverage, improvement: recentAverage - earlyAverage },
      ...
      // 不稳定性检测
      if (variance > 100) { // 方差过大说明成绩不稳定
        insights.push({
          type: 'concern',
          subject,
          description: `${subject}成绩波动较大，最高${maxScore}分，最低${minScore}分`,
          data: { variance, maxScore, minScore, scoreRange: maxScore - minScore },
    ...
    if (overallAverage < 75) {
      insights.push({
        ...
        data: { overallAverage, totalSubjects: Object.keys(subjectGroups).length },
  ...
  // 班级对比分析
  static analyzeClassComparison(studentScores: GradeRecord[], classScores: GradeRecord[]): {
    ...
    // 按科目分组学生成绩
    const studentBySubject = studentScores.reduce((groups, record) => {
      if (!groups[record.subject]) {
        groups[record.subject] = [];
      }
    ...
    // 按科目分组班级成绩
    const classBySubject = classScores.reduce((groups, record) => {
      if (!groups[record.subject]) {
        groups[record.subject] = [];
      }
      groups[record.subject].push(record.score);
    ...
    const comparison = Object.keys(studentBySubject).map(subject => {
      const studentAvg = studentBySubject[subject].reduce((a, b) => a + b, 0) / studentBySubject[subject].length;
      const classSubjectScores = classBySubject[subject] || [];
      const classAvg = classSubjectScores.length > 0 
        ? classSubjectScores.reduce((a, b) => a + b, 0) / classSubjectScores.length 
        : 0;
      
      // 计算排名
      const higherScores = classSubjectScores.filter(score => score > studentAvg).length;
    ...
    // 计算整体表现
    const studentOverallAvg = Object.values(studentBySubject)
      .flat()
      .reduce((a, b) => a + b, 0) / Object.values(studentBySubject).flat().length;
    
    const classOverallAvg = Object.values(classBySubject)
      .flat()
      .reduce((a, b) => a + b, 0) / Object.values(classBySubject).flat().length;
    ...
    const lowerScores = allClassScores.filter(score => score < studentOverallAvg).length;
    const percentileRank = allClassScores.length > 0 
  ...
  static generateLearningRecommendations(
    ...
    category: 'study_method' | 'time_management' | 'subject_focus' | 'exam_strategy';
