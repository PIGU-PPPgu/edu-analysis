...
export const uxUtils = {
  ...
  getDeviceInfo: () => {
    ...
    const isTablet = width >= defaultUXConfig.responsive.mobileBreakpoint && 
  ...
  // 延迟执行（防止加载闪烁）
  delayedExecution: async (fn: () => void, minDelay: number = defaultUXConfig.loading.minLoadingTime) => {
...
export const performanceMonitor = {
  ...
  measurePageLoad: () => {
    if (typeof window !== 'undefined' && 'performance' in window) {
      ...
      const domContentLoaded = perfData.timing.domContentLoadedEventEnd - perfData.timing.navigationStart;
      
      return {
        totalLoadTime: loadTime,
        domContentLoadedTime: domContentLoaded,
        timeToFirstByte: perfData.timing.responseStart - perfData.timing.navigationStart,
        resourceLoadTime: perfData.timing.loadEventEnd - perfData.timing.domContentLoadedEventEnd
      };
    }
    return null;
  },
