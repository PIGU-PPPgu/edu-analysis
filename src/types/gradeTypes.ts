...
export const STANDARD_FIELD_MAPPING: FieldMapping = {
  ...
  nameFields: [
    'name', '姓名', '学生姓名', '姓名', '名称', 'studentName',
    '考生姓名', '学生', '真实姓名'
  ],
  classFields: [
  ...
  subjectMappings: {
    ...
    '化学': {
      scoreFields: ['化学', '化学分数', '化学成绩', 'chemistry', 'chemistry_score'],
    ...
    '政治': {
      scoreFields: ['政治', '政治分数', '政治成绩', '道法', '道法分数', 'politics', 'politics_score'],
    ...
    '地理': {
      scoreFields: ['地理', '地理分数', '地理成绩', 'geography', 'geography_score'],
      gradeFields: ['地理等级', '地理级别', 'geography_grade'],
      rankFields: ['地理排名', '地理班名', 'geography_rank']
    },
    '总分': {
      scoreFields: ['总分', '总分分数', '总成绩', 'total', 'total_score', '总计'],
...
export const convertToStandardFields = (rawData: any): Partial<GradeRecord> => {
  ...
  for (const field of STANDARD_FIELD_MAPPING.studentIdFields) {
    if (rawData[field] !== undefined && rawData[field] !== null && String(rawData[field]).trim()) {
