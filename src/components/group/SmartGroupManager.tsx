...
export function SmartGroupManager({ className, students, onGroupsCreated }: SmartGroupManagerProps) {
  ...
  const handleCreateCustomGroup = () => {
    ...
    const selectedStudentData = Array.from(selectedStudents).map(studentId => {
    ...
    const newGroup: GroupAllocationResult = {
      ...
      predicted_performance: selectedStudentData.reduce((sum, s) => sum + s.contribution_score, 0) / selectedStudentData.length,
    };
   ...
   * 调整分组
   */
  const handleAdjustGroup = (groupId: string, studentId: string, action: 'remove' | 'add') => {
  ...
  const getGroupingQuality = () => {
    ...
    const totalStudents = generatedGroups.reduce((sum, g) => sum + g.members.length, 0);
    const avgBalance = generatedGroups.reduce((sum, g) => {
      const balance = (g.group_balance.academic_balance + g.group_balance.personality_balance + g.group_balance.skill_complementarity) / 3;
      return sum + balance;
    }, 0) / generatedGroups.length;

    const sizeVariance = Math.max(...generatedGroups.map(g => g.members.length)) - Math.min(...generatedGroups.map(g => g.members.length));
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select 
                ...
                onValueChange={(value: GroupingStrategy) => setConfig(prev => ({ ...prev, strategy: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <p className="text-xs text-muted-foreground">
                {config.strategy === 'balanced' && '确保每组学术能力和个性特征平衡'}
                {config.strategy === 'mixed_ability' && '每组包含不同能力水平的学生'}
                {config.strategy === 'homogeneous' && '相似水平的学生分在同一组'}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Slider
                ...
                onValueChange={([value]) => setConfig(prev => ({ ...prev, groupCount: value }))}
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span>范围: {optimalRange.min}-{optimalRange.max}</span>
              </div>
            </div>

            {/* 组大小范围 */}
            <div className="space-y-2">
              <Label>每组人数: {config.minGroupSize}-{config.maxGroupSize}人</Label>
              <div className="space-y-2">
                <div>
                  <Label className="text-xs">最少人数</Label>
                  <Slider
                    value={[config.minGroupSize]}
                    onValueChange={([value]) => setConfig(prev => ({ ...prev, minGroupSize: value }))}
                    max={5}
                    min={2}
                    step={1}
                  />
                ...
                <div>
                  ...
                  <Slider
                    ...
                    onValueChange={([value]) => setConfig(prev => ({ ...prev, maxGroupSize: value }))}
                    max={8}
                    min={3}
                    step={1}
                  />
          ...
          <div className="mt-4 pt-4 border-t space-y-3">
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, considerPersonality: e.target.checked }))}
              />
              <Label htmlFor="considerPersonality" className="text-sm">考虑性格特征</Label>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, considerAcademicLevel: e.target.checked }))}
              />
              <Label htmlFor="considerAcademicLevel" className="text-sm">考虑学术水平</Label>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <input
                ...
                onChange={(e) => setConfig(prev => ({ ...prev, allowManualAdjustment: e.target.checked }))}
              />
              <Label htmlFor="allowManualAdjustment" className="text-sm">允许手动调整</Label>
            </div>
          </div>

          {/* 操作按钮 */}
          <div className="mt-6 flex gap-3">
            <Button onClick={handleAutoGrouping} disabled={isGenerating} className="flex items-center gap-2">
              {isGenerating ? (
                      ...
                      {students.map((student) => (
                        <div
                          ...
                          className={`flex items-center justify-between p-2 rounded cursor-pointer hover:bg-muted ${
                            selectedStudents.has(student.student_id) ? 'bg-primary/10' : ''
                          }`}
                          onClick={() => {
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2">
                            {student.overall_score && (
                              <Badge variant="secondary">{student.overall_score}分</Badge>
                            )}
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {quality && (
                  <Badge variant={quality.overall_quality > 80 ? "default" : "secondary"}>
                ...
                <AlertDescription>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2">
                    <div>
                      ...
                      <div className="text-2xl">{Math.round(quality.coverage)}%</div>
                    </div>
                    <div>
                      <div className="text-sm font-medium">大小均衡</div>
                      <div className="text-2xl">{Math.round(quality.size_balance)}分</div>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className={`text-2xl ${quality.overall_quality > 80 ? 'text-green-600' : 'text-yellow-600'}`}>
                  ...
                  <CardContent>
                      ...
                      {group.members.map((member) => (
                        <div key={member.student_id} className="flex items-center justify-between text-sm">
                          <span className="flex items-center gap-2">
                            ...
                            {member.role === 'specialist' && <Badge variant="secondary" className="text-xs">专家</Badge>}
                          </span>
                          {config.allowManualAdjustment && (
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => handleAdjustGroup(group.group_id, member.student_id, 'remove')}
                    ...
                    <div className="space-y-2 pt-2 border-t">
                      <div className="flex justify-between text-xs">
                        <span>学术平衡</span>
                        <span>{Math.round(group.group_balance.academic_balance)}%</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span>性格平衡</span>
                        <span>{Math.round(group.group_balance.personality_balance)}%</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span>技能互补</span>
                        <span>{Math.round(group.group_balance.skill_complementarity)}%</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <CardContent>
          <div className="space-y-3 text-sm">
            <div className="flex items-start gap-2">
              ...
              <div>
                <strong>平衡分组：</strong>适合大多数教学场景，确保每组都有不同能力水平的学生，促进互助学习。
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Users className="h-4 w-4 text-green-500 mt-0.5" />
              <div>
                <strong>最佳组大小：</strong>建议每组3-5人，既保证参与度又便于管理。
              </div>
            </div>
            <div className="flex items-start gap-2">
              <AlertTriangle className="h-4 w-4 text-yellow-500 mt-0.5" />
              <div>
                <strong>注意事项：</strong>自动分组基于学生成绩和学习特征，教师可根据实际情况手动调整。
              </div>
            </div>
          </div>
        </CardContent>
