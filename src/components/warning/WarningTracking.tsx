...
interface TrackingRecord {
  ...
  actionType: 'contact_parent' | 'student_meeting' | 'tutoring' | 'counseling' | 'other';
}

// 预警跟踪组件
const WarningTracking: React.FC<{ warningId?: string }> = ({ warningId }) => {
  const [trackingRecords, setTrackingRecords] = useState<TrackingRecord[]>([]);
  const [selectedWarning, setSelectedWarning] = useState<string>(warningId || '');
  ...
  useEffect(() => {
    const mockData: TrackingRecord[] = [
      {
        ...
        notes: '学生反馈数学基础较薄弱，特别是函数部分理解困难。已安排课后辅导。',
        status: 'in_progress',
    ...
    if (selectedWarning) {
      setTrackingRecords(mockData.filter(r => r.warningId === selectedWarning));
    } else {
  ...
  const updateEffectiveness = (id: string, effectiveness: TrackingRecord['effectiveness']) => {
    setTrackingRecords(prev => 
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      ...
      case 'follow_up_needed': return '需跟进';
  ...
  const getEffectivenessText = (effectiveness: string) => {
    switch (effectiveness) {
      case 'high': return '效果良好';
      case 'medium': return '效果一般';
      case 'low': return '效果较差';
      default: return '待评估';
    }
  };
  ...
  const getActionIcon = (actionType: string) => {
    switch (actionType) {
      ...
      case 'student_meeting': return <User className="h-4 w-4" />;
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => setNewRecord(prev => ({ 
                    ...prev, 
                    actionType: e.target.value as TrackingRecord['actionType'] 
                  }))}
        ...
        {trackingRecords.length === 0 ? (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-400 text-sm mt-2">开始记录您的预警处理行动</p>
            </CardContent>
          </Card>
        ) : (
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      value={record.status}
                      onChange={(e) => updateRecordStatus(record.id, e.target.value as TrackingRecord['status'])}
                    ...
                    <select
                      ...
                      onChange={(e) => updateEffectiveness(record.id, e.target.value as TrackingRecord['effectiveness'])}
                      className="text-xs border rounded px-2 py-1"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {trackingRecords.filter(r => r.status === 'in_progress').length}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {trackingRecords.filter(r => r.effectiveness === 'high').length}
              </div>
