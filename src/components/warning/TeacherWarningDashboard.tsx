...
const TeacherWarningDashboard: React.FC = () => {
  ...
  const loadWarningData = async () => {
    try {
      ...
      // 过滤和排序预警记录
      let filteredWarnings = warningsResult.filter(
        (w) => w.status === "active"
      );

      if (selectedPriority !== "all") {
        filteredWarnings = filteredWarnings.filter(
          (w) => w.warning_rules?.severity === selectedPriority
        );
      }
      ...
      filteredWarnings.sort((a, b) => {
        ...
        const severityDiff =
          (severityOrder[b.warning_rules?.severity] || 0) -
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {formatNumber(
                    stats?.warningsByType?.find((t) => t.type === "学业预警")
                      ?.count || 0
                  )}
                </p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600 mb-2">
                            ...
                            {warning.students?.class_name || "未知班级"}
                          </p>

                          <p className="text-sm text-gray-700 mb-3">
                            <span className="font-medium">问题:</span>{" "}
                            {warning.warning_rules?.name || "预警规则"}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <Alert className="mb-3 border-blue-200 bg-blue-50">
                      ...
                      <AlertDescription className="text-blue-800">
                        <div className="space-y-2">
                          <p>
                            <span className="font-semibold">立即行动:</span>{" "}
                            {advice.immediate}
                          </p>
                          <p>
                            <span className="font-semibold">后续跟进:</span>{" "}
                            {advice.followup}
                          </p>
                          <p>
                            <span className="font-semibold">完成时限:</span>{" "}
                            {advice.timeline}
                          </p>
                        </div>
                      </AlertDescription>
