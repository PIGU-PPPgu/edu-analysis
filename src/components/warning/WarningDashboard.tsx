...
const WarningDashboard: React.FC<WarningDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      warningData &&
      (!warningData.warningsByType ||
        !Array.isArray(warningData.warningsByType))
    ) {
  ...
  const generateAIInsights = async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          aiClient.chat &&
          typeof aiClient.chat.completions?.create === "function"
        ) {
            ...
            messages: [
              {
                role: "system",
                content:
                  "你是一位教育数据分析专家，擅长分析学生预警数据并提供干预建议。",
              },
        ...
        throw new Error(
          `AI请求错误: ${apiError instanceof Error ? apiError.message : "未知错误"}`
        );
      }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          description={
            stats.totalStudents
              ? `占比 ${((stats.atRiskStudents / stats.totalStudents) * 100).toFixed(1)}%`
              : "占比计算中"
          }
          change={2.5}
          trend="up"
        ...
        <StatCard
          ...
          description={
            stats.atRiskStudents
              ? `占风险学生 ${((stats.highRiskStudents / stats.atRiskStudents) * 100).toFixed(1)}%`
              : "占比计算中"
          }
          change={-1.2}
          trend="down"
                    ...
                    >
                      ...
                      <div className="w-full bg-gray-200 h-2 mt-2 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${classData.studentCount > 0 ? (classData.atRiskCount / classData.studentCount) * 100 : 0}%`,
                          }}
                        ></div>
                      </div>
                    </Card>
