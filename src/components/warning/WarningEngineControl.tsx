...
import { PlayIcon, StopIcon, RefreshCwIcon, AlertTriangleIcon, CheckCircleIcon, XCircleIcon } from 'lucide-react';
import { executeWarningRules, getWarningEngineStatus as getEngineStatus, type WarningEngineResult as EngineExecutionResult } from '@/services/warningEngineService';
                ...
                <div className="space-y-2">
                  ...
                  {lastResult.success ? (
                    <div className="text-sm space-y-1">
                      ...
                      <div>执行时间: {(lastResult.executionTime / 1000).toFixed(1)}s</div>
                    </div>
                  ) : (
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-sm text-muted-foreground">生成预警</div>
                  </div>
                  <div className="text-center">
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>执行时间:</span>
                    <span>{new Date(executionStats.lastExecution.createdAt).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>执行状态:</span>
                    <Badge variant={executionStats.lastExecution.success ? "secondary" : "destructive"}>
                      {executionStats.lastExecution.success ? '成功' : '失败'}
                    </Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>处理学生:</span>
                    <span>{executionStats.lastExecution.studentsProcessed || 0} 人</span>
                  </div>
                  <div className="flex justify-between">
                    <span>生成预警:</span>
                    <span>{executionStats.lastExecution.warningsGenerated || 0} 条</span>
                  </div>
                </div>
              </CardContent>
            </Card>
