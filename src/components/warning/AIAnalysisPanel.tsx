...
function AIAnalysisPanel({ request, onRefresh }: AIAnalysisPanelProps) {
  ...
  const loadAnalysis = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : '获取AI分析失败';
      ...
      <CardContent>
        <Tabs defaultValue="summary" className="space-y-4">
          ...
          <TabsContent value="summary" className="space-y-4">
            {/* 总体风险评估 */}
            <Card className={`border ${getRiskColor(analysis.riskSummary.overallRisk)}`}>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${
                      analysis.riskSummary.riskScore >= 80 ? 'bg-red-500' :
                      analysis.riskSummary.riskScore >= 60 ? 'bg-orange-500' :
                      analysis.riskSummary.riskScore >= 30 ? 'bg-yellow-500' : 'bg-green-500'
                    }`}
              ...
              <Card>
                ...
                <CardContent className="pt-0">
                  <div className="space-y-2">
                    {analysis.riskSummary.primaryConcerns.map((concern, index) => (
                      <div 
                        key={index}
            ...
            {analysis.riskSummary.improvementAreas.length > 0 && (
              <Card>
                ...
                <CardContent className="pt-0">
                  <div className="space-y-2">
                    {analysis.riskSummary.improvementAreas.map((area, index) => (
                      <div 
                        key={index}
              ...
              <CardContent className="pt-0">
                <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="font-medium">
                    {analysis.patterns.trendDirection === 'improving' ? '改善趋势' :
                     analysis.patterns.trendDirection === 'declining' ? '恶化趋势' : '稳定状态'}
                  </span>
                </div>
              </CardContent>
            </Card>
                    ...
                    {analysis.patterns.correlations.map((correlation, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge variant={
                            Math.abs(correlation.strength) > 0.7 ? 'default' :
                            Math.abs(correlation.strength) > 0.4 ? 'secondary' : 'outline'
                          }>
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          {rec.resources.length > 0 && (
                            <div>
                              <span className="text-xs font-medium text-gray-500">所需资源:</span>
        ...
        <div className="mt-6 pt-4 border-t">
          <div className="flex items-center justify-between text-xs text-gray-500">
            ...
            <span>
              生成时间: {new Date(analysis.metadata.generatedAt).toLocaleString()}
            </span>
          </div>
