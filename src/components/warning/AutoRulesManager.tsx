...
const AutoRulesManager: React.FC<AutoRulesManagerProps> = ({ onRulesExecuted }) => {
  ...
  const getConditionDescription = (condition: any) => {
    ...
    console.log('  🔧 生成描述，字段值:', { type, operator, value, timeframe, subject });
    ...
    const finalDescription = typeDescriptions[type] || `${type}: ${operatorText} ${value}`;
          ...
          {lastExecution && (
            <>
              ...
              <div className="space-y-3">
                <h4 className="text-sm font-medium text-gray-800">最近执行结果</h4>
                
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="text-center">
                    <div className="flex items-center justify-center mb-1">
                      <FileCheck className="h-4 w-4 text-blue-500 mr-1" />
                      <span className="text-lg font-bold text-gray-800">{lastExecution.executedRules}</span>
                    </div>
                  ...
                  <div className="text-center">
                    <div className="flex items-center justify-center mb-1">
                      ...
                      <span className="text-lg font-bold text-gray-800">{lastExecution.totalWarnings}</span>
                    </div>
                  ...
                  <div className="text-center">
                    <div className="flex items-center justify-center mb-1">
                      ...
                      <span className="text-lg font-bold text-gray-800">
                        {lastExecution.results.reduce((sum, r) => sum + r.matchedStudents.length, 0)}
                      </span>
                    </div>
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                    <div className="flex items-center">
                      ...
                      <p className="text-sm text-green-800">
                        成功为 <Badge variant="outline" className="mx-1">{lastExecution.totalWarnings}</Badge> 名学生生成预警记录
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-4">
            {rules.map((rule) => (
              <div key={rule.id} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      ...
                      <ul className="mt-1 space-y-1">
                        {(Array.isArray(rule.conditions) ? rule.conditions : []).map((condition, index) => (
                          <li key={index} className="ml-2 flex items-start">
                            <span className="text-[#c0ff3f] mr-1">•</span>
                            <span className="text-gray-700">{getConditionDescription(condition)}</span>
                          </li>
                        ))}
                        {(!Array.isArray(rule.conditions) || rule.conditions.length === 0) && (
                          <li className="ml-2 text-amber-600 flex items-center">
                            <AlertTriangle className="h-3 w-3 mr-1" />
                            <span>该规则尚未配置具体触发条件，可能无法正常执行</span>
                          </li>
                        )}
                      </ul>
                    </div>
                ...
                {/* 规则执行结果 */}
                {lastExecution && lastExecution.results.find(r => r.ruleId === rule.id) && (
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    {(() => {
                      const result = lastExecution.results.find(r => r.ruleId === rule.id)!;
                      return (
                        <div className="flex items-center justify-between text-xs">
                          <div className="flex items-center space-x-4">
                            <span className="text-gray-600">
                              匹配学生: <strong>{result.matchedStudents.length}</strong>
                            </span>
                            <span className="text-gray-600">
                              生成预警: <strong>{result.warningsGenerated}</strong>
                            </span>
