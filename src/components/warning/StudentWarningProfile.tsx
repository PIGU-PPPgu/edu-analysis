...
const StudentWarningProfile: React.FC<StudentWarningProfileProps> = ({
  studentUuid,
  isOpen,
  onOpenChange,
}) => {
  ...
  const renderGradeBadge = (score: number) => {
    ...
    if (score >= 70)
      return <Badge className="bg-yellow-500 hover:bg-yellow-600">中等</Badge>;
    if (score >= 60)
      return <Badge className="bg-orange-500 hover:bg-orange-600">及格</Badge>;
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          {profileData?.student_info ? (
            <DialogDescription>
              {profileData.student_info.student_name} (
              {profileData.student_info.student_display_id}) -{" "}
              {profileData.student_info.class_name}
            </DialogDescription>
          ) : (
            <DialogDescription>加载学生信息中...</DialogDescription>
          )}
        </DialogHeader>

        {isLoading && <div className="text-center py-8">加载学生数据中...</div>}
        {error && (
          <div className="text-center py-8 text-red-600">加载失败: {error}</div>
        )}
          ...
          <div className="grid gap-6 py-4">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center space-x-4 pb-2">
                  <Avatar className="h-16 w-16">
                    <AvatarImage
                      src={`https://api.dicebear.com/7.x/initials/svg?seed=${profileData.student_info.student_name}`}
                      alt={profileData.student_info.student_name}
                    />
                    <AvatarFallback>
                      {profileData.student_info.student_name?.substring(0, 2)}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {profileData.student_info.student_display_id} |{" "}
                      {profileData.student_info.class_name} (
                      {profileData.student_info.grade_level})
                    </p>
                ...
                <CardContent className="text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    <p>
                      <strong>邮箱:</strong>{" "}
                      {profileData.student_info.student_email || "N/A"}
                    </p>
                    <p>
                      <strong>性别:</strong>{" "}
                      {profileData.student_info.student_gender || "N/A"}
                    </p>
                    <p>
                      <strong>生日:</strong>{" "}
                      {profileData.student_info.student_date_of_birth || "N/A"}
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            {profileData.latest_grades && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    最近考试日期:{" "}
                    {profileData.latest_grades.latest_exam_date_for_student ||
                      "N/A"}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold">
                        {profileData.latest_grades.latest_average_score?.toFixed(
                          1
                        ) ?? "N/A"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-2xl font-bold text-red-500">
                        {profileData.latest_grades
                          .latest_failing_subjects_count ?? "N/A"}
                      </p>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2">
                    {profileData.latest_grades.latest_exam_details?.map(
                      (exam, index) => (
                        <li
                          key={index}
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            <span
                              className={`font-bold ${exam.score < 60 ? "text-red-500" : "text-green-600"}`}
                            >
                              {exam.score}
                            </span>
                    ...
                    {(!profileData.latest_grades.latest_exam_details ||
                      profileData.latest_grades.latest_exam_details.length ===
                        0) && (
                      <p className="text-xs text-muted-foreground">
                        暂无最近考试成绩数据。
                      </p>
            ...
            {profileData.homework_stats && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                    <CheckCircle className="h-5 w-5 mr-2 text-[#c0ff3f]" />
                    作业提交统计
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                    <div>
                      ...
                      <p className="text-xl font-bold">
                        {profileData.homework_stats.total_homeworks_submitted ??
                          "N/A"}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="text-xl font-bold">
                        {profileData.homework_stats.average_homework_grade?.toFixed(
                          1
                        ) ?? "N/A"}
                      </p>
                      {profileData.homework_stats.average_homework_grade !==
                        null &&
                        typeof profileData.homework_stats
                          .average_homework_grade !== "undefined" && (
                          <Progress
                            value={
                              profileData.homework_stats.average_homework_grade
                            }
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-orange-500">
                        {profileData.homework_stats.late_submissions_count ??
                          "N/A"}
                      </p>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2 max-h-40 overflow-y-auto">
                    {profileData.homework_stats.submission_details
                      ?.slice(0, 5)
                      .map((hw, index) => (
                        <li
                          key={index}
                          className={`p-2 rounded-md ${hw.is_late ? "bg-orange-50" : "bg-green-50"}`}
                        >
                          <div className="flex justify-between items-center">
                            ...
                            {hw.grade !== null && (
                              <span
                                className={`text-xs font-semibold ${hw.grade < 60 ? "text-red-600" : "text-green-700"}`}
                              >
                                得分: {hw.grade}
                              </span>
                            )}
                    ...
                    {(!profileData.homework_stats.submission_details ||
                      profileData.homework_stats.submission_details.length ===
                        0) && (
                      <p className="text-xs text-muted-foreground">
                        暂无作业提交数据。
                      </p>
            ...
            {profileData.attendance_summary && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center">
                    <Clock className="h-5 w-5 mr-2 text-[#c0ff3f]" />
                    出勤记录
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-3 gap-4 mb-4 text-center">
                    <div>
                      ...
                      <p className="text-xl font-bold text-red-500">
                        {profileData.attendance_summary.total_absences ?? "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">总迟到</p>
                      <p className="text-xl font-bold text-yellow-500">
                        {profileData.attendance_summary.total_lates ?? "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">
                      ...
                      <p className="text-xl font-bold text-blue-500">
                        {profileData.attendance_summary.total_excused ?? "N/A"}
                      </p>
                    </div>
                  </div>
                  <Separator className="my-3" />
                  ...
                  <ul className="space-y-1 max-h-40 overflow-y-auto">
                    {profileData.attendance_summary.recent_attendance_details
                      ?.slice(0, 10)
                      .map((att, index) => (
                        <li
                          key={index}
