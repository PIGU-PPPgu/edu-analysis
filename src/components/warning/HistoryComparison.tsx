            ...
            <div className="flex items-baseline gap-2 mb-2">
              ...
              <div className="flex items-center">
                ...
                <span className={`text-xs font-medium ml-1 ${getTrendColor()}`}>
                  {change > 0 ? '+' : ''}{change}{isPercentage ? '%' : unit}
                </span>
              </div>
            </div>
            <p className="text-xs text-gray-500">
              上期: {formatNumber(previous)}{unit}
            </p>
          </div>
...
// 分类变化条形图
const CategoryChangeChart = ({ changes }: { changes: { [key: string]: number } }) => {
  const categories = Object.entries(changes || {}).filter(([_, change]) => Math.abs(change) > 0.1);
  ...
  const maxChange = Math.max(...categories.map(([_, change]) => Math.abs(change)));
...
const HistoryComparison: React.FC<HistoryComparisonProps> = ({ onPeriodChange }) => {
  ...
  const getTrendBadge = (trend: string) => {
    const trendMap = {
      improving: { label: '改善中', className: 'bg-green-100 text-green-800 border-green-200' },
      worsening: { label: '恶化中', className: 'bg-red-100 text-red-800 border-red-200' },
      stable: { label: '保持稳定', className: 'bg-gray-100 text-gray-800 border-gray-200' },
      increasing: { label: '上升', className: 'bg-red-100 text-red-800 border-red-200' },
      decreasing: { label: '下降', className: 'bg-green-100 text-green-800 border-green-200' },
      up: { label: '上升', className: 'bg-red-100 text-red-800 border-red-200' },
      down: { label: '下降', className: 'bg-green-100 text-green-800 border-green-200' }
    };
    
    const config = trendMap[trend as keyof typeof trendMap] || { label: trend, className: 'bg-gray-100 text-gray-800 border-gray-200' };
    ...
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              <Select value={period} onValueChange={(value: HistoryPeriod) => handlePeriodChange(value)}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              ...
              <div>{getTrendBadge(comparison?.comparison?.trends?.overall || 'stable')}</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-sm text-gray-500 mb-1">风险水平</div>
              <div>{getTrendBadge(comparison?.comparison?.trends?.riskLevel || 'stable')}</div>
            </div>
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="text-sm text-gray-500 mb-1">主要变化</div>
              <div className="text-sm font-medium text-gray-700">
                {comparison?.comparison?.trends?.resolution ? getTrendBadge(comparison.comparison.trends.resolution) : '无显著变化'}
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <ComparisonCard
          ...
          change={((comparison.current.newIssues || 0) - (comparison.previous.newIssues || 0)) / (comparison.previous.newIssues || 1) * 100}
          icon={TrendingUp}
          unit="条"
          isPercentage={true}
        />
        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <TrendChart data={trendData} metric="totalWarnings" title="预警总数变化" />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <TrendChart data={trendData} metric="highSeverity" title="高危预警变化" />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <TrendChart data={trendData} metric="mediumSeverity" title="中危预警变化" />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <TrendChart data={trendData} metric="lowSeverity" title="低危预警变化" />
              </CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="categories" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent>
                <CategoryChangeChart changes={comparison.comparison.categoryChanges || {}} />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <CategoryChangeChart changes={comparison.comparison.scopeChanges || {}} />
              </CardContent>
            </Card>
          </div>
        </TabsContent>
        ...
        <TabsContent value="classes" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>当前期间与上一期间的班级风险对比</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="font-medium text-gray-700 mb-3">当前期间 ({comparison.current.period})</h4>
                  <div className="space-y-3">
                    {(comparison.current.classRiskData || []).map((classData, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span className="font-medium text-gray-700">{classData.className}</span>
                ...
                <div>
                  <h4 className="font-medium text-gray-700 mb-3">上一期间 ({comparison.previous.period})</h4>
                  <div className="space-y-3">
                    {(comparison.previous.classRiskData || []).map((classData, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                        <span className="font-medium text-gray-700">{classData.className}</span>
                        <div className="text-right">
                          <div className="text-sm font-medium text-gray-800">
