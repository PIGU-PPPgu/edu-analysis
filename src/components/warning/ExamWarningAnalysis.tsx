...
import { getExamWarningStatistics, getExamStatistics, type Exam } from '@/services/examService';
...
const ExamWarningAnalysis: React.FC<ExamWarningAnalysisProps> = ({ className = "" }) => {
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <h3 className="text-2xl font-bold text-gray-800">{formatNumber(examStatistics.totalStudents)}</h3>
              <p className="text-sm text-gray-600">参考学生</p>
            </div>
            
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <h3 className="text-2xl font-bold text-gray-800">{examStatistics.averageScore}</h3>
              <p className="text-sm text-gray-600">平均分</p>
            </div>
            
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <h3 className="text-2xl font-bold text-gray-800">{examStatistics.passRate}%</h3>
          ...
          <div className="mt-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-medium text-gray-700">及格率分析</span>
              <span className="text-sm text-gray-600">{examStatistics.passRate}%</span>
            </div>
            <Progress 
              value={examStatistics.passRate} 
              className="h-3"
              // @ts-ignore
              style={{ '--progress-color': examStatistics.passRate >= 80 ? '#c0ff3f' : examStatistics.passRate >= 60 ? '#fbbf24' : '#ef4444' }}
            />
          </div>
        </CardContent>
      </Card>
  ...
  const renderWarningInsights = () => {
    ...
    const riskLevel = examStatistics.passRate >= 80 ? 'low' : examStatistics.passRate >= 60 ? 'medium' : 'high';
          ...
          {isLoading ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
              <TabsList className="grid w-full grid-cols-3 bg-gray-100">
                <TabsTrigger value="overview" className="data-[state=active]:bg-[#c0ff3f] data-[state=active]:text-black">
                  考试概览
                </TabsTrigger>
                <TabsTrigger value="warnings" className="data-[state=active]:bg-[#c0ff3f] data-[state=active]:text-black">
                  预警分析
                </TabsTrigger>
                <TabsTrigger value="insights" className="data-[state=active]:bg-[#c0ff3f] data-[state=active]:text-black">
                  智能建议
                </TabsTrigger>
              </TabsList>
                    ...
                    <CardContent>
                      {analysis?.riskLevels && (
                        <div className="space-y-3">
                          {Object.entries(analysis.riskLevels).map(([level, data]) => (
                            <div key={level} className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm capitalize">
                                  {level === 'high' ? '高风险' :
                                   level === 'medium' ? '中风险' : '低风险'}
                                </span>
                              </div>
                              <span className="text-sm font-medium">
                                {(data as any).count} 人 ({((data as any).percentage).toFixed(1)}%)
                              </span>
                            </div>
                          ))}
                        </div>
            ...
            <Card>
              <CardContent className="py-12">
                <div className="text-center text-gray-500">
                  ...
                  <p className="text-sm">该考试可能还没有成绩数据，请先导入成绩后再进行分析。</p>
                </div>
              </CardContent>
            </Card>
          )}
...
const generateExamInsights = (statistics: any, warningData: any) => {
  ...
  if (statistics.passRate < 60) {
    insights.push({
      ...
      action: "建议对不及格学生进行个别辅导，分析失分原因并制定针对性提升方案。"
    });
  ...
  if (warningData.atRiskStudents > 0) {
    const riskRate = (warningData.atRiskStudents / warningData.totalStudents * 100).toFixed(1);
    insights.push({
      title: "风险学生关注",
  ...
  if (warningData.riskByClass && warningData.riskByClass.length > 0) {
    const highRiskClasses = warningData.riskByClass.filter((cls: any) => 
      cls.atRiskCount / cls.studentCount > 0.3
    );
