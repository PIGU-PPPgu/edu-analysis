    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            为学生"{warningRecord.student?.name || "未知学生"}
            "创建预警干预计划。
          </DialogDescription>
        </DialogHeader>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {scheduledDate
                      ? format(scheduledDate, "yyyy-MM-dd", { locale: zhCN })
                      : "选择计划日期"}
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-auto p-0">
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const createdAssessment =
        await createInterventionAssessment(newAssessment);
...
const ActivityList = ({
  ...
  const getActivityTypeLabel = (type: string) => {
    const option = getInterventionTypeOptions().find(
      (opt) => opt.value === type
    );
    ...
    <div className="p-4 rounded-lg border border-blue-100 bg-blue-50">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center gap-1">
          <span
            className={`font-bold text-lg ${getRatingColor(assessment.effectiveness_rating)}`}
          >
      ...
      {assessment.metrics && (
        <div className="space-y-3 mb-4">
          <div>
            ...
            <Progress
              value={assessment.metrics.attendance_improvement}
              className="h-1.5"
            />
          </div>
          <div>
            ...
            <Progress
              value={assessment.metrics.grade_improvement}
              className="h-1.5"
            />
          </div>
          <div>
            ...
            <Progress
              value={assessment.metrics.engagement_improvement}
              className="h-1.5"
            />
          </div>
        </div>
...
const PlanDetail = ({
  ...
  const calculateProgress = () => {
    ...
    const completed = plan.activities.filter(
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="p-6 space-y-6">
        ...
        {latestAssessment ? (
          ...
          (plan.activities || []).some((a) => a.status === "completed") ? (
          <div className="flex justify-between items-center">
...
const InterventionWorkflow = ({
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-800">预警干预工作流</h2>
        <CreatePlanForm
          warningRecord={warningRecord}
