...
const ClusterCard = ({
  ...
  // 计算风险指数（基于因素权重）
  const riskIndex =
    cluster.risk_factors.reduce((acc, factor) => acc + factor.weight, 0) /
...
const GenerateInterventionDialog = ({
  ...
  useEffect(() => {
    if (cluster) {
      ...
      setDescription(
        `为${cluster.cluster_name}群体设计的批量干预措施，针对${cluster.risk_factors.map((f) => f.factor).join("、")}等问题。`
      );
      ...
      <DialogContent className="sm:max-w-[550px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              为"{cluster.cluster_name}"({cluster.student_count}
              名学生)创建批量干预计划。
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-2">
              ...
              <div className="border rounded-md p-4 space-y-2">
                ...
                {interventionOptions.map((option) => {
                  const recommended = cluster.recommended_interventions.find(
                    (r) => r.type === option.value
                  );
...
const StudentList = ({
  ...
  students: StudentRiskClusterAssignment[];
}) => {
  return (
...
// 风险聚类分析主组件
const RiskClusterView: React.FC<RiskClusterViewProps> = ({
  simplified = false,
}) => {
  ...
  const [selectedCluster, setSelectedCluster] = useState<RiskCluster | null>(
    null
  );
            ...
            {clusters.map((cluster) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500 line-clamp-2">
                  主要特征:{" "}
                  {cluster.risk_factors.map((f) => f.factor).join("、")}
                </p>
              </div>
            ))}
          </div>
                ...
                <Card className="mt-8">
                  <CardHeader>
                    <div className="flex justify-between items-center">
                      <div>
                        <CardTitle>
                          {selectedCluster.cluster_name}详情
                        </CardTitle>
                        <CardDescription>
                          共{selectedCluster.student_count}名学生，风险指数
                          {(
                            (selectedCluster.risk_factors.reduce(
                              (acc, f) => acc + f.weight,
                              0
                            ) /
                              selectedCluster.risk_factors.length) *
                            100
                          ).toFixed(0)}
                          %
                        </CardDescription>
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      <div>
                        ...
                        <div className="space-y-3">
                          {selectedCluster.recommended_interventions.map(
                            (intervention, idx) => (
                              <div
                                key={idx}
                              ...
                              >
                                <div className="flex justify-between items-center">
                                  ...
                                  <Badge
                                    variant="outline"
                                    className="bg-green-50 text-green-700 border-none"
                                  >
                                    {(intervention.effectiveness * 100).toFixed(
                                      0
                                    )}
                                    % 有效
                                  </Badge>
