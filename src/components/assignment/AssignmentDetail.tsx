...
export function AssignmentDetail({
  ...
  const loadAssignmentData = async () => {
    ...
    try {
      // 并行加载所有数据
      const [knowledgePointsResult, existingScoresResult, homeworkScoreResult] =
        await Promise.all([
      ...
      if (
        existingScoresResult?.success &&
        existingScoresResult.data.length > 0
      ) {
  ...
  const saveKnowledgePointScores = async () => {
    ...
    try {
      // 准备数据
      const knowledgePointScores = Object.entries(scores).map(
        ([pointId, score]) => ({
                ...
                {knowledgePoints.map((point) => (
                  ...
                  >
                    ...
                    <Rating
                      ...
                      onChange={(value) =>
                        handleKnowledgePointScoreChange(point.id, value)
                      }
                    />
                  </div>
                ))}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setOverallScore(parseInt(e.target.value) || null)
                      }
                      className="w-24"
                    />
                    <Button
