...
const DataExport: React.FC<DataExportProps> = ({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // 调用导出回调
      if (onExport) {
        onExport(exportFormat, activeFields.map(f => f.key), exportData);
      } else {
        // 默认导出行为
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          {selectedFormat && (
            <div className="flex items-center gap-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
              <selectedFormat.icon className={cn('h-4 w-4', selectedFormat.color)} />
              <span className="text-sm text-blue-700">{selectedFormat.description}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            >
              {selectedFields.every(field => field.selected) ? '全不选' : '全选'}
            </Button>
          </div>
          ...
          <div className="text-xs text-gray-500">
            已选择 {selectedFields.filter(f => f.selected).length} / {selectedFields.length} 个字段
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-100">
          <Button 
            ...
            disabled={isExporting || selectedFields.filter(f => f.selected).length === 0}
          ...
          <div className="flex gap-2">
            <Button 
              ...
              onClick={() => {
                ...
                const body = encodeURIComponent(`请查看附件中的数据报告。\n\n导出时间: ${new Date().toLocaleString()}\n记录数量: ${data.length}条`);
