...
export const VirtualTable: React.FC<VirtualTableProps> = ({
  ...
  const measurePerformance = useCallback(
    (renderStartTime: number, rowCount?: number, columnCount?: number) => {
      ...
      if (onPerformanceMetrics) {
        onPerformanceMetrics({
          ...
          columnCount:
            columnCount ||
            headers.filter((h) => state.visibleColumns.has(h)).length,
  ...
  const handleColumnFilter = useCallback(
    (column: string, filterValue: string) => {
      const startTime = performance.now();
  ...
  const { displayHeaders, displayData, totalPages } = useMemo(() => {
    ...
    return {
      ...
      totalPages: showPagination
        ? Math.ceil(state.filteredData.length / pageSize)
        : 1,
    };
  }, [
    headers,
  ...
  const totalColumnsWidth = useMemo(() => {
    return displayHeaders.reduce(
      (sum, header) => sum + state.columnWidths[header],
      0
    );
      ...
      >
        {displayHeaders.map((header, index) => (
          <div
            ...
            style={{
              width: state.columnWidths[header],
              minWidth: state.columnWidths[header],
            }}
          >
            {/* 列标题行 */}
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <Badge variant="outline">{currentPageData}</Badge>
          ...
          {showSearch && (
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <Input
                  placeholder={
                    state.performanceMode ? "搜索关键字段..." : "搜索数据..."
                  }
        ...
        {headers.length > state.visibleColumns.size && (
          <div className="flex items-center space-x-2 mt-2">
            ...
            <div className="flex flex-wrap gap-1">
              {headers
                .filter((h) => !state.visibleColumns.has(h))
                .map((header) => (
                  <Button
                    key={header}
                    variant="outline"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t">
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-600">
                显示 {state.currentPage * pageSize + 1} -{" "}
                {Math.min(
                  (state.currentPage + 1) * pageSize,
                  state.filteredData.length
                )}{" "}
                条， 共 {state.filteredData.length} 条记录
              </span>
            </div>
...
export const GradeDataPreview: React.FC<{
  ...
  const renderCell = useCallback(
    (value: any, columnKey: string, rowIndex: number) => {
      // 如果是分数列，添加颜色标识和等级显示
      if (
        typeof value === "number" &&
        (columnKey.includes("分数") || columnKey.includes("score"))
      ) {
      ...
      if (columnKey.includes("等级") || columnKey.includes("grade")) {
        ...
        if (value === "A" || value === "优秀")
          gradeColorClass = "text-green-600 font-semibold";
        else if (value === "B" || value === "良好")
          gradeColorClass = "text-blue-600";
        else if (value === "C" || value === "中等")
          gradeColorClass = "text-orange-600";
        else if (value === "D" || value === "及格")
          gradeColorClass = "text-yellow-600";
        else if (value === "E" || value === "不及格")
