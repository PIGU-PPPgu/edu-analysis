 ...
 */
const StudentCard: React.FC<StudentCardProps> = memo(({ student, onView, onSmartAnalysis }) => {
  // 计算平均成绩和趋势
  const averageScore = student.recent_scores?.length 
    ? student.recent_scores.reduce((sum, score) => sum + score, 0) / student.recent_scores.length 
    : (student.average_score || 0);

  // 判断学习趋势
  const getTrend = () => {
    if (!student.recent_scores || student.recent_scores.length < 2) return null;
  ...
  // 获取风险等级
  const getRiskLevel = () => {
    if (averageScore >= 85) return { level: 'low', text: '优秀', color: 'bg-green-100 text-green-800' };
    if (averageScore >= 70) return { level: 'medium', text: '良好', color: 'bg-blue-100 text-blue-800' };
    if (averageScore >= 60) return { level: 'medium', text: '及格', color: 'bg-yellow-100 text-yellow-800' };
    return { level: 'high', text: '待提升', color: 'bg-red-100 text-red-800' };
  };
    ...
    <Card className="overflow-hidden hover:shadow-lg transition-all duration-200 border-l-4 border-l-primary/20">
          ...
          <div className="flex items-center">
            ...
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                ...
                {averageScore >= 95 && <Trophy className="h-4 w-4 text-yellow-500" />}
              </CardTitle>
              <CardDescription className="flex items-center gap-2">
                <span>学号: {student.student_id}</span>
                <Badge variant="outline" className="text-xs">{student.gender || "未知"}</Badge>
              </CardDescription>
            </div>
          </div>
      ...
      <CardContent className="pt-0 pb-4 space-y-3">
        ...
        <div className="flex gap-2 pt-2">
          <Button size="sm" variant="outline" onClick={() => onView(student.id)} className="flex-1">
            <UserCircle className="h-4 w-4 mr-1" />
            详情
          </Button>
          {onSmartAnalysis && (
            <Button size="sm" onClick={() => onSmartAnalysis(student.id)} className="flex-1">
