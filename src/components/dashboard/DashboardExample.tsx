              ...
              <div className="flex items-center gap-2 text-sm text-gray-600">
                ...
                {selectedClass && (
                  <Badge variant="secondary">
                    班级: {mockClasses.find(c => c.id === selectedClass)?.name}
                  </Badge>
                )}
                {selectedStudent && (
                  <Badge variant="secondary">
                    学生: {mockStudents.find(s => s.id === selectedStudent)?.name}
                  </Badge>
                )}
                {selectedSubject && (
                  <Badge variant="secondary">
                    学科: {mockSubjects.find(s => s.id === selectedSubject)?.name}
                  </Badge>
                )}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              ...
              <p className="text-sm text-gray-600">涵盖相关性、预测、画像等分析</p>
            </CardContent>
          </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger>
                  <SelectValue placeholder={selectedClass ? "请选择学生" : "请先选择班级"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="flex gap-3 pt-4">
              ...
              <Button 
                variant="outline" 
                onClick={handleResetDemo}
                disabled={!selectedClass && !selectedStudent && !selectedSubject}
              >
                重置配置
              </Button>
            </div>
