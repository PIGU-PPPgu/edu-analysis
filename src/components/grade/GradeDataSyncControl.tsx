...
export function GradeDataSyncControl() {
  ...
  const handleClearGradeData = async () => {
    ...
    setIsLoading(true);
    try {
      const { error } = await supabase.from("grades").delete().neq("id", "");
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {isLoading
                ? "..."
                : gradeStats?.totalGrades.toLocaleString() || 0}
            </div>
            <p className="text-xs text-muted-foreground">所有科目成绩记录</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">有成绩记录的学生数</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">不同科目种类</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">所有成绩平均值</p>
          </CardContent>
        </Card>
      </div>
            ...
            {gradeStats.dateRange.earliest && (
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {gradeStats.dateRange.earliest} 至{" "}
                  {gradeStats.dateRange.latest}
                </p>
              </div>
            )}
          </CardContent>
          ...
          <AlertDescription>
            <div className="space-y-2">
              ...
              <div className="grid gap-2 text-sm">
                ...
                <div>
                  耗时: {(lastSyncResult.syncDuration / 1000).toFixed(1)}s
                </div>
              </div>
...
async function syncHomeworkGrades(): Promise<{
  synced: number;
  errors: string[];
}> {
  try {
...
async function generateMockExamGrades(): Promise<number> {
  try {
    // 获取考试数据
    const { data: exams } = await supabase.from("exams").select("*").limit(3);
