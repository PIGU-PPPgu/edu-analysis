...
const getGradeLevel = (score: number): { level: string; color: string } => {
  if (score >= 90) return { level: '优秀', color: 'bg-[#B9FF66] text-black border-2 border-black' };
  if (score >= 80) return { level: '良好', color: 'bg-[#F7931E] text-white border-2 border-black' };
  if (score >= 70) return { level: '中等', color: 'bg-[#FED7D7] text-black border-2 border-black' };
  if (score >= 60) return { level: '及格', color: 'bg-[#9C88FF] text-white border-2 border-black' };
  return { level: '不及格', color: 'bg-[#FF6B6B] text-white border-2 border-black' };
};
      ...
      <Card className={cn('border-2 border-black shadow-[6px_6px_0px_0px_#FF6B6B]', className)}>
        <CardContent className="p-8">
          <div className="text-center">
            <p className="text-[#FF6B6B] mb-4 font-bold">数据加载失败: {error}</p>
            <Button 
      ...
      <CardContent className="p-0">
        ...
        <div className="flex items-center justify-between p-4 bg-[#F3F3F3] border-b-2 border-black">
          <div className="text-sm font-bold text-[#191A23]">
            {isWideFormat ? (
              <>
                共 <span className="text-[#B9FF66] bg-[#191A23] px-2 py-1 rounded">{total}</span> 名学生，
                当前第 <span className="text-[#F7931E] bg-[#191A23] px-2 py-1 rounded">{currentPage}</span> 页，
                共 <span className="text-[#9C88FF] bg-[#191A23] px-2 py-1 rounded">{totalPages}</span> 页
              </>
            ) : (
              <>
                共 <span className="text-[#B9FF66] bg-[#191A23] px-2 py-1 rounded">{total}</span> 条记录，
                当前第 <span className="text-[#F7931E] bg-[#191A23] px-2 py-1 rounded">{currentPage}</span> 页，
                共 <span className="text-[#9C88FF] bg-[#191A23] px-2 py-1 rounded">{totalPages}</span> 页
              </>
            )}
          ...
          <div className="text-sm font-bold text-[#191A23]">
            本页显示 <span className="text-[#FF6B6B] bg-[#191A23] px-2 py-1 rounded">{data.length}</span> 条
          </div>
        </div>
            ...
            <Table>
              <TableHeader className="bg-[#F3F3F3]">
                <TableRow className="border-b-2 border-black">
                  <TableHead className="font-black text-[#191A23] border-r border-gray-300">学号</TableHead>
                  <TableHead className="font-black text-[#191A23] border-r border-gray-300">姓名</TableHead>
                  <TableHead className="font-black text-[#191A23] border-r border-gray-300">班级</TableHead>
                  {isWideFormat ? (
                    // 宽格式：显示所有科目列
                    ...
                    // 长格式：显示科目、分数、等级、排名列
                    <>
                      <TableHead className="font-black text-[#191A23] border-r border-gray-300">科目</TableHead>
                      <TableHead className="font-black text-[#191A23] border-r border-gray-300 text-center">分数</TableHead>
                      <TableHead className="font-black text-[#191A23] border-r border-gray-300 text-center">等级</TableHead>
                      <TableHead className="font-black text-[#191A23] text-center">排名</TableHead>
                    </>
                  )}
                  {isWideFormat && (
                    <TableHead className="font-black text-[#191A23] text-center">平均分</TableHead>
                  )}
                </TableRow>
              </TableHeader>
              <TableBody>
                      ...
                      >
                        ...
                        <TableCell className="text-center">
                          <span className="font-bold text-[#191A23]">
                            {record.rank_in_class ? `第${record.rank_in_class}名` : '-'}
                          </span>
                        </TableCell>
                      </TableRow>
                    );
