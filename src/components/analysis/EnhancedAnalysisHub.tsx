...
// 导入增强功能库
import { DataExporter, exportTemplates, BatchExporter } from '@/lib/export-utils';
import { EnhancedAnalyzer, type GradeRecord, type TrendAnalysisResult, type PerformanceInsight } from '@/lib/enhanced-analysis';
import { EnhancedWarningSystem, type WarningAlert, type WarningAnalytics } from '@/lib/enhanced-warning-system';

// 导入UX组件
import { ErrorBoundary } from '@/components/shared/ErrorBoundary';
import { LoadingSpinner, PageLoading, EmptyState } from '@/components/shared/LoadingStates';
import { ResponsiveContainer, ResponsiveGrid, MobileCard } from '@/components/shared/ResponsiveLayout';
...
const EnhancedAnalysisHub: React.FC = () => {
  ...
  const [warningAnalytics, setWarningAnalytics] = useState<WarningAnalytics | null>(null);
  ...
  const loadInitialData = async () => {
    await withLoading(async () => {
      ...
      // 加载预警分析数据
      const analytics = await EnhancedWarningSystem.getWarningAnalytics('month');
  ...
  const analyzeStudent = async (studentId: string) => {
    ...
    const result = await handleAsyncError(async () => {
      await withLoading(async () => {
        ...
        const trendAnalysis = EnhancedAnalyzer.analyzeTrend(scores, timePoints);

        // 性能洞察
        const insights = EnhancedAnalyzer.generatePerformanceInsights(gradeRecords);

        // 预警评估
        const rules = EnhancedWarningSystem.getDefaultRules().map((rule, index) => ({
          ...rule,
          id: `rule_${index}`,
          isActive: true,
          priority: index + 1,
        ...
        const warnings = await EnhancedWarningSystem.evaluateStudent(studentId, rules);
        ...
        if (warnings.some(w => w.severity === 'critical')) {
          ...
          riskLevel = 'high';
        } else if (warnings.some(w => w.severity === 'medium') || overallScore < 70) {
          riskLevel = 'medium';
        }
  ...
  const handleExport = async (format: 'excel' | 'csv' | 'pdf' | 'json') => {
    ...
    await withLoading(async () => {
      const exportData = exportTemplates.studentGrades(analysisData.gradeRecords);
  ...
  const handleBatchExport = async () => {
    await withLoading(async () => {
      ...
      // 添加多种格式的导出任务
      if (analysisData) {
        const baseData = exportTemplates.studentGrades(analysisData.gradeRecords);
        
        batchExporter
          .addExport(baseData, { format: 'excel', fileName: `${analysisData.studentName}_详细报告.xlsx` })
          .addExport(baseData, { format: 'pdf', fileName: `${analysisData.studentName}_摘要报告.pdf` })
          .addExport(baseData, { format: 'json', fileName: `${analysisData.studentName}_数据备份.json` });
      }
  ...
  // 获取班级列表
  const classList = Array.from(new Set(allStudents.map(s => s.class_name))).sort();

  // 渲染风险级别标签
  const renderRiskBadge = (riskLevel: string) => {
    const configs = {
      low: { variant: 'secondary' as const, text: '低风险', color: 'text-green-600' },
      medium: { variant: 'default' as const, text: '中风险', color: 'text-yellow-600' },
      high: { variant: 'destructive' as const, text: '高风险', color: 'text-orange-600' },
      critical: { variant: 'destructive' as const, text: '极高风险', color: 'text-red-600' }
    };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">搜索学生</label>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">筛选班级</label>
            ...
            <TabsContent value="overview" className="space-y-6">
              <ResponsiveGrid className="grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <MobileCard className="p-6">
                  <div className="flex items-center">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-600">学生信息</p>
                      <p className="text-2xl font-bold text-gray-900 mt-1">
                        {analysisData.studentName}
                      </p>
                      <p className="text-sm text-gray-600">{analysisData.className}</p>
                    </div>
                ...
                <MobileCard className="p-6">
                  <div className="flex items-center">
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-600">成绩趋势</p>
                      <div className="flex items-center mt-1">
                        {renderTrendIcon(analysisData.trendAnalysis.trend)}
                        <span className="text-2xl font-bold text-gray-900 ml-2">
                          {analysisData.trendAnalysis.trend === 'up' ? '上升' : 
                           analysisData.trendAnalysis.trend === 'down' ? '下降' : '稳定'}
                        </span>
                      </div>
                      <p className="text-sm text-gray-600">
                        变化率: {analysisData.trendAnalysis.changeRate.toFixed(1)}%
                      </p>
                    </div>
                  </div>
                </MobileCard>
              ...
              <MobileCard className="p-6">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <h4 className="font-medium text-gray-900 mb-2">趋势概述</h4>
                      <div className="space-y-2">
                        <p className="text-sm">
                        ...
                        <p className="text-sm">
                          ...
                          <span className="ml-2 font-medium">
                            {(analysisData.trendAnalysis.trendStrength * 100).toFixed(1)}%
                          </span>
                        </p>
                        <p className="text-sm">
