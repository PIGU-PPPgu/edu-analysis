...
const AutoAnalysisTrigger: React.FC<AutoAnalysisTriggerProps> = ({
  ...
  const [analysisStatus, setAnalysisStatus] = useState<'idle' | 'triggering' | 'processing' | 'completed' | 'failed'>('idle');
  ...
  // 获取成绩数据
  const fetchGradeData = async (examTitle: string, className?: string): Promise<string> => {
    try {
      // 从Supabase获取成绩数据
      let query = supabase
        .from('grade_data')
  ...
  // 根据学生数量选择分析类型
  const getAnalysisType = (studentCount: number): 'simple' | 'detailed' | 'premium' | 'batch' => {
    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex items-center gap-2">
              <Badge className={statusDisplay.color}>
                <StatusIcon className={`h-3 w-3 mr-1 ${analysisStatus === 'processing' || analysisStatus === 'triggering' ? 'animate-spin' : ''}`} />
                {statusDisplay.text}
              </Badge>
            </div>
          </div>
                ...
                <div className="flex items-center gap-2">
                  {wechatAvailable && (
                    <Switch
                      ...
                      disabled={analysisStatus === 'processing' || analysisStatus === 'triggering'}
                    />
                  )}
                  {!wechatAvailable && (
                    <Button
          ...
          {/* 进度条 */}
          {(analysisStatus === 'triggering' || analysisStatus === 'processing') && (
