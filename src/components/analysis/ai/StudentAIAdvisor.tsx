...
const StudentAIAdvisor: React.FC<StudentAIAdvisorProps> = ({
  ...
  const startAIGuidance = async () => {
    ...
    try {
      ...
      setAnalysisError(
        error instanceof Error ? error.message : "指导过程中出现未知错误"
      );
    } finally {
...
** 关注级别**: ${
      student.priority_level === "urgent"
        ? " 紧急关注"
        : student.priority_level === "important"
          ? " 重点关注"
          : student.priority_level === "normal"
            ? " 正常关注"
            : " 保持现状"
    }
...
`;
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      case "urgent":
        return "bg-[#FF6B6B] text-white border-[#FF6B6B]";
      case "important":
        return "bg-[#F7931E] text-white border-[#F7931E]";
      case "normal":
        return "bg-[#9C88FF] text-white border-[#9C88FF]";
      case "maintain":
        return "bg-[#B9FF66] text-[#191A23] border-[#B9FF66]";
      default:
        return "bg-[#F3F3F3] text-[#191A23] border-[#191A23]";
    }
  };
  ...
  const filteredStudents =
    analysisResult?.students?.filter((student: any) => {
      const priorityMatch =
        selectedPriority === "all" ||
        student.priority_level === selectedPriority;
      const studentMatch =
        selectedStudent === "all" || student.student_id === selectedStudent;
            ...
            <CardContent className="p-6 bg-white">
              ...
              <div className="mt-6 p-4 bg-[#F8F8F8] border border-[#B9FF66] rounded-lg">
                ...
                <ul className="space-y-2">
                  {analysisResult.summary.overall_recommendations.map(
                    (rec: string, index: number) => (
                      <li
                        key={index}
                        className="text-[#191A23] font-medium flex items-start gap-2"
                      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredStudents.map((student: any, index: number) => (
              <Card
                ...
                className="border-3 border-[#191A23] shadow-[4px_4px_0px_0px_#B9FF66] bg-white"
              >
                <CardHeader className="bg-[#B9FF66]/20 border-b-2 border-[#B9FF66] p-5">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      className={`border-2 border-black font-bold ${getPriorityColor(student.priority_level)}`}
                    >
                  ...
                  <div className="p-4 bg-[#F8F8F8] border border-[#B9FF66] rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge className="bg-[#191A23] text-white text-xs">
                        {(student.learning_pattern.confidence * 100).toFixed(0)}
                        % 置信度
                      </Badge>
                    </div>
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-[#191A23]">
                        {student.learning_pattern.description}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div className="p-3 bg-[#B9FF66]/10 border border-[#B9FF66] rounded">
                        <p className="text-xs font-bold text-[#191A23] mb-1">
                          学习目标
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {student.personalized_plan.goals.map(
                            (goal: string, idx: number) => (
                              <Badge
                                key={idx}
                                className="bg-[#B9FF66] text-[#191A23] text-xs border border-black"
                              >
                                {goal}
                              </Badge>
                            )
                      ...
                      <div className="p-3 bg-[#F7931E]/10 border border-[#F7931E] rounded">
                        ...
                        <ul className="text-xs text-[#191A23] space-y-1">
                          {student.personalized_plan.methods.map(
                    ...
                    <div className="flex items-center justify-between p-3 bg-[#9C88FF]/10 border border-[#9C88FF] rounded">
                      <div>
                        ...
                        <p className="text-xs text-[#191A23]">
                          {student.personalized_plan.timeline}
                        </p>
                      </div>
                    ...
                    <div className="p-3 bg-[#191A23] text-white rounded">
                      ...
                      <p className="text-xs">
                        {student.personalized_plan.expected_outcome}
                      </p>
                    </div>
                  </div>
                </CardContent>
