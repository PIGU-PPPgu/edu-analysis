...
export const AdvancedDashboard: React.FC<AdvancedDashboardProps> = ({ 
  ...
  const { gradeData, examList } = useGradeAnalysis();
  ...
  useEffect(() => {
    ...
    const fetchAnalysisData = async () => {
      ...
      try {
        const { data, error } = await gradeAnalysisService.analyzeExamData(examId);
  ...
  useEffect(() => {
    ...
    const fetchClassRanking = async () => {
      try {
        const { data, error } = await gradeAnalysisService.getClassRanking(examId);
  ...
  const getClassSubjectData = () => {
    ...
    return analysisResult.subjectPerformance?.[selectedSubject]?.classBreakdown?.[selectedClass];
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500 pt-1">
            ...
            {examDate && <span>考试日期: {new Date(examDate).toLocaleDateString()}</span>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="flex items-baseline">
              <p className="text-2xl font-bold">{overallStats.average.toFixed(1)}</p>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-500">中位数</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-baseline">
              <p className="text-2xl font-bold">{overallStats.median.toFixed(1)}</p>
              <span className="text-sm text-gray-500 ml-2">分</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-baseline">
              <p className="text-2xl font-bold text-green-600">{overallStats.max.toFixed(1)}</p>
              <span className="text-sm text-gray-500 ml-2">分</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-baseline">
              <p className="text-2xl font-bold text-red-500">{overallStats.min.toFixed(1)}</p>
              <span className="text-sm text-gray-500 ml-2">分</span>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="distribution">
        ...
        <TabsContent value="distribution" className="pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      ...
                      <p className="text-lg font-bold">{overallStats.stdDev.toFixed(2)}</p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-medium">成绩区间分析</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  <div>
        ...
        <TabsContent value="classes" className="pt-4">
          {classRanking.length > 0 ? (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg font-medium">班级排名</CardTitle>
                </CardHeader>
                <CardContent>
                  <Table>
                    <TableHeader>
                        ...
                        >
                          ...
                          <TableCell className="text-right">
                            <div className="flex items-center justify-end gap-2">
                              <span className={classData.passRate < 0.6 ? "text-red-500" : "text-green-600"}>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-lg font-medium">班级详情</CardTitle>
                        ...
                        <dl className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <dd className="text-lg font-semibold">
                              {classRanking.find(c => c.className === selectedClass)?.studentCount || 0}
                            </dd>
                          </div>
                          <div>
                            <dt className="text-xs text-gray-500">班级排名</dt>
                            <dd className="text-lg font-semibold">
                              {classRanking.findIndex(c => c.className === selectedClass) + 1}/{classRanking.length}
                            </dd>
                          </div>
                          <div>
                            <dt className="text-xs text-gray-500">平均分</dt>
                            <dd className="text-lg font-semibold">
                              {classRanking.find(c => c.className === selectedClass)?.averageScore.toFixed(1) || 0}
                            </dd>
                          </div>
                          <div>
                            <dt className="text-xs text-gray-500">及格率</dt>
                            <dd className="text-lg font-semibold">
                              {(classRanking.find(c => c.className === selectedClass)?.passRate * 100 || 0).toFixed(1)}%
                            </dd>
                          </div>
                        </dl>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-3">
                          {subjectList.map(subject => {
                            const subjectData = 
                              (subject === "总分") 
                                ? classRanking.find(c => c.className === selectedClass)
                                : analysisResult?.subjectPerformance?.[subject]?.classBreakdown?.[selectedClass];
        ...
        <TabsContent value="subjects" className="pt-4">
          <div className="space-y-6">
            ...
            <Card>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-lg font-medium">学科分析</CardTitle>
                ...
                {subjectData ? (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div className="text-center p-3 bg-purple-50 rounded-lg">
                      <p className="text-sm font-medium text-gray-500">标准差</p>
                      <p className="text-2xl font-bold">
                        {selectedSubject === "总分" 
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right">最高分</TableHead>
                          <TableHead className="text-right">最低分</TableHead>
                          <TableHead className="text-right">对比全校平均</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {selectedSubject === "总分" ? (
                          // 总分的班级数据来自classPerformance
                          analysisResult?.classPerformance?.map((classData: any, index: number) => {
                            const diff = classData.average - overallStats.average;
                          ...
                          Object.entries(subjectData.classBreakdown || {}).map(([className, data]: [string, any]) => {
