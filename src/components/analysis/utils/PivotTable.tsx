...
const PivotTable: React.FC<PivotTableProps> = ({ 
  ...
  if (!Array.isArray(data)) {
    console.error('PivotTable: data must be an array, received:', typeof data, data);
    return <div className="text-center py-8 text-red-500">数据格式错误：需要数组类型</div>;
  }
  ...
  try {
    ...
    console.error('PivotTable rendering error:', error);
    return <div className="text-center py-8 text-red-500">数据处理错误：{error.message}</div>;
  }
};
