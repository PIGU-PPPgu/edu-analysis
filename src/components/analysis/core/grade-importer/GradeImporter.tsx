...
const GradeImporter: React.FC<GradeImporterProps> = ({ onDataImported }) => {
  ...
  const [userInterfaceMode, setUserInterfaceMode] = useState<
    "simple" | "advanced"
  >("simple");
      ...
      if (typeof actions.setFileData === "function") {
        ...
        if (fileData.aiAnalysis) {
          ...
          if (autoMappingConfig) {
            ...
            if (confidence >= 0.85) {
              console.log(
                "[GradeImporter]  高置信度AI映射，自动跳转到验证步骤"
              );
              actions.setCurrentStep("validation");
              setActiveStepIndex(2);
              toast.success(
                `AI自动映射完成 (置信度: ${Math.round(confidence * 100)}%)，请检查数据验证结果`
              );
              ...
              toast.warning(
                `AI映射置信度: ${Math.round(confidence * 100)}%，请确认字段映射`
              );
            } else {
              ...
              setActiveStepIndex(1);
              toast.info(
                `AI映射置信度较低 (${Math.round(confidence * 100)}%)，请手动确认映射`
              );
            }
          } else {
            // AI解析失败，进入手动流程
        ...
        console.error(
          "actions.setFileData 不是一个函数:",
          typeof actions.setFileData
        );
  ...
  const handleValidationComplete = useCallback(
    async (result: ValidationResult, validData: any[]) => {
      // 保存验证结果
      ...
      if (result.summary.errorRows > 0) {
        toast.warning(
          `数据验证完成，发现 ${result.summary.errorRows} 行错误数据`
        );
      } else {
        toast.success(
          `数据验证完成，共 ${result.summary.validRows} 行有效数据`
        );
      }
    },
    [actions]
  ...
  const handleStepClick = useCallback(
    (stepIndex: number) => {
      if (stepIndex <= activeStepIndex || stepIndex === 4) {
        // 配置管理可以随时访问
    ...
    <div className="w-full space-y-6">
      ...
      <Alert className="mb-4 bg-amber-50 border-amber-200">
        ...
        <AlertDescription className="text-amber-800">
          <strong>提示：</strong>
          您正在使用标准导入模式。如需更简单的导入体验，请选择"新版导入"模式。
        </AlertDescription>
      </Alert>
          ...
          <div className="space-y-4">
            {uploadedData && uploadedData.length > 0 ? (
              ...
              <Alert>
                ...
                <AlertDescription>请先上传文件数据</AlertDescription>
              </Alert>
            )}
          </div>
        )}
        ...
        {activeStepIndex === 2 && (
          <div className="space-y-4">
            {uploadedData && mappingConfig ? (
              ...
              <Alert>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>请先完成文件上传和字段映射</AlertDescription>
              </Alert>
            )}
          </div>
        )}
          ...
          <CardContent>
            ...
            {importResult.errors.length > 0 && (
              <Alert variant="destructive">
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  导入过程中发现 {importResult.errors.length}{" "}
                  个错误，请检查数据质量。
                </AlertDescription>
              </Alert>
            )}
