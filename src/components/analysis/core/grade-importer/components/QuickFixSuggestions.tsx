...
const QuickFixSuggestions: React.FC<QuickFixSuggestionsProps> = ({
  ...
  const generateQuickFixes = (): QuickFixAction[] => {
    ...
    // 2. 检查常见科目字段的智能分组
    const subjectFields = Object.entries(fieldMappings).filter(
      ([_, mapped]) => {
      ...
      if (Object.keys(suggestedMappings).length > 0) {
        fixes.push({
          ...
          fix: {
            ...
            preview: `添加映射：${Object.entries(suggestedMappings)
              .map(([k, v]) => `${k} → ${v}`)
              .join(", ")}`,
    ...
    // 4. 检查置信度低的映射
    if (
      currentMapping.aiSuggestions &&
      currentMapping.aiSuggestions.confidence < 0.7
    ) {
      const lowConfidenceFields = Object.entries(fieldMappings).filter(
        ([original, _]) => {
          return (
            getFieldConfidence(original, currentMapping.aiSuggestions) < 0.6
          );
        }
      );
    ...
    if (currentMapping.wideTableFormat?.detected && subjectFields.length > 5) {
      fixes.push({
        ...
        fix: {
          type: "reorganize_mappings",
          params: {
            fieldMappings: optimizeWideTableMappings(
              fieldMappings,
              currentMapping.wideTableFormat
            ),
          },
          preview: "优化宽表格字段映射结构",
    ...
    return fixes.filter(
      (fix) => !appliedFixes.has(fix.id) && !dismissedSuggestions.has(fix.id)
    );
  };
  ...
  // 辅助函数：检查科目组织
  const checkSubjectOrganization = (
    subjectFields: [string, string][]
  ): string[] => {
    return subjectFields
      .filter(
        ([original, mapped]) =>
          !mapped.includes("_score") && !mapped.includes("_grade")
      )
      .map(([original, _]) => original);
  };

  // 辅助函数：生成标准化科目映射
  const generateStandardizedSubjectMappings = (
    subjectFields: [string, string][]
  ...
  // 辅助函数：生成科目自定义字段
  const generateSubjectCustomFields = (
    subjectFields: [string, string][]
  ): Record<string, string> => {
  ...
  // 辅助函数：生成类型修正映射
  const generateTypeCorrectedMappings = (
    mismatches: any[]
  ): Record<string, string> => {
    const mappings: Record<string, string> = {};
    // 实现类型修正逻辑
  ...
  // 辅助函数：优化宽表格映射
  const optimizeWideTableMappings = (
    mappings: Record<string, string>,
    wideTableFormat: any
  ): Record<string, string> => {
    // 实现宽表格优化逻辑
  ...
  const quickFixes = useMemo(
    () => generateQuickFixes(),
    [headers, sampleData, currentMapping, appliedFixes, dismissedSuggestions]
  );
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Badge
                            variant={
                              action.impact === "high" ? "default" : "outline"
                            }
                            className="text-xs"
                          >
                            {action.impact === "high"
