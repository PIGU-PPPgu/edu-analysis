...
const FlexibleGradeImporter: React.FC<GradeImporterProps> = ({
  onDataImported,
}) => {
  // 状态管理
  const [importMode, setImportMode] = useState<ImportMode>("quick");
  const [currentStep, setCurrentStep] = useState<
    "mode" | "upload" | "mapping" | "confirm" | "completed"
  >("mode");
  ...
  const handleFileUploaded = useCallback(
    (uploadedFileData: any, fileInfo: any) => {
  ...
  const performSmartMapping = (data: any) => {
    ...
    subjects.forEach((subject) => {
      ...
      subjectHeaders.forEach((header) => {
        ...
        // 判断字段类型
        if (
          headerLower.includes("排名") ||
          headerLower.includes("名次") ||
          headerLower.includes("rank")
        ) {
          if (headerLower.includes("班级") || headerLower.includes("班")) {
            smartMappings[subject.key].classRank = header;
          } else if (
            headerLower.includes("年级") ||
            headerLower.includes("级") ||
            headerLower.includes("区")
          ) {
            smartMappings[subject.key].gradeRank = header;
          } else if (
            headerLower.includes("校") ||
            headerLower.includes("学校")
          ) {
            smartMappings[subject.key].schoolRank = header;
          }
        } else if (
          headerLower.includes("等级") ||
          headerLower.includes("级别") ||
  ...
  const getAvailableHeaders = (
    ...
    Object.entries(subjectMappings).forEach(([subjectKey, fields]) => {
      Object.entries(fields).forEach(([fieldType, headerName]) => {
        if (
          headerName &&
          !(subjectKey === excludeSubject && fieldType === excludeField)
        ) {
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card
                  className="cursor-pointer hover:shadow-md transition-shadow"
                  onClick={() => handleModeSelect("confirm")}
                >
                  <CardContent className="pt-6 text-center">
            ...
            <CardContent>
              ...
              <div className="space-y-6">
                {subjects.map((subject) => {
                  const hasAnyMapping = Object.values(
                    subjectMappings[subject.key]
                  ).some(Boolean);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {fieldTypes.map((fieldType) => {
                            const currentMapping =
                              subjectMappings[subject.key][fieldType.key];
