...
export const useGradeImporter = (): GradeImporterHook => {
  ...
  const generateMapping = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "字段映射生成失败";
      updateState({
  ...
  const setValidationResult = useCallback(
    (result: ValidationResult, validData: any[]) => {
      updateState({
  ...
  const startImport = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "数据导入失败";
      updateState({
  ...
  const loadConfigTemplate = useCallback(
    (templateId: string) => {
      // 从本地存储或API加载配置模板
  ...
  const saveConfigTemplate = useCallback(
    (name: string, description: string) => {
      ...
      const existingTemplates = localStorage.getItem(
