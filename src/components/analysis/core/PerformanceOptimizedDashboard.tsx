...
const StatisticCard = memo<{
  ...
  <Card className="relative overflow-hidden hover:shadow-lg transition-all duration-300 border-0 shadow-sm">
    <CardContent className="p-6">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          {trend !== undefined && (
            <div
              className={`flex items-center mt-1 text-xs ${trend > 0 ? "text-green-600" : trend < 0 ? "text-red-600" : "text-gray-500"}`}
            >
              <TrendingUp
        ...
        <div
          className={`p-3 rounded-xl ${color.replace("text-", "bg-").replace("-600", "-100")} ${color.replace("text-", "text-")}`}
        >
          {icon}
        </div>
      </div>
    ...
    {/* 渐变背景装饰 */}
    <div
      className={`absolute inset-0 bg-gradient-to-br ${color.replace("text-", "from-").replace("-600", "-50")} to-transparent opacity-5 pointer-events-none`}
    />
  </Card>
));
      ...
      return (
        <div className="bg-white p-4 shadow-xl rounded-lg border border-gray-200 backdrop-blur-sm">
          ...
          <p className="text-xs text-gray-500 mt-1">
            分数段: {data.minScore}-{data.maxScore}
          </p>
        </div>
      );
    }
...
const PerformanceOptimizedDashboard: React.FC<
  PerformanceOptimizedDashboardProps
> = ({
  gradeData,
  title = "成绩数据看板",
  loading = false,
