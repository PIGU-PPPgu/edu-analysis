  ...
  useEffect(() => {
    ...
    if (selectedAnomalyType !== "all") {
      filtered = filtered.filter(
        (item) => item.anomaly_type === selectedAnomalyType
      );
    }

    // 按置信度筛选
    filtered = filtered.filter(
      (item) => item.confidence * 100 >= confidenceThreshold
    );
    ...
    if (showOnlyCritical) {
      filtered = filtered.filter(
        (item) =>
          item.anomaly_type === "sudden_drop" ||
          item.anomaly_type === "outlier_low"
      );
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
          <div>
            ...
            <Select value={selectedExam} onValueChange={setSelectedExam}>
              ...
              <SelectContent>
                ...
                <SelectItem value="2023-term1-final">
                  23-24学年第一学期期末
                </SelectItem>
                <SelectItem value="2023-term1-mid">
                  23-24学年第一学期期中
                </SelectItem>
              </SelectContent>
            </Select>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={setSelectedAnomalyType}
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      filteredAnomalies.filter(
                        (a) => a.anomaly_type === "sudden_increase"
                      ).length
                    }
                  </p>
                </div>
            ...
            <div className="space-y-4">
              {filteredAnomalies.map((anomaly, index) => (
                <Card key={anomaly.id} className="overflow-hidden">
                  <div
                    className={`h-1 w-full ${ANOMALY_CONFIG[anomaly.anomaly_type].color}`}
                  />
                  <CardContent className="p-4 pt-5">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                      ...
                      {anomaly.previous_scores &&
                        anomaly.previous_scores.length > 0 && (
                          <div className="flex items-end gap-1 h-14 min-w-[150px]">
                            {anomaly.previous_scores.map((score, i) => {
                              ...
                              const isLast =
                                i === anomaly.previous_scores!.length - 1;
                              return (
                                <div
                                  key={i}
