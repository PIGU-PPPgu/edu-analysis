  ...
  Object.entries(classByName).forEach(([className, records]) => {
    ...
    Object.entries(SUBJECT_CONFIG).forEach(([subject, config]) => {
      const scores = records
        .map((r) => (r[config.field as keyof WideGradeRecord] as number) || 0)
        .filter((s) => s > 0);
    ...
    const excellentRate =
      totalScores.length > 0 ? (excellentCount / totalScores.length) * 100 : 0;
    const passRate =
      totalScores.length > 0 ? (passCount / totalScores.length) * 100 : 0;
    ...
    const balanceScore = Math.min(
      100,
      Object.values(subjectStrengths).reduce(
...
const MultiDimensionalRankingSystem: React.FC<
  MultiDimensionalRankingSystemProps
> = ({ gradeData, className = "" }) => {
  const [selectedDimension, setSelectedDimension] =
    useState<keyof ClassRankingData["dimensionScores"]>("academic");
  const [showDetails, setShowDetails] = useState(true);
  const [sortBy, setSortBy] = useState<
    "comprehensive" | "academic" | "improvement" | "stability"
  >("comprehensive");
  const [viewMode, setViewMode] = useState<"table" | "chart" | "radar">(
    "table"
  );

  // 计算班级排名数据
  const classRankings = useMemo(
    () => calculateClassRankings(gradeData),
    [gradeData]
  );
    ...
    switch (sortBy) {
      case "academic":
        return rankings.sort(
          (a, b) => b.dimensionScores.academic - a.dimensionScores.academic
        );
      case "improvement":
        return rankings.sort(
          (a, b) =>
            b.dimensionScores.improvement - a.dimensionScores.improvement
        );
      case "stability":
        return rankings.sort(
          (a, b) => b.dimensionScores.stability - a.dimensionScores.stability
        );
      default:
    ...
    return [
      {
        ...
        ...sortedRankings.reduce(
          (acc, ranking) => ({
            ...acc,
            [ranking.className]: ranking.dimensionScores.academic,
          }),
          {}
        ),
      },
      {
        ...
        ...sortedRankings.reduce(
          (acc, ranking) => ({
            ...acc,
            [ranking.className]: ranking.dimensionScores.stability,
          }),
          {}
        ),
      },
      {
        ...
        ...sortedRankings.reduce(
          (acc, ranking) => ({
            ...acc,
            [ranking.className]: ranking.dimensionScores.improvement,
          }),
          {}
        ),
      },
      {
        ...
        ...sortedRankings.reduce(
          (acc, ranking) => ({
            ...acc,
            [ranking.className]: ranking.dimensionScores.balance,
          }),
          {}
        ),
      },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-black text-[#191A23] mb-2">
              {(
                classRankings.reduce((sum, c) => sum + c.averageScore, 0) /
                classRankings.length
              ).toFixed(1)}
            </div>
            <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">
                ...
                >
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`px-4 py-2 rounded-lg border-2 border-black font-black shadow-[2px_2px_0px_0px_#191A23] ${getGradeColor(ranking.competitiveIndex)}`}
                        >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="p-3 bg-[#B9FF66]/20 border-2 border-[#B9FF66] rounded-lg">
                          ...
                          <p className="text-xs text-[#191A23]/70">
                            优秀率: {ranking.excellentRate.toFixed(1)}%
                          </p>
                        </div>
                        ...
                        <div className="p-3 bg-white border-2 border-[#6B7280] rounded-lg">
                          ...
                          <p className="text-xs text-[#191A23]/70">
                            标准差: {ranking.standardDeviation.toFixed(1)}
                          </p>
                        </div>
                ...
                <RadarChart data={radarData}>
                  ...
                  {sortedRankings.slice(0, 5).map((ranking, index) => (
                    <Radar
                      ...
                      stroke={
                        Object.values(SUBJECT_CONFIG)[
                          index % Object.values(SUBJECT_CONFIG).length
                        ].color
                      }
                      fill={
                        Object.values(SUBJECT_CONFIG)[
