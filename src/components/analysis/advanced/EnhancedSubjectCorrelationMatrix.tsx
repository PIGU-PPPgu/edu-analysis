...
const SUBJECT_CONFIG = {
  '语文': { scoreField: 'chinese_score', gradeField: 'chinese_grade', color: '#6B7280' },
  '数学': { scoreField: 'math_score', gradeField: 'math_grade', color: '#000000' },
  '英语': { scoreField: 'english_score', gradeField: 'english_grade', color: '#6B7280' },
  '物理': { scoreField: 'physics_score', gradeField: 'physics_grade', color: '#191A23' },
  '化学': { scoreField: 'chemistry_score', gradeField: 'chemistry_grade', color: '#6B7280' },
  '生物': { scoreField: 'biology_score', gradeField: 'biology_grade', color: '#000000' },
  '历史': { scoreField: 'history_score', gradeField: 'history_grade', color: '#6B7280' },
  '地理': { scoreField: 'geography_score', gradeField: 'geography_grade', color: '#191A23' },
  '政治': { scoreField: 'politics_score', gradeField: 'politics_grade', color: '#6B7280' }
} as const;

// 高级皮尔逊相关系数计算（包含置信区间）
const calculateAdvancedCorrelation = (x: number[], y: number[]): Omit<EnhancedCorrelationData, 'subject1' | 'subject2'> => {
  ...
  const confidence95Lower = (Math.exp(2 * zLower) - 1) / (Math.exp(2 * zLower) + 1);
  const confidence95Upper = (Math.exp(2 * zUpper) - 1) / (Math.exp(2 * zUpper) + 1);
...
const calculateWideTableCorrelationMatrix = (gradeData: WideGradeRecord[]): EnhancedCorrelationData[] => {
  const subjects = Object.keys(SUBJECT_CONFIG) as Array<keyof typeof SUBJECT_CONFIG>;
  const correlations: EnhancedCorrelationData[] = [];

  // 预处理数据 - 为每个科目提取有效分数
  const subjectScores = subjects.reduce((acc, subject) => {
    const scoreField = SUBJECT_CONFIG[subject].scoreField as keyof WideGradeRecord;
  ...
  for (let i = 0; i < subjects.length; i++) {
    for (let j = i + 1; j < subjects.length; j++) {
      ...
      const scores1Map = new Map(subjectScores[subject1].map(item => [item.studentId, item.score]));
      const scores2Map = new Map(subjectScores[subject2].map(item => [item.studentId, item.score]));
      
      // 找到同时有两个科目成绩的学生
      const commonStudentIds = Array.from(scores1Map.keys()).filter(id => scores2Map.has(id));
...
const getCorrelationStyle = (correlation: number, significance: string) => {
  ...
  if (isPositive) {
    if (absCorr >= 0.7) return { bg: 'bg-[#B9FF66]', text: 'text-[#191A23]', border: 'border-black', intensity: 'high' };
    if (absCorr >= 0.4) return { bg: 'bg-[#B9FF66]/70', text: 'text-[#191A23]', border: 'border-black', intensity: 'medium' };
    return { bg: 'bg-[#B9FF66]/40', text: 'text-[#191A23]', border: 'border-black', intensity: 'low' };
  } else {
    if (absCorr >= 0.7) return { bg: 'bg-[#6B7280]', text: 'text-white', border: 'border-black', intensity: 'high' };
    if (absCorr >= 0.4) return { bg: 'bg-[#6B7280]/70', text: 'text-white', border: 'border-black', intensity: 'medium' };
    return { bg: 'bg-[#6B7280]/40', text: 'text-[#191A23]', border: 'border-black', intensity: 'low' };
  }
};

const EnhancedSubjectCorrelationMatrix: React.FC<EnhancedSubjectCorrelationMatrixProps> = ({
  gradeData,
  title = "增强版科目相关性矩阵",
  className = "",
  showHeatMap = true,
  ...
  const correlations = useMemo(() => calculateWideTableCorrelationMatrix(gradeData), [gradeData]);
  ...
  const handleExportData = () => {
    const csvContent = [
      ['科目1', '科目2', '相关系数', '置信区间下限', '置信区间上限', 'P值', '样本量', '显著性', '协方差', '科目1均分', '科目2均分', '科目1标准差', '科目2标准差'],
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[#191A23]/80 font-medium mt-1">
                  分析 {availableSubjects.length} 个科目 | {filteredCorrelations.length} 个科目对 | 平均相关性 {averageCorrelation.toFixed(3)}
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#B9FF66] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#B9FF66]">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-black text-[#191A23] mb-2">{availableSubjects.length}</div>
        ...
        <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-black text-[#191A23] mb-2">{filteredCorrelations.length}</div>
            <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">分析科目对</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
          <CardContent className="p-6 text-center">
            ...
            <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">置信区间</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="space-y-4">
            {filteredCorrelations.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-xl font-black text-[#191A23] uppercase tracking-wide mb-2">当前筛选条件下暂无数据</p>
                <p className="text-[#191A23]/70 font-medium">请调整筛选条件或检查数据完整性</p>
              </div>
            ) : (
              filteredCorrelations.map((corr, index) => {
                const style = getCorrelationStyle(corr.correlation, corr.significance);
                return (
                  <Card key={index} className="border-2 border-black shadow-[2px_2px_0px_0px_#191A23] transition-all hover:translate-x-[-1px] hover:translate-y-[-1px] hover:shadow-[4px_4px_0px_0px_#191A23]">
                    <CardContent className="p-6">
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2">
                              <div className={`px-3 py-1 rounded-lg text-lg font-black shadow-[2px_2px_0px_0px_#191A23] border-2 ${style.border} ${style.bg} ${style.text}`}>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="p-3 bg-[#B9FF66]/20 border-2 border-[#B9FF66] rounded-lg">
                            <p className="font-bold text-[#191A23] mb-1">统计检验</p>
                            <p className="text-[#191A23]/80">p值: {corr.pValue.toFixed(4)}</p>
                            <p className="text-[#191A23]/80">样本: {corr.sampleSize}名学生</p>
                          </div>
                          <div className="p-3 bg-white border-2 border-[#6B7280] rounded-lg">
                            <p className="font-bold text-[#191A23] mb-1">置信区间</p>
                            <p className="text-[#191A23]/80">下限: {corr.confidence95Lower.toFixed(3)}</p>
                            <p className="text-[#191A23]/80">上限: {corr.confidence95Upper.toFixed(3)}</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-sm">
                          <div className="p-3 bg-white border-2 border-[#6B7280] rounded-lg">
                            <p className="font-bold text-[#191A23] mb-1">{corr.subject1}</p>
                            <p className="text-[#191A23]/80">均分: {corr.meanScore1.toFixed(1)}</p>
                            <p className="text-[#191A23]/80">标准差: {corr.stdDev1.toFixed(1)}</p>
                          </div>
                          <div className="p-3 bg-white border-2 border-[#6B7280] rounded-lg">
                            <p className="font-bold text-[#191A23] mb-1">{corr.subject2}</p>
                            <p className="text-[#191A23]/80">均分: {corr.meanScore2.toFixed(1)}</p>
                            <p className="text-[#191A23]/80">标准差: {corr.stdDev2.toFixed(1)}</p>
                          </div>
                        </div>
                      </div>
                    </CardContent>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <div>
                        <p className="font-black text-[#191A23] text-xl mb-3">
                          {corr.subject1} 与 {corr.subject2} 强{corr.correlation > 0 ? '正' : '负'}相关
                        </p>
                        <p className="font-medium text-[#191A23] leading-relaxed mb-3">
                          相关系数: <span className="font-black text-[#191A23]">{corr.correlation.toFixed(3)}</span> 
                          (95%置信区间: {corr.confidence95Lower.toFixed(3)} - {corr.confidence95Upper.toFixed(3)})
                        </p>
                        <p className="font-medium text-[#191A23] leading-relaxed">
                          ...
                          <span className="font-black">{corr.correlation > 0 ? '呈现强正向关联' : '呈现强反向关联'}</span>
                        </p>
                      </div>
                      
                      <div className="p-4 bg-white/50 border-2 border-[#191A23] rounded-lg">
                        <p className="font-black text-[#191A23] mb-2">教学建议</p>
