...
// 计算线性回归和趋势分析
const calculateTrendAnalysis = (scores: number[]): { slope: number; correlation: number; volatility: number } => {
...
// 处理学生趋势数据
const processStudentTrendData = (gradeData: WideGradeRecord[], studentId: string): StudentTrendData[] => {
...
// 分析所有科目趋势
const analyzeAllSubjectTrends = (trendData: StudentTrendData[]): TrendAnalysisResult[] => {
...
const StudentTrendAnalysis: React.FC<StudentTrendAnalysisProps> = ({
  ...
  const radarData = useMemo(() => {
    ...
    return Object.entries(SUBJECT_CONFIG)
      .filter(([_, config]) => latestData[config.field as keyof StudentTrendData])
      .map(([subject, config]) => ({
        subject,
  ...
  const handleExportData = () => {
    ...
    const csvContent = [
      ['考试', '日期', '总分', '班级排名', '年级排名', ...Object.keys(SUBJECT_CONFIG)],
    ...
    link.setAttribute('download', `${studentOptions.find(s => s.id === selectedStudent)?.name || '学生'}_成绩趋势分析.csv`);
    ...
    return (
      <Card className={`bg-white border-2 border-black shadow-[6px_6px_0px_0px_#B9FF66] ${className}`}>
        <CardContent className="p-12 text-center">
          ...
          <p className="text-[#191A23]/70 font-medium">请先导入学生成绩数据进行趋势分析</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">最新总分</div>
              </CardContent>
            </Card>
            ...
            <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">进步科目数</div>
              </CardContent>
            </Card>
            ...
            <Card className="border-2 border-black shadow-[4px_4px_0px_0px_#6B7280] transition-all hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_#6B7280]">
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm font-bold text-[#191A23] uppercase tracking-wide">班级排名</div>
              </CardContent>
            </Card>
          </div>
                ...
                {viewMode === 'radar' && radarData.length > 0 && (
                  <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={radarData}>
                      ...
                      <PolarAngleAxis dataKey="subject" tick={{ fill: '#191A23', fontWeight: 'bold' }} />
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className={`p-2 rounded-full border-2 border-black ${
                            analysis.trend === 'improving' ? 'bg-[#B9FF66]' :
                            analysis.trend === 'declining' ? 'bg-[#6B7280]' :
                            'bg-white'
                          }`}>
                          ...
                          <div>
                            <p className="font-black text-[#191A23] text-lg">{analysis.subject}</p>
                            <p className="text-sm font-medium text-[#191A23]/70">
                              平均分: {analysis.averageScore.toFixed(1)} | 最新: {analysis.latestScore} | 
                              变化: {analysis.improvement > 0 ? '+' : ''}{analysis.improvement.toFixed(1)}分
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                          <Badge className={`font-bold shadow-[2px_2px_0px_0px_#191A23] border-2 border-black ${
                            analysis.trend === 'improving' ? 'bg-[#B9FF66] text-[#191A23]' :
                            analysis.trend === 'declining' ? 'bg-[#6B7280] text-white' :
                            'bg-[#6B7280] text-white'
                          }`}>
                            {analysis.trend === 'improving' ? '进步中' :
                             analysis.trend === 'declining' ? '需关注' : '稳定'}
                          </Badge>
                          <div className="text-right">
                            <p className="text-lg font-black text-[#191A23]">{analysis.latestScore}</p>
      ...
      {selectedStudent && studentTrendData.length === 0 && (
        <Card className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_#6B7280]">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-2xl font-black text-[#191A23] uppercase tracking-wide mb-3">该学生暂无考试数据</p>
            <p className="text-[#191A23]/70 font-medium">请确保该学生至少参加了一次考试</p>
          </CardContent>
        </Card>
      )}
    </div>
