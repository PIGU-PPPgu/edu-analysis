        ...
        <div className="flex items-start justify-between">
          <div className="space-y-3 flex-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-base font-bold text-[#191A23] uppercase tracking-wide">
                {title}
              </p>
            </div>
            ...
            <div className="space-y-2">
              <h3 className="text-4xl font-black text-[#191A23] leading-none">
                {value}
              </h3>
...
const ModernGradeAnalysisDashboard: React.FC = () => {
      ...
      return {
        ...
        percentage:
          statistics.totalRecords > 0
            ? (count / statistics.totalRecords) * 100
            : 0,
        color: range.color,
      };
    });
  ...
  if (error) {
    return (
      <Alert
        variant="destructive"
        className="max-w-2xl mx-auto border-2 border-black shadow-[6px_6px_0px_0px_#FF6B6B]"
      >
      ...
      {statistics && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            color="green"
            trend={
              statistics.avgScore >= 80
                ? "up"
                : statistics.avgScore >= 60
                  ? "neutral"
                  : "down"
            }
            trendValue={
          ...
          <StatCard
            ...
            color="purple"
            trend={
              (statistics.totalScoreStats?.passRate || 0) >= 80
                ? "up"
                : (statistics.totalScoreStats?.passRate || 0) >= 60
                  ? "neutral"
                  : "down"
            }
            trendValue={
          ...
          <StatCard
            ...
            color="yellow"
            trend={
              (statistics.totalScoreStats?.excellentRate || 0) >= 20
                ? "up"
                : (statistics.totalScoreStats?.excellentRate || 0) >= 10
                  ? "neutral"
                  : "down"
            }
            trendValue={
                          ...
                          return (
                            <div className="bg-white p-3 border-2 border-black shadow-[4px_4px_0px_0px_#B9FF66] rounded-lg">
                              ...
                              <p className="font-bold text-[#F7931E]">
                                {payload[0].value}人 (
                                {Math.round(
                                  Number(payload[0].payload.percentage) * 10
                                ) / 10}
                                %)
                              </p>
                            </div>
                            ...
                            if (active && payload && payload.length) {
                              return (
                                <div className="bg-white p-3 border-2 border-black shadow-[4px_4px_0px_0px_#F7931E] rounded-lg">
                                  <p className="font-black text-[#191A23]">
                                    {payload[0].payload.grade}等级
                                  </p>
                                  <p className="font-bold text-[#F7931E]">
                                    {payload[0].value}人 (
                                    {Math.round(
                                      payload[0].payload.percentage * 10
                                    ) / 10}
                                    %)
                                  </p>
                                </div>
                      ...
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-white p-3 border-2 border-black shadow-[4px_4px_0px_0px_#F7931E] rounded-lg">
                            ...
                            <p className="font-bold text-[#F7931E]">
                              平均分: {payload[0].value}分
                            </p>
                            <p className="font-bold text-[#B9FF66]">
                              及格率: {payload[0].payload.passRate}%
                            </p>
                            <p className="font-bold text-[#191A23]">
                              参与人数: {payload[0].payload.count}人
                            </p>
                          </div>
                        );
                      }
                      ...
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-white p-3 border-2 border-black shadow-[4px_4px_0px_0px_#9C88FF] rounded-lg">
                            <p className="font-black text-[#191A23]">{label}</p>
                            <p className="font-bold text-[#9C88FF]">
                              平均分: {payload[0].value}分
                            </p>
                            <p className="font-bold text-[#B9FF66]">
                              及格率: {payload[0].payload.passRate}%
                            ...
                            <p className="font-bold text-[#F7931E]">
                              学生数: {payload[0].payload.studentCount}人
                            </p>
                          </div>
                        );
                      }
