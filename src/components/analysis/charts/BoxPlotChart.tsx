...
const BoxPlotChart: React.FC<BoxPlotChartProps> = ({
  ...
  // SVG配置 - 🔧 放大图表
  const chartWidth = Math.max(800, validatedData.length * 150); // 增加宽度
  const chartHeight = height - 60; // 减少上下留白
  const plotWidth = chartWidth - 120; // 左右边距
  const plotHeight = chartHeight - 60; // 上下边距
  const leftMargin = 80; // 增加左边距
  const topMargin = 30; // 增加上边距
  const boxWidth = 60; // 增大箱体宽度

  // 坐标转换函数
  const toPercent = (value: number, fullScore: number) =>
    normalizeByPercent ? (value / fullScore) * 100 : value;

  const scaleY = (value: number) => {
    const normalized = (value - yMin) / (yMax - yMin);
    return plotHeight - normalized * plotHeight + topMargin;
  };

  const getXPosition = (index: number) => {
    const spacing = plotWidth / validatedData.length;
    return leftMargin + spacing * index + spacing / 2;
  };
    ...
    <div className="w-full">
      ...
      <div className="w-full overflow-x-auto flex justify-center">
        ...
        >
          ...
          <text
            x={25}
            y={plotHeight / 2 + topMargin}
            textAnchor="middle"
            fontSize="16"
            fill="#191A23"
            fontWeight="bold"
            transform={`rotate(-90, 25, ${plotHeight / 2 + topMargin})`}
          >
