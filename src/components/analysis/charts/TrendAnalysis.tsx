...
const TrendAnalysis: React.FC<TrendAnalysisProps> = ({
  ...
  if (trendData.length === 0) {
    return (
      <Card
        className={`border-2 border-black shadow-[4px_4px_0px_0px_#9C88FF] ${className}`}
      >
        <CardContent className="p-8 text-center">
          <div className="p-4 bg-[#9C88FF] rounded-full border-2 border-black mx-auto mb-4 w-fit">
            <TrendingUp className="h-8 w-8 text-white" />
          </div>
          <p className="text-lg font-black text-[#191A23] uppercase tracking-wide">
            暂无趋势数据
          </p>
          <p className="text-[#191A23]/70 font-medium mt-2">
            需要多次考试数据来分析趋势
          </p>
        </CardContent>
      </Card>
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-black text-[#191A23] uppercase tracking-wide">
          成绩趋势
        </h3>
        <Badge
          className={`font-bold border-2 border-black shadow-[2px_2px_0px_0px_#191A23] ${
            trend.direction === "up"
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="p-2 bg-[#F7931E]/20 border border-[#F7931E] rounded">
          <div className="font-black text-[#191A23]">
            {Math.round(
              trendData.reduce((sum, point) => sum + point.average, 0) /
                trendData.length
            )}
          </div>
        ...
        <div className="p-2 bg-[#9C88FF]/20 border border-[#9C88FF] rounded">
          <div className="font-black text-[#191A23]">
            {Math.max(...trendData.map((p) => p.average))}
          </div>
