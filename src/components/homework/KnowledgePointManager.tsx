    ...
    try {
      ...
      if (result.success) {
        setKnowledgePoints((prev) =>
          prev.filter((point) => point.id !== selectedPointId)
        );
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {knowledgePoints.map((point) => (
            <Card
              key={point.id}
              className={`${point.is_ai_generated ? "border-blue-200 bg-blue-50/30" : ""}`}
            >
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              您正在删除知识点 "{selectedPointName}
              "。此操作不可撤销，且如果该知识点已用于评分，则无法删除。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
