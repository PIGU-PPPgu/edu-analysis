...
const StudentHomeworkList = () => {
  ...
  const [currentStudentId, setCurrentStudentId] = React.useState<string | null>(
    null
  );
  ...
  const fetchHomeworks = async () => {
    try {
      ...
      const homeworksData =
        await getStudentHomeworkSubmissions(currentStudentId);
    ...
    <div className="space-y-6">
      ...
      {homeworks.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {homeworks.map((homework) => {
            ...
            const isOverdue =
              dueDate < new Date() &&
              (!isSubmitted || submission.status === "pending");
      ...
      {selectedHomework && (
        <SubmitHomeworkDialog
          ...
          onSubmitted={(content) =>
            handleSubmitHomework(selectedHomework.id, content)
          }
        />
      )}
    </div>
