...
const ManageHomeworkCriteriaDialog: React.FC<
  ManageHomeworkCriteriaDialogProps
> = ({ open, onOpenChange, homeworkId, onUpdated }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      // 处理评分标准
      const criteriaToAdd = criteria.filter(
        (item) => item.isNew && !item.isDeleted
      );
      const criteriaToDelete = criteria.filter(
        (item) => !item.isNew && item.isDeleted
      );
      const criteriaToUpdate = criteria.filter(
        (item) => !item.isNew && !item.isDeleted
      );
      ...
      // 处理知识点
      const knowledgeToAdd = knowledgePoints.filter(
        (item) => item.isNew && !item.isDeleted
      );
      const knowledgeToDelete = knowledgePoints.filter(
        (item) => !item.isNew && item.isDeleted
      );
      const knowledgeToUpdate = knowledgePoints.filter(
        (item) => !item.isNew && !item.isDeleted
      );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[800px]">
        <DialogHeader>
          ...
          <DialogDescription>为作业定义评分标准和相关知识点</DialogDescription>
        </DialogHeader>
                  ...
                  .map((item, index) => (
                    <Card key={item.id || `new-${index}`}>
                      <CardContent className="p-4 space-y-3">
                        ...
                        <div className="space-y-2">
                          <Label htmlFor={`criteria-description-${index}`}>
                            描述
                          </Label>
                          <Textarea
            ...
            {criteria.filter((item) => !item.isDeleted).length > 0 && (
              <div className="flex items-center justify-between pt-2">
                ...
                {Math.abs(
                  criteria
                    .filter((item) => !item.isDeleted)
                    .reduce((sum, item) => sum + Number(item.weight), 0) - 100
                ) > 0.001 && (
                ...
                knowledgePoints
                  ...
                  .map((item, index) => (
                    <Card key={item.id || `new-${index}`}>
                      <CardContent className="p-4 space-y-3">
                        <div className="space-y-2">
                          <Label htmlFor={`knowledge-name-${index}`}>
                            知识点名称
                          </Label>
                          <Input
                        ...
                        <div className="space-y-2">
                          <Label htmlFor={`knowledge-description-${index}`}>
                            描述
                          </Label>
                          <Textarea
