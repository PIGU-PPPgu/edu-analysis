...
export function SecurityDemo() {
  ...
  const handleInputValidation = () => {
    ...
    const fileResult = testFile ? security.validateFile(testFile, {
  ...
  const handleSecureApiCall = async () => {
    try {
      const result = await supabaseSecureClient.queryTable('students', {
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <Badge variant={security.isAuthenticated ? "default" : "secondary"}>
          ...
          {security.isAuthenticated && (
            <div className="mt-4 p-3 bg-muted rounded-lg">
              ...
              <div className="mt-2 flex gap-4 text-sm">
                <span className={security.isAdmin ? "text-green-600" : "text-gray-500"}>
                  管理员: {security.isAdmin ? "是" : "否"}
                </span>
                <span className={security.isTeacher ? "text-blue-600" : "text-gray-500"}>
                  教师: {security.isTeacher ? "是" : "否"}
                </span>
                <span className={security.isStudent ? "text-orange-600" : "text-gray-500"}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1">文本输入验证</label>
            <Input
              value={testInput}
            ...
            {validationResults.input && (
              <Alert className={validationResults.input.isValid ? "border-green-200" : "border-red-200"}>
                <AlertDescription>
                  <strong>验证结果:</strong> {validationResults.input.isValid ? "有效" : "无效"}
                  {!validationResults.input.isValid && (
                    <ul className="mt-1 list-disc list-inside">
                      {validationResults.input.errors.map((error: string, idx: number) => (
                        <li key={idx} className="text-red-600">{error}</li>
                      ))}
                    </ul>
                  )}
                  {validationResults.input.sanitized && (
                    <p className="mt-1"><strong>清理后:</strong> {validationResults.input.sanitized}</p>
                  )}
                </AlertDescription>
              </Alert>
            )}
          ...
          <div>
            ...
            {validationResults.email && (
              <Alert className={validationResults.email.isValid ? "border-green-200" : "border-red-200"}>
                <AlertDescription>
                  <strong>邮箱验证:</strong> {validationResults.email.isValid ? "有效" : "无效"}
                  {!validationResults.email.isValid && (
                    <ul className="mt-1 list-disc list-inside">
                      {validationResults.email.errors.map((error: string, idx: number) => (
                        <li key={idx} className="text-red-600">{error}</li>
                      ))}
                    </ul>
                  )}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">密码强度验证</label>
            <Input
              type="password"
              value={testPassword}
            ...
            {validationResults.password && (
              <Alert className={validationResults.password.isValid ? "border-green-200" : "border-red-200"}>
                <AlertDescription>
                  <strong>密码强度:</strong> {validationResults.password.isValid ? "符合要求" : "不符合要求"}
                  {!validationResults.password.isValid && (
                    <ul className="mt-1 list-disc list-inside">
                      {validationResults.password.errors.map((error: string, idx: number) => (
                        <li key={idx} className="text-red-600">{error}</li>
                      ))}
                    </ul>
                  )}
          ...
          <div>
            ...
            {validationResults.file && (
              <Alert className={validationResults.file.isValid ? "border-green-200" : "border-red-200"}>
                <AlertDescription>
                  <strong>文件验证:</strong> {validationResults.file.isValid ? "有效" : "无效"}
                  <p>类型: {validationResults.file.mimeType}</p>
                  <p>大小: {(validationResults.file.size / 1024).toFixed(2)} KB</p>
                  {!validationResults.file.isValid && (
                    <ul className="mt-1 list-disc list-inside">
                      {validationResults.file.errors.map((error: string, idx: number) => (
                        <li key={idx} className="text-red-600">{error}</li>
                      ))}
                    </ul>
                  )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleDataProtection} disabled={!security.isAuthenticated}>
            测试数据保护
          </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <Button onClick={handlePermissionReport} disabled={!security.isAuthenticated}>
            生成权限报告
          </Button>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleSecureApiCall} disabled={!security.isAuthenticated}>
            执行安全API调用
          </Button>
