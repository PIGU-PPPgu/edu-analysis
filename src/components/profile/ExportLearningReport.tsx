...
const ExportLearningReport: React.FC<ExportLearningReportProps> = ({ student }) => {
  const [isGenerating, setIsGenerating] = useState(false);
  const [reportFormat, setReportFormat] = useState("pdf");
  const [reportSections, setReportSections] = useState<ReportSection[]>([
    { id: "overview", title: "成绩概览", description: "包含学生成绩数据和统计信息", selected: true },
    { id: "learning_style", title: "学习风格分析", description: "学生的学习风格和偏好", selected: true },
    { id: "behavior", title: "学习行为分析", description: "学习习惯和行为模式", selected: true },
    { id: "ability", title: "能力评估", description: "学生各项能力的评估结果", selected: true },
    { id: "ai_analysis", title: "AI学习画像分析", description: "AI生成的综合学习评估", selected: false },
  ]);
  ...
  const toggleAll = (selected: boolean) => {
    setReportSections(prev => prev.map(section => ({ ...section, selected })));
  };
  
  // 生成报告
  const handleGenerateReport = () => {
    const selectedSections = reportSections.filter(section => section.selected);
