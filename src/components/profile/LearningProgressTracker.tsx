...
const LearningProgressTracker: React.FC<LearningProgressTrackerProps> = ({
  ...
  const calculateImprovementRate = () => {
    ...
    return { 
      ...
      trend: parseFloat(rate) > 0 ? 'positive' : parseFloat(rate) < 0 ? 'negative' : 'neutral'
    };
  };
  ...
  const getAchievementIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'flag': return <Flag className="h-5 w-5 text-red-500" />;
  ...
  const getAchievementTypeBadge = (type: string) => {
    switch (type) {
      case 'academic':
        return <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-100">学术成就</Badge>;
      case 'behavior':
        return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">行为表现</Badge>;
      case 'milestone':
        return <Badge className="bg-red-100 text-red-800 hover:bg-red-100">学习里程碑</Badge>;
      case 'improvement':
        return <Badge className="bg-green-100 text-green-800 hover:bg-green-100">进步成就</Badge>;
      default:
            ...
            <Card className="bg-slate-50">
              <CardContent className="p-4">
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <span className={`text-2xl font-bold ${improvementInfo.trend === 'positive' ? 'text-green-600' : improvementInfo.trend === 'negative' ? 'text-red-600' : 'text-gray-600'}`}>
                    {improvementInfo.rate > 0 ? '+' : ''}{improvementInfo.rate}%
                  </span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="p-4">
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <span className="text-2xl font-bold text-indigo-600">
                    {progressData[progressData.length - 1]?.studentScore || '暂无'}
                  </span>
                </div>
              </CardContent>
            </Card>
            ...
            <ChartContainer config={{
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Tooltip />
                  <Legend />
                  <Line type="monotone" dataKey="studentScore" name="学生成绩" stroke="#10b981" activeDot={{ r: 8 }} strokeWidth={2} />
                  <Line type="monotone" dataKey="average" name="班级平均" stroke="#94a3b8" strokeDasharray="3 3" />
                </LineChart>
              </ResponsiveContainer>
            </ChartContainer>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {achievements.map((achievement) => (
                <div key={achievement.id} className="flex items-start gap-3 p-3 rounded-lg border bg-slate-50">
