...
const LearningStyleAnalysis: React.FC<LearningStyleAnalysisProps> = ({ 
  ...
  const dominantStyle = React.useMemo(() => {
    return learningStyleData.reduce((prev, current) => (prev.value > current.value) ? prev : current);
  }, [learningStyleData]);
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
              ...
              <p className="font-bold text-indigo-600 mb-2">{dominantStyle.name} ({dominantStyle.value}%)</p>
              <p className="text-sm text-slate-600">{dominantStyle.description}</p>
            </div>
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              ...
              <p className="text-sm text-blue-700 mb-3">
                该学生是一个{dominantStyle.name.replace("型学习", "")}型学习者，适合通过
                {dominantStyle.name === "视觉型学习" && "图像、视频等视觉资料"}
                {dominantStyle.name === "听觉型学习" && "听讲、讨论等听觉活动"}
                {dominantStyle.name === "读写型学习" && "阅读、记笔记等文字活动"}
                {dominantStyle.name === "实践型学习" && "动手实践、实验等体验活动"}
                进行学习。
              </p>
              <p className="text-sm text-blue-700">
                建议教师在教学过程中，针对该学生的学习特点，增加
                {dominantStyle.name === "视觉型学习" && "图表、视频等视觉辅助工具"}
                {dominantStyle.name === "听觉型学习" && "讨论、讲解等听觉互动环节"}
                {dominantStyle.name === "读写型学习" && "阅读材料、写作练习等活动"}
                {dominantStyle.name === "实践型学习" && "实验、角色扮演等实践性活动"}
                的比例，以提高学习效果。
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 space-y-3">
          ...
          {learningStyleData.map((style, index) => (
            ...
            >
              <p className="font-medium text-sm mb-1" style={{ color: style.color }}>{style.name} ({style.value}%)</p>
