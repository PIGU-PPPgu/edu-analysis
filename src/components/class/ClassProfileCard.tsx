...
export const ClassProfileCard: React.FC<ClassProfileCardProps> = ({
  classData,
}) => {
    ...
    <Card className="w-full shadow-lg">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg">
          ...
          <Badge
            className={`${excellentColors.bg} ${excellentColors.text} border-0`}
          >
            {(classData.excellentRate || 0) > 80
              ? "表现优秀"
              : (classData.excellentRate || 0) > 60
                ? "稳步提升"
                : "需要关注"}
          </Badge>
        </div>
