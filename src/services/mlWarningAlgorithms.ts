...
export class GradeTrendPredictor {
   ...
   * 简单线性回归实现
   */
  private static linearRegression(y: number[]): {
    slope: number;
    intercept: number;
    r_squared: number;
  } {
...
export class RiskScoreCalculator {
  ...
  static calculateRisk(
    ...
    if (volatilityRisk > 40)
      riskFactors.push(`成绩波动较大(标准差${std.toFixed(1)})`);
    ...
    if (grades.length > 0 && grades[0].subject_scores) {
      ...
      const subjectStd = Math.sqrt(
        subjectScores.reduce((sum, s) => sum + Math.pow(s - subjectAvg, 2), 0) /
...
export class SimilarStudentFinder {
  ...
  private static createFeatureVector(student: StudentMLData): number[] {
    ...
    const ranks = student.grades
      .map((g) => g.total_rank_in_class)
      .filter((r) => r > 0);
    const rankTrend =
      ranks.length >= 2 ? ranks[ranks.length - 1] - ranks[0] : 0;
    ...
    for (const cfg of config.filter((c) => c.enabled)) {
      ...
      switch (cfg.type) {
        ...
        case "similar_students":
          result = SimilarStudentFinder.findSimilarStudents(
            studentData,
            allStudents
          );
          break;
