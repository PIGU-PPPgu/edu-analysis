...
import { warningDataIntegrationService, type WarningTriggerEvent } from "./warningDataIntegrationService";
import { enhancedAITagsService, type AITagsGenerationConfig } from "./enhancedAITagsService";
import { gradeDataValidator, type ValidationReport, type ValidationOptions } from "./gradeDataValidator";
...
export class AutoSyncService {
  ...
  async syncImportedData(
    ...
    console.log('🤖 [AutoSync] 开始智能数据同步，处理', gradeData.length, '条成绩记录');
    ...
    try {
      ...
      const validationReport = await this.validateGradeData(gradeData, validationOptions);
      ...
      if (!validationReport.success) {
        ...
        result.errors.push(`数据校验失败: 发现 ${validationReport.summary.critical} 个严重错误`);
      ...
      console.log('✅ [AutoSync] 数据校验通过，质量评分:', validationReport.dataQuality.score);
  ...
  private async detectAndCreateClasses(gradeData: any[]): Promise<ClassInfo[]> {
    // 1. 提取所有班级名称
    const classNamesFromData = [...new Set(gradeData.map(record => record.class_name))]
    ...
    // 3. 识别新班级
    const newClassNames = classNamesFromData.filter(name => !existingClassNames.has(name));
   ...
   * 包含数据质量检查和智能合并策略
   */
  private async detectAndCreateStudents(gradeData: any[]): Promise<StudentInfo[]> {
    // 1. 数据质量预检查和清理
    ...
    const studentsFromData = new Map<string, {name: string, class_name: string, rawRecords: any[]}>();
    ...
    const { data: existingStudents, error: queryError } = await supabase
      .from('students')
      .select(`
        id,
        student_id,
    ...
    // 4. 智能重复检测和潜在合并分析
    const duplicateAnalysis = this.analyzeDuplicateStudents(studentsFromData, existingStudents);
    ...
    console.log('📋 [AutoSync] 现有学生示例键:', [...existingStudentKeys].slice(0, 5));
    ...
    console.log('🆕 [AutoSync] 需要创建的新学生:', newStudentsData.length, '名');
    
    // 打印需要创建的学生详情（调试用）
    if (newStudentsData.length > 0) {
      console.log('🔍 [AutoSync] 新学生详情:', newStudentsData.map(s => `${s.name}(${s.class_name})`));
    }
    ...
    for (const studentData of newStudentsData) {
      try {
        ...
        if (!classId) {
          console.warn(`⚠️ 找不到班级 ${studentData.class_name} 的ID，跳过学生 ${studentData.name}`);
          continue;
        }

        const studentId = this.generateStudentId(studentData.name, studentData.class_name);
        const newStudent = {
          id: crypto.randomUUID(),
        ...
        if (insertError) {
          ...
          throw new Error(`创建学生 ${studentData.name} 失败: ${insertError.message}`);
        }

        console.log(`✅ [AutoSync] 成功创建学生: ${studentData.name} (${studentData.class_name})`);
        newStudents.push({
          id: newStudent.id,
          student_id: studentId,
          name: studentData.name,
  ...
  private async syncStudentClassRelations(gradeData: any[]): Promise<void> {
    ...
    console.log('📋 [AutoSync] 需要同步关联的学生:', studentClassMap.size, '名');
    ...
    for (const student of existingStudents) {
      ...
      const expectedClassId = expectedClassName ? classIdMap.get(expectedClassName) : null;

      // 检查是否需要更新关联
      if (expectedClassName && expectedClassId && student.class_id !== expectedClassId) {
        try {
          console.log(`🔄 [AutoSync] 更新学生关联: ${student.name} -> ${expectedClassName} (${currentClassName || '无班级'} => ${expectedClassName})`);
          ...
          if (updateError) {
            console.error(`❌ 更新学生 ${student.name} 班级关联失败:`, updateError);
          } else {
            console.log(`✅ [AutoSync] 成功更新学生关联: ${student.name} -> ${expectedClassName}`);
            syncedCount++;
            
            // 同时更新grade_data_new表中的student_id引用（确保使用正确的UUID）
            await this.updateGradeDataStudentId(student.name, expectedClassName, student.id);
          }
        } catch (error) {
    ...
    console.log(`✅ [AutoSync] 关联关系同步完成: ${syncedCount} 个更新, ${skippedCount} 个跳过`);
  }
   ...
   */
  private async updateGradeDataStudentId(studentName: string, className: string, studentUuid: string): Promise<void> {
    try {
      console.log(`🔗 [AutoSync] 更新成绩数据中的学生ID引用: ${studentName} -> ${studentUuid}`);
      ...
      if (updateError) {
        console.error(`❌ 更新成绩数据学生ID引用失败 (${studentName}):`, updateError);
      } else {
  ...
  private async updateClassStatistics(): Promise<number> {
    ...
    console.log(`✅ [AutoSync] 统计数据更新完成，处理了 ${updatedCount} 个班级`);
    return updatedCount;
  }
   ...
   * 智能分析班级名称，提取年级信息
   */
  private analyzeClassName(className: string): { grade: string; level: string } {
    // 简单的智能分析逻辑
   ...
   */
  private async createClassInfoRecord(classData: any, classInfo: any): Promise<void> {
    try {
  ...
  async validateSyncStatus(): Promise<{
    ...
    const [
      ...
      supabase.from('grade_data_new').select('*', { count: 'exact', head: true }),
      supabase.from('grade_data_new').select('class_name').limit(1000)
    ]);

    const uniqueGradeClasses = [...new Set(gradeClasses?.map(g => g.class_name) || [])];
  ...
  private analyzeDuplicateStudents(
    newStudentsMap: Map<string, {name: string, class_name: string, rawRecords: any[]}>,
    existingStudents: any[]
  ): {
    exactDuplicates: string[],
    potentialDuplicates: Array<{newKey: string, existingStudent: any, similarity: number}>,
    ...
    const potentialDuplicates: Array<{newKey: string, existingStudent: any, similarity: number}> = [];
    ...
    [...newStudentsMap.entries()].forEach(([newKey, newStudent]) => {
      ...
      existingStudents?.forEach(existing => {
        ...
        if (similarity > 0.8 && similarity < 1.0) { // 80%以上相似度但不完全相同
          potentialDuplicates.push({
            newKey,
            existingStudent: existing,
            similarity
   ...
   * 在数据同步完成后通知预警系统刷新数据
   */
  private async triggerWarningSystemUpdate(syncResult: SyncResult): Promise<void> {
    try {
      // 收集受影响的班级名称
      ...
      console.log('🚨 [AutoSync] 触发预警系统更新，受影响班级:', affectedClassesArray);
      
      // 异步触发预警更新，不阻塞主流程
      warningDataIntegrationService.triggerWarningUpdate(warningEvent).catch(error => {
  ...
  async validateStudentDataIntegrity(classId?: string): Promise<{
    ...
    classIdMismatches: Array<{studentName: string, expectedClass: string, actualClass: string}>,
    summary: {
    ...
    const classIdMismatches: Array<{studentName: string, expectedClass: string, actualClass: string}> = [];

    try {
      // 获取学生数据
      const studentsQuery = supabase.from('students').select('id, student_id, name, class_id, classes(name)');
      if (classId) {
      ...
      // 获取成绩数据
      const gradeQuery = supabase.from('grade_data_new').select('student_id, name, class_name');
   ...
   */
  private async generateAITagsForNewStudents(syncResult: SyncResult, aiConfig: AITagsGenerationConfig): Promise<void> {
    try {
      ...
      console.log(`🧠 [AutoSync] 开始为 ${syncResult.newStudents.length} 名新学生生成AI标签...`);

      // 提取新学生的ID列表
      const newStudentIds = syncResult.newStudents.map(student => student.id);

      // 调用增强的AI标签服务
      const aiTagsResult = await enhancedAITagsService.generateTagsForNewStudents(
        newStudentIds, 
        aiConfig
      );
      ...
      if (aiTagsResult.failed.length > 0) {
        console.warn('⚠️ [AutoSync] 部分学生AI标签生成失败:', 
          aiTagsResult.failed.map(f => `${f.studentId}: ${f.error}`).slice(0, 3)
        );
        
        // 将AI标签失败信息添加到错误列表中，但不影响整体成功状态
        syncResult.errors.push(`AI标签生成部分失败：${aiTagsResult.failed.length} 个学生`);
      }

      if (aiTagsResult.skipped.length > 0) {
        console.log('ℹ️ [AutoSync] 部分学生因数据不足跳过AI标签生成:', 
          aiTagsResult.skipped.map(s => `${s.studentId}: ${s.reason}`).slice(0, 3)
        );
      }
      ...
      // 记录错误但不中断主流程
      syncResult.errors.push(`AI标签生成失败: ${error instanceof Error ? error.message : '未知错误'}`);
      syncResult.aiTagsResult = {
        successful: 0,
  ...
  private async validateGradeData(
    ...
    try {
      ...
      const report = await gradeDataValidator.validateGradeData(gradeData, validationOptions);
      ...
      if (report.dataQuality.score < 70) {
        console.warn(`⚠️ [AutoSync] 数据质量偏低 (${report.dataQuality.score}分)，建议检查数据源`);
        toast.warning(`数据质量评分: ${report.dataQuality.score}分 (${report.dataQuality.label})`, {
          description: '建议修复数据质量问题以获得更好的分析结果'
        });
      } else {
        console.log(`✅ [AutoSync] 数据质量良好 (${report.dataQuality.score}分)`);
      }
  ...
  async quickDataQualityCheck(gradeData: any[]): Promise<{
    ...
    try {
      ...
      const hasRequiredFields = gradeData.every(record => 
        record.student_id && record.name && record.class_name
      );
