...
export const DATABASE_FIELDS = {
  ...
  // 科目分数字段（与数据库表完全匹配）
  scores: {
    chinese_score: { name: '语文分数', type: 'number', range: [0, 150], dbColumn: 'chinese_score', subject: 'chinese' },
    math_score: { name: '数学分数', type: 'number', range: [0, 150], dbColumn: 'math_score', subject: 'math' },
    english_score: { name: '英语分数', type: 'number', range: [0, 150], dbColumn: 'english_score', subject: 'english' },
    physics_score: { name: '物理分数', type: 'number', range: [0, 100], dbColumn: 'physics_score', subject: 'physics' },
    chemistry_score: { name: '化学分数', type: 'number', range: [0, 100], dbColumn: 'chemistry_score', subject: 'chemistry' },
    biology_score: { name: '生物分数', type: 'number', range: [0, 100], dbColumn: 'biology_score', subject: 'biology' },
    politics_score: { name: '政治分数', type: 'number', range: [0, 100], dbColumn: 'politics_score', subject: 'politics' },
    history_score: { name: '历史分数', type: 'number', range: [0, 100], dbColumn: 'history_score', subject: 'history' },
    geography_score: { name: '地理分数', type: 'number', range: [0, 100], dbColumn: 'geography_score', subject: 'geography' },
    total_score: { name: '总分', type: 'number', range: [0, 900], dbColumn: 'total_score', subject: 'total' }
  },
  
  // 等级字段（标准等级系统）
  grades: {
    chinese_grade: { name: '语文等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'chinese_grade', subject: 'chinese' },
    math_grade: { name: '数学等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'math_grade', subject: 'math' },
    english_grade: { name: '英语等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'english_grade', subject: 'english' },
    physics_grade: { name: '物理等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'physics_grade', subject: 'physics' },
    chemistry_grade: { name: '化学等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'chemistry_grade', subject: 'chemistry' },
    biology_grade: { name: '生物等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'biology_grade', subject: 'biology' },
    politics_grade: { name: '政治等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'politics_grade', subject: 'politics' },
    history_grade: { name: '历史等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'history_grade', subject: 'history' },
    geography_grade: { name: '地理等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'geography_grade', subject: 'geography' },
    total_grade: { name: '总分等级', type: 'string', values: ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'E'], dbColumn: 'total_grade', subject: 'total' }
  },
  
  // 排名字段（与实际数据库结构匹配）
  ranks: {
    rank_in_class: { name: '班级排名', type: 'number', range: [1, 100], dbColumn: 'rank_in_class' },
    rank_in_grade: { name: '年级排名', type: 'number', range: [1, 1000], dbColumn: 'rank_in_grade' },
    rank_in_school: { name: '校排名', type: 'number', range: [1, 3000], dbColumn: 'rank_in_school' }
  },
  ...
  examInfo: {
    ...
    exam_scope: { name: '考试范围', type: 'string', values: ['class', 'grade', 'school'], dbColumn: 'exam_scope' }
  },
  
  // 系统元数据字段
  metadata: {
    metadata: { name: '元数据', type: 'jsonb', auto: true, dbColumn: 'metadata' },
    created_by: { name: '创建者', type: 'uuid', auto: true, dbColumn: 'created_by' },
    created_at: { name: '创建时间', type: 'timestamp', auto: true, dbColumn: 'created_at' },
    updated_at: { name: '更新时间', type: 'timestamp', auto: true, dbColumn: 'updated_at' }
  }
};
...
export class IntelligentFieldValidator {
   ...
   * 验证字段映射的完整性和准确性
   */
  validateMapping(headers: string[], currentMappings: Record<string, string>, sampleData: any[]): ValidationResult {
    ...
    headers.forEach(header => {
      if (!currentMappings[header]) {
        const analysis = this.analyzeUnmappedField(header, sampleData, headers.indexOf(header));
    ...
    if (unmappedFields.length > 0) {
      suggestions.push(`发现 ${unmappedFields.length} 个未映射字段，建议使用智能字段确认对话框进行映射`);
      
      // 统计未映射字段类型
      const scoreFields = unmappedFields.filter(f => f.suggestedType === 'score').length;
      const gradeFields = unmappedFields.filter(f => f.suggestedType === 'grade').length;
      const rankFields = unmappedFields.filter(f => f.suggestedType === 'classRank' || f.suggestedType === 'gradeRank' || f.suggestedType === 'schoolRank').length;
      
      if (scoreFields > 0) suggestions.push(`包含 ${scoreFields} 个可能的分数字段`);
      if (gradeFields > 0) suggestions.push(`包含 ${gradeFields} 个可能的等级字段`);
      if (rankFields > 0) suggestions.push(`包含 ${rankFields} 个可能的排名字段`);
    }
    
    if (missingRequired.length > 0) {
      suggestions.push(`⚠️ 缺少必需字段：${missingRequired.map(f => DATABASE_FIELDS.required[f]?.name).join('、')}`);
    ...
    const hasHighConfidenceUnmapped = unmappedFields.filter(f => f.confidence > 0.7).length;
   ...
   * 分析未映射字段
   */
  private analyzeUnmappedField(header: string, sampleData: any[], columnIndex: number): UnmappedField {
   ...
   * 分析样本数据
   */
  private analyzeSampleData(sampleValues: string[]): { suggestedType: string; confidence: number; reason: string } {
