...
export class DatabaseDiagnostics {
      ...
      try {
        ...
        if (error) {
          ...
          if (tableName === 'grade_data' && (count || 0) === 0) {
            ...
            health.tables[tableName].suggestions.push('请检查成绩数据是否已正确导入');
          }
        }
      } catch (error) {
        health.tables[tableName] = {
  ...
  private async checkDataIntegrity(health: DatabaseHealth): Promise<void> {
    try {
      ...
      // 检查重复的成绩记录
      const { data: duplicateGrades, error: dupError } = await supabase
        .rpc('check_duplicate_grades');
   ...
   * 检查约束和索引
   */
  private async checkConstraintsAndIndexes(health: DatabaseHealth): Promise<void> {
    try {
      // 检查grade_data表的唯一约束
      const { data: constraintInfo, error } = await supabase
        .rpc('check_table_constraints', { table_name: 'grade_data' });
      
      if (!error && constraintInfo) {
        // 分析约束信息
        const hasCorrectUniqueConstraint = constraintInfo.some((constraint: any) => 
  ...
  private evaluateOverallHealth(health: DatabaseHealth): void {
    ...
    // 统计数据完整性问题
    const { orphanedRecords, duplicateRecords, missingReferences } = health.dataIntegrity;
  ...
  private async fixGradeDataConstraints(): Promise<DiagnosticResult> {
    try {
      ...
      // 删除错误的约束（如果存在）
      const wrongConstraint = currentConstraints?.find((c: any) => 
        c.constraint_name === 'grade_data_exam_id_student_id_key'
      );
  ...
  async checkExamDataIntegrity(examId: string): Promise<DiagnosticResult> {
    try {
      ...
      return {
        ...
        suggestions: unmatchedStudents > 0 ? 
