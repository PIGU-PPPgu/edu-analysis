...
export class WarningDataIntegrationService {
   ...
   * 数据完整性检查和自动修复
   */
  async checkAndRepairDataIntegrity(
    classNames?: string[]
    ...
    try {
      ...
      // 2. 识别缺少student_id的记录
      const recordsWithMissingIds = gradeData.filter(
        (record) => !record.student_id || record.student_id.trim() === ""
      );
      ...
      if (recordsWithMissingIds.length > 0) {
        const fixedCount = await this.repairMissingStudentIds(
          recordsWithMissingIds
        );
      ...
      if (uniqueStudentIds.length > 0) {
        ...
        const orphanedIds = uniqueStudentIds.filter(
      ...
      if (classInconsistencies.length > 0) {
        ...
        const classFixedCount =
          await this.repairClassInconsistencies(classInconsistencies);
        report.fixedRecords += classFixedCount;
      }

      console.log(
        `✅ [预警集成] 数据完整性检查完成，修复了 ${report.fixedRecords} 条记录`
      );
      return report;
    } catch (error) {
        ...
        try {
          ...
          if (student) {
            ...
            if (!updateError) {
              fixedCount++;
              console.log(
                `✅ 修复 ${record.name}(${record.class_name}) 的student_id引用`
              );
            }
          }
        } catch (error) {
          console.warn(
            `⚠️ 无法修复 ${record.name}(${record.class_name}):`,
            error
          );
        }
      }
   ...
   * 检查班级关联一致性
   */
  private async checkClassConsistency(gradeData: any[]): Promise<
    Array<{
      studentName: string;
   ...
   * 修复班级关联不一致
   */
  private async repairClassInconsistencies(
      ...
      try {
        ...
        if (correctClass) {
          ...
          if (!updateError) {
            fixedCount++;
            console.log(
              `✅ 修复 ${inconsistency.studentName} 的班级关联: ${inconsistency.studentTableClassName} -> ${inconsistency.gradeClassName}`
            );
          }
        }
      } catch (error) {
        console.warn(
          `⚠️ 无法修复 ${inconsistency.studentName} 的班级关联:`,
          error
        );
      }
    }
  ...
  async triggerWarningUpdate(event: WarningTriggerEvent): Promise<boolean> {
    ...
    try {
      // 1. 先进行数据完整性检查和修复
      const integrityReport = await this.checkAndRepairDataIntegrity(
        event.affectedClasses
      );
...
// 导出单例实例
export const warningDataIntegrationService =
  new WarningDataIntegrationService();
