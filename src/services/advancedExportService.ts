...
class AdvancedExportService {
  ...
  // PDF导出实现（简化HTML转PDF）
  private async generatePDF(data: any[], fields: string[], title: string): Promise<Blob> {
    const headers = fields.length > 0 ? fields : Object.keys(data[0] || {});
  ...
  // 导出学生学习报告
  async exportStudentReport(studentId: string, options: ExportOptions): Promise<ExportResult> {
    try {
      // 获取学生基本信息
      ...
      const reportData = {
        ...
        统计信息: {
          ...
          平均分: gradeData?.length ? 
            (gradeData.reduce((sum, g) => sum + (g.total_score || 0), 0) / gradeData.length).toFixed(2) : 0,
          最高分: Math.max(...(gradeData?.map(g => g.total_score || 0) || [0])),
          最低分: Math.min(...(gradeData?.map(g => g.total_score || 0) || [100]))
        }
      };

      return await this.processExport([reportData], options, `${studentData.name}学习报告`);
    } catch (error) {
      return {
        success: false,
  ...
  // 导出班级分析报告
  async exportClassAnalysis(className: string, options: ExportOptions): Promise<ExportResult> {
    try {
      const { data, error } = await supabase
        .from('grade_data_new')
        .select('*')
      ...
      return await this.processExport([analysisData], options, `${className}班级分析报告`);
    } catch (error) {
      return {
        success: false,
  ...
  // 通用导出处理
  private async processExport(data: any[], options: ExportOptions, title: string): Promise<ExportResult> {
    const fileName = options.fileName || `${title}_${new Date().toISOString().split('T')[0]}`;
  ...
  private calculateClassStats(data: any[]): any {
    ...
    return {
      总分统计: {
        平均分: scores.length ? (scores.reduce((a, b) => a + b, 0) / scores.length).toFixed(2) : 0,
        最高分: Math.max(...scores),
        最低分: Math.min(...scores),
        及格率: scores.length ? ((scores.filter(s => s >= 60).length / scores.length) * 100).toFixed(1) + '%' : '0%'
      },
      科目统计: subjects.reduce((acc, subject) => {
        const subjectScores = data.map(d => d[`${subject}_score`]).filter(s => s != null && s > 0);
        if (subjectScores.length > 0) {
          acc[subject] = {
            平均分: (subjectScores.reduce((a, b) => a + b, 0) / subjectScores.length).toFixed(2),
  ...
  // 批量导出多个班级
  async exportMultipleClasses(classNames: string[], options: ExportOptions): Promise<ExportResult> {
    try {
      const { data, error } = await supabase
        .from('grade_data_new')
        .select('*')
  ...
  // 按考试导出成绩
  async exportByExam(examTitle: string, options: ExportOptions): Promise<ExportResult> {
    try {
      const { data, error } = await supabase
        .from('grade_data_new')
        .select('*')
      ...
      return await this.processExport(data || [], options, `${examTitle}_成绩单`);
    } catch (error) {
      return {
        success: false,
