...
export class UnmappedFieldHandler {
  ...
  private readonly standardFields = {
    // 基础信息
    'student_id': { display: '学号', patterns: ['学号', 'id', 'student_id', '考生号', '编号'] },
    'name': { display: '姓名', patterns: ['姓名', 'name', '学生姓名', '考生姓名'] },
    'class_name': { display: '班级', patterns: ['班级', 'class', '现班', '所在班级'] },
    ...
    'politics_score': { display: '政治成绩', patterns: ['政治', 'politics', '道法'] },
    'history_score': { display: '历史成绩', patterns: ['历史', 'history'] },
    'geography_score': { display: '地理成绩', patterns: ['地理', 'geography'] },
    'total_score': { display: '总分', patterns: ['总分', 'total', '合计', '总成绩'] },
    
    // 排名字段
    'rank_in_class': { display: '班级排名', patterns: ['班级排名', '班排名', '班内排名'] },
    'rank_in_grade': { display: '年级排名', patterns: ['年级排名', '年排名', '级排名'] },
    'rank_in_school': { display: '学校排名', patterns: ['学校排名', '校排名', '全校排名'] }
  };
  ...
  analyzeUnmappedFields(
    ...
    // 找出未映射的字段
    const unmappedFieldNames = headers.filter(header => !existingMappings[header]);
   ...
   * 🔬 检测字段数据类型
   */
  private detectFieldType(fieldName: string, sampleValues: any[]): { type: 'score' | 'grade' | 'rank' | 'text' | 'unknown'; confidence: number } {
    ...
    // 基于字段名的类型推断
    if (fieldLower.includes('分数') || fieldLower.includes('成绩') || fieldLower.includes('score')) {
      // 检查是否都是数字且在合理范围内
      const numericValues = sampleValues.filter(val => !isNaN(Number(val))).map(Number);
    ...
    if (fieldLower.includes('排名') || fieldLower.includes('名次') || fieldLower.includes('rank')) {
      const numericValues = sampleValues.filter(val => !isNaN(Number(val))).map(Number);
      if (numericValues.length / sampleValues.length > 0.8) {
    ...
    if (fieldLower.includes('等级') || fieldLower.includes('grade')) {
      ...
      const gradeMatches = sampleValues.filter(val => 
   ...
   * 💡 生成字段映射建议
   */
  private generateFieldSuggestions(fieldName: string, typeAnalysis: { type: string; confidence: number }): FieldSuggestion[] {
    ...
    for (const [standardField, config] of Object.entries(this.standardFields)) {
      for (const pattern of config.patterns) {
        if (fieldLower.includes(pattern.toLowerCase())) {
          const confidence = this.calculateMatchConfidence(fieldName, pattern, typeAnalysis);
          suggestions.push({
    ...
    const subjectPatterns = ['chinese', 'math', 'english', 'physics', 'chemistry', 'biology', 'politics', 'history', 'geography'];
    ...
    for (let i = 0; i < subjects.length; i++) {
      ...
      if (fieldLower.includes(subject) || fieldLower.includes(subjectEn)) {
        const confidence = this.calculateMatchConfidence(fieldName, subject, typeAnalysis);
        ...
        switch (typeAnalysis.type) {
          ...
          case 'rank':
            ...
            } else if (fieldLower.includes('年级') || fieldLower.includes('grade')) {
              suggestions.push({
    ...
    if (suggestions.length === 0 || Math.max(...suggestions.map(s => s.confidence)) < 0.5) {
      const customFieldName = this.generateCustomFieldName(fieldName, typeAnalysis.type);
      suggestions.push({
  ...
  applyUserMappings(
    ...
    customFieldsToCreate: Array<{ name: string; type: string; displayName: string }>;
  } {
    const mappings: Record<string, string> = {};
    const customFieldsToCreate: Array<{ name: string; type: string; displayName: string }> = [];
    ...
    for (const field of unmappedFields) {
      ...
      if (selectedTarget) {
        ...
        // 检查是否需要创建自定义字段
        const suggestion = field.suggestions.find(s => s.targetField === selectedTarget);
  ...
  generateMappingSummary(result: FieldMappingResult): string {
    const totalFields = Object.keys(result.mappedFields).length + result.unmappedFields.length;
