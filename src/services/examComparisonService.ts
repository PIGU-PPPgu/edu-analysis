...
export class ExamComparisonService {
      ...
      data.forEach((record) => {
        ...
        if (!examMap.has(key)) {
          examMap.set(key, {
            ...
            exam_date:
              record.exam_date || new Date().toISOString().split("T")[0],
  ...
  async compareExams(examIds: string[]): Promise<ExamComparisonResult[]> {
    try {
      ...
      console.log(
        `✅ [ExamComparison] 完成 ${results.length} 个考试的对比分析`
      );
      return results;
    } catch (error) {
  ...
  private async analyzeExam(
    ...
    try {
      ...
      const examInfo: ExamInfo = {
        ...
        exam_date:
          gradeData[0].exam_date || new Date().toISOString().split("T")[0],
      ...
      const classStats = Array.from(classStatsMap.values())
        .map((classData) => {
          ...
          const variance =
            scores.reduce(
              (acc: number, score: number) =>
                acc + Math.pow(score - average, 2),
              0
            ) / scores.length;
          ...
          return {
            ...
            excellentRate:
              Math.round((excellentCount / scores.length) * 100 * 10) / 10,
      ...
      const overallAverage =
        gradeData.reduce((sum, record) => sum + (record.total_score || 0), 0) /
        totalParticipants;
  ...
  async analyzeStudentTrend(
    ...
    try {
      ...
      subjects.forEach((subject) => {
        const subjectScores = examHistory
          .map((exam) => exam.subjectScores[subject])
          .filter((score) => score > 0);
        if (subjectScores.length >= 2) {
          const subjectTrendAnalysis =
            this.calculateTrendAnalysis(subjectScores);
          subjectTrends[subject] = {
            trend: this.determineTrend(
              subjectTrendAnalysis.slope,
              subjectTrendAnalysis.correlation
            ),
            ...
            improvement:
              subjectScores[0] - subjectScores[subjectScores.length - 1],
          };
        }
      });
      ...
      return {
        ...
        examHistory,
        trendAnalysis: {
          overallTrend: this.determineTrend(
            trendAnalysis.slope,
          ...
          bestExam: {
            examTitle: bestExam.examTitle,
            score: bestExam.totalScore,
          },
          worstExam: {
            examTitle: worstExam.examTitle,
            score: worstExam.totalScore,
          },
        },
        subjectTrends,
        predictions,
  ...
  async compareClasses(
    startDate?: string,
    endDate?: string
  ): Promise<ClassComparisonResult | null> {
    try {
      ...
      const { data: gradeData, error } = await query.order("exam_date", {
        ascending: false,
      });
      ...
      const classStudentExamMap = new Map<
        string,
        Map<
          string,
          Map<
      ...
      // 第二步：计算班级表现数据
      const classPerformance = Array.from(classStudentExamMap.entries())
        .map(([className, studentMap]) => {
          ...
          const averageScore =
            allStudentScores.reduce((a, b) => a + b, 0) /
            allStudentScores.length;
          const trendAnalysis =
            examAverages.length >= 2
              ? this.calculateTrendAnalysis(examAverages)
              : { slope: 0, correlation: 0, volatility: 0 };

          return {
            className,
            studentCount: Math.round(
              examStudentCounts.reduce((a, b) => a + b, 0) /
                examStudentCounts.length
            ), // 平均参考人数
            ...
            improvement:
              examAverages.length >= 2
                ? examAverages[0] - examAverages[examAverages.length - 1]
                : 0,
            rank: 0, // 将在后面计算
            percentile: 0, // 将在后面计算
          };
      ...
      classPerformance.forEach((classData, index) => {
        ...
        classData.percentile = Math.round(
          ((classPerformance.length - index) / classPerformance.length) * 100
        );
      });
      ...
      subjectScoreMap.forEach((classScoreMap, subject) => {
        const classSubjectStats = Array.from(classScoreMap.entries())
      ...
      const insights = {
        ...
        mostImprovedClass:
          classPerformance.sort((a, b) => b.improvement - a.improvement)[0]
            ?.className || "无",
        needsAttentionClasses: classPerformance
      ...
      const comparisonPeriod = {
        ...
        examCount: new Set(gradeData.map((r) => r.exam_title || r.exam_id))
          .size,
      };
   ...
   * 计算线性回归和趋势分析
   */
  private calculateTrendAnalysis(scores: number[]): {
    slope: number;
    correlation: number;
    volatility: number;
  } {
  ...
  private generatePredictions(examHistory: any[], trendAnalysis: any): any {
    ...
    if (examHistory.length >= 3) {
      ...
      predictions.nextExamPrediction = Math.max(
        0,
        Math.min(100, lastScore + predictedChange)
      );
