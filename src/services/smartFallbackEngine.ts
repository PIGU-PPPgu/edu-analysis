...
// 字段重要性等级
export type FieldImportance = 'critical' | 'important' | 'optional' | 'nice_to_have';
...
export class SmartFallbackEngine {
  ...
  private readonly recognitionPatterns: Record<string, string[]> = {
    ...
    'student_id': ['学号', '考生号', '学生号', 'id', 'student_id', '编号', '考号'],
  ...
  private evaluateAIResult(
    ...
    // 检查是否包含关键字段
    const hasCriticalFields = Object.values(aiMapping).some(field => 
      this.fieldImportance[field] === 'critical'
    );
  ...
  private executePatternMatching(
    ...
    Object.entries(this.recognitionPatterns).forEach(([systemField, patterns]) => {
  ...
  private executeMinimalViable(
    headers: string[],
    sampleData: any[]
  ): { mappings: Record<string, string>; confidence: number; hasCriticalFields: boolean } {
  ...
  private createSuccessResult(
    ...
    const postImportActions = this.generatePostImportActions(missingFields, headers);
   ...
   * 计算整体置信度
   */
  private calculateOverallConfidence(mappings: Record<string, string>, headers: string[]): number {
    const mappedCount = Object.keys(mappings).length;
    const totalCount = headers.length;
    const coverageRatio = mappedCount / Math.max(totalCount, 1);
    
    // 检查关键字段覆盖
    const criticalCovered = Object.values(mappings).some(field => 
      this.fieldImportance[field] === 'critical'
    );
   ...
   * 识别缺失字段
   */
  private identifyMissingFields(mappings: Record<string, string>): MissingFieldInfo[] {
    const mappedSystemFields = Object.values(mappings);
    const missingFields: MissingFieldInfo[] = [];

    Object.entries(this.fieldImportance).forEach(([systemField, importance]) => {
   ...
   * 生成自定义字段
   */
  private generateCustomFields(mappings: Record<string, string>): Record<string, string> {
