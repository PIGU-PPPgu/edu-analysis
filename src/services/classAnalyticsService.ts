...
export interface SubjectComparisonAnalysis {
  ...
  subjectComparison: Array<{
    ...
    // 班级内排名分布
    topPerformers: Array<{ // 前5名学生
      studentName: string;
      score: number;
      rank: number;
    }>;
    
    needsAttention: Array<{ // 后5名学生
      studentName: string;
      score: number;
      rank: number;
    }>;
...
export class ClassAnalyticsService {
  ...
  async getClassBasicInfo(className: string): Promise<ClassBasicInfo | null> {
    try {
      ...
      // 统计基本信息
      const uniqueStudents = new Set(gradeData.map(record => record.student_id || record.name));
   ...
   * 获取班级成绩概览
   */
  async getClassGradeOverview(className: string, examTitle?: string): Promise<ClassGradeOverview | null> {
    try {
      ...
      const examRecords = gradeData.filter(record => record.exam_title === latestExam);
      ...
      const subjectStats = subjects
        .map(subject => {
          ...
          return {
            ...
            difficulty: this.assessSubjectDifficulty(this.calculateAverage(scores))
          };
        })
   ...
   * 获取科目对比分析
   */
  async getSubjectComparisonAnalysis(className: string, examTitle: string): Promise<SubjectComparisonAnalysis | null> {
    try {
      ...
      // 计算科目间相关性（简化版）
      const subjectCorrelations = this.calculateSubjectCorrelations(gradeData, subjects);
   ...
   * 获取学生表现追踪
   */
  async getStudentPerformanceTracking(className: string, limit: number = 50): Promise<StudentPerformanceTracking | null> {
    try {
      ...
      const studentProgress = Array.from(studentDataMap.entries())
        ...
        .map(([studentKey, records]) => {
          ...
          // 风险评估
          const riskAssessment = this.assessStudentRisk(rankingTrend, progressAnalysis);
   ...
   * 获取成绩分布分析
   */
  async getGradeDistributionAnalysis(className: string, examTitle: string): Promise<GradeDistributionAnalysis | null> {
    try {
  ...
  private calculateSubjectCount(sampleRecord: any): number {
    const subjects = ['chinese', 'math', 'english', 'physics', 'chemistry', 'biology', 'politics', 'history', 'geography'];
    return subjects.filter(subject => sampleRecord[`${subject}_score`] !== null).length;
  }
  
  private extractGradeLevel(className: string): string {
    if (className.includes('初一') || className.includes('七年级')) return '初一';
    if (className.includes('初二') || className.includes('八年级')) return '初二';
    if (className.includes('初三') || className.includes('九年级')) return '初三';
  ...
  private assessSubjectDifficulty(averageScore: number): 'easy' | 'medium' | 'hard' {
    if (averageScore >= 80) return 'easy';
    if (averageScore >= 65) return 'medium';
    return 'hard';
  }
  
  private calculateRankDistribution(ranks: number[]): Array<{rankRange: string; count: number; percentage: number}> {
    const ranges = [
    ...
    return ranges.map(({ range, min, max }) => {
      ...
      const percentage = ranks.length > 0 ? Math.round((count / ranks.length) * 100) : 0;
      return { rankRange: range, count, percentage };
    });
  }
  
  private calculateSubjectCorrelations(gradeData: any[], subjects: string[]): Array<{subject1: string; subject2: string; correlation: number; strength: 'strong' | 'medium' | 'weak'}> {
    const correlations: Array<{subject1: string; subject2: string; correlation: number; strength: 'strong' | 'medium' | 'weak'}> = [];
  ...
  private analyzeStudentProgress(rankingTrend: any[]): {
    ...
    for (let i = 1; i < rankingTrend.length; i++) {
      ...
      const scoreChange = rankingTrend[i].totalScore - rankingTrend[i - 1].totalScore;
  ...
  private assessStudentRisk(rankingTrend: any[], progressAnalysis: any): {
    riskLevel: 'low' | 'medium' | 'high';
    riskFactors: string[];
    recommendations: string[];
  } {
    ...
    if (progressAnalysis.trend === 'declining' && progressAnalysis.trendStrength !== 'slight') {
  ...
  private calculateScoreDistribution(gradeData: any[]): Array<{
    ...
    return ranges.map(({ range, min, max }) => {
      const students = gradeData
        .filter(record => record.total_score >= min && record.total_score <= max)
        .map(record => ({
          name: record.name,
      ...
      const percentage = gradeData.length > 0 ? Math.round((students.length / gradeData.length) * 100) : 0;
  ...
  private analyzeDistributionShape(scores: number[]): {
    ...
    let type: 'normal' | 'skewed_left' | 'skewed_right' | 'bimodal' | 'uniform' = 'normal';
