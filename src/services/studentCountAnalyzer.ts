...
export class StudentCountAnalyzer {
  ...
  public analyzeStudentCounts(
    ...
    console.log(
      `系统学生: ${systemStudents.length}人, 文件学生: ${fileStudents.length}人`
    );
    ...
    // 分类问题班级
    const classesWithMissingStudents = classCounts.filter(
      (c) => c.status === "missing"
    );
    const classesWithExtraStudents = classCounts.filter(
      (c) => c.status === "extra"
    );
    ...
    // 生成分析建议
    const { recommendations, warnings, actions } =
      this.generateAnalysisRecommendations(
        classCounts,
        tolerancePercentage,
        minClassSize,
        maxClassSize
  ...
  private generateMissingStudentDetails(
    ...
    const matchedSystemStudentIds = new Set(
      matchedStudents.map(
        (m) => m.systemStudent.id || m.systemStudent.student_id
      )
    );

    return classesWithMissing.map((classInfo) => {
      // 找到该班级中未匹配的系统学生
      const classSystemStudents = systemStudents.filter(
        (s) => s.class_name === classInfo.className
      );
      const unmatchedSystemStudents = classSystemStudents.filter(
        (s) => !matchedSystemStudentIds.has(s.student_id)
      );
  ...
  private generateExtraStudentDetails(
    ...
    const matchedFileStudents = new Set(
      matchedStudents.map((m) => m.fileStudent)
    );

    return classesWithExtra.map((classInfo) => {
      // 找到该班级中未匹配的文件学生
      const classFileStudents = fileStudents.filter(
        (s) => s.class_name === classInfo.className
      );
      const unmatchedFileStudents = classFileStudents.filter(
        (s) => !matchedFileStudents.has(s)
      );

      // 分析可能的原因
      const possibleReasons = this.analyzeExtraReasons(
        classInfo,
        unmatchedFileStudents
      );
  ...
  private analyzeMissingReasons(
    ...
    const studentsInOtherClasses = missingStudents.filter((s) =>
      fileStudentNames.has(s.name)
    );

    if (studentsInOtherClasses.length > 0) {
      reasons.push(
        `${studentsInOtherClasses.length}个学生可能在文件中的其他班级`
      );
    }

    // 检查缺失比例
    const missingPercentage =
      Math.abs(classInfo.difference) / classInfo.systemCount;
  ...
  private analyzeExtraReasons(
    ...
    if (studentsWithoutId.length > 0) {
      reasons.push(
        `${studentsWithoutId.length}个学生缺少学号，可能是新转入学生`
      );
    }

    if (studentsWithId.length > 0) {
      reasons.push(
        `${studentsWithId.length}个学生有学号但系统中不存在，可能需要创建学生档案`
      );
    }
  ...
  private generateAnalysisRecommendations(
    ...
    const normalClasses = classCounts.filter(
      (c) => c.status === "normal"
    ).length;
    ...
    if (problemClasses === 0) {
      ...
      if (problemPercentage > 50) {
        warnings.push(
          `❌ ${problemPercentage.toFixed(0)}%的班级存在数量差异，建议检查数据来源和质量`
        );
      } else {
        recommendations.push(
          `⚠️ ${problemClasses}个班级存在数量差异，需要逐一确认`
        );
      }
    }
    ...
    for (const classInfo of classCounts) {
      if (classInfo.systemCount > 0) {
        // 排除新班级
        if (classInfo.systemCount < minClassSize) {
          warnings.push(
            `${classInfo.className}班级规模偏小(${classInfo.systemCount}人)，建议关注`
          );
        } else if (classInfo.systemCount > maxClassSize) {
          warnings.push(
            `${classInfo.className}班级规模偏大(${classInfo.systemCount}人)，建议关注`
          );
        }
      }
    }
    ...
    if (newClasses.length > 0) {
      ...
      recommendations.push(
        "新班级需要确认是否为有效班级，并创建相应的班级档案"
      );
    }

    // 数据质量建议
    const totalDifference = classCounts.reduce(
      (sum, c) => sum + Math.abs(c.difference),
      0
    );
    const totalStudents = classCounts.reduce(
      (sum, c) => sum + c.systemCount,
      0
    );
  ...
  public getProcessingSuggestions(analysisResult: StudentCountAnalysisResult): {
    ...
    // 需要特别确认的情况
    const highDifferenceClasses = analysisResult.classCounts.filter(
      (c) => Math.abs(c.difference) / (c.systemCount || 1) > 0.2
    );

    if (highDifferenceClasses.length > 0) {
      confirmations.push(
        `${highDifferenceClasses.length}个班级差异较大，需要特别确认`
      );
    }
