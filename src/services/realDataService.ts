...
export async function getWarningsByType(): Promise<WarningTypeStats[]> {
  try {
    ...
    const totalCount = Object.values(typeCount).reduce((sum, count) => sum + count, 0);
...
export async function getRiskByClass(): Promise<ClassRiskStats[]> {
  try {
    ...
    warningsResult.data?.forEach(warning => {
      // 找到学生所在班级
      const student = studentsResult.data?.find(s => s.student_id === warning.student_id);
      if (student?.class_name) {
        classWarnings[student.class_name] = (classWarnings[student.class_name] || 0) + 1;
      }
    });
...
export async function getCommonRiskFactors(): Promise<RiskFactorStats[]> {
  try {
    ...
    if (error) {
      console.error('获取预警记录失败:', error);
      return [
        { factor: '学习态度问题', count: 18, percentage: 35.3, severity: 'medium' },
        { factor: '基础知识薄弱', count: 15, percentage: 29.4, severity: 'high' },
        { factor: '学习方法不当', count: 10, percentage: 19.6, severity: 'medium' },
        { factor: '心理压力过大', count: 8, percentage: 15.7, severity: 'high' }
      ];
    }
    ...
    const totalCount = Object.values(factorCount).reduce((sum, item) => sum + item.count, 0);
 ...
 * 智能班级名称解析
 */
function resolveClassName(inputClassName: string, availableClasses: string[]): string {
  ...
  if (inputClassName.startsWith('class-')) {
    ...
    if (!resolved.includes('班') && !resolved.includes('级')) {
      const matched = availableClasses.find(className =>
 ...
 * 获取班级画像统计数据
 */
export async function getClassPortraitStats(inputClassName: string): Promise<ClassPortraitStats | null> {
  try {
    console.log('🔍 获取班级画像统计（使用映射）:', inputClassName);

    // 导入映射服务
    const { getGradesByClassName, batchGetGradeTableIds } = await import('./enhancedMappingService');
    ...
    const availableClasses = [...new Set(allClasses?.map(item => item.class_name) || [])];
    const resolvedClassName = resolveClassName(inputClassName, availableClasses);
    ...
    // 使用映射服务获取成绩数据
    const { data: gradesData, error: gradesError } = await getGradesByClassName(resolvedClassName);
    if (gradesError) {
      console.error('通过映射获取成绩数据失败:', gradesError);
      return null;
    }

    console.log(`找到学生: ${studentsData.length}人, 映射成绩记录: ${gradesData?.length || 0}条`);
    ...
    // 计算分数统计
    const totalScores = gradesData?.map(g => g.total_score).filter(score => score != null) || [];
    ...
    const excellentRate = totalScores.length > 0 ? Math.round((excellentCount / totalScores.length) * 100 * 10) / 10 : 0;
    const passRate = totalScores.length > 0 ? Math.round((passCount / totalScores.length) * 100 * 10) / 10 : 0;
    ...
    const subjectStats = subjects.map(subject => {
      const scores = gradesData?.map(g => (g as any)[subject.field])
 ...
 * 获取学生表现统计数据
 */
export async function getStudentPerformanceStats(studentId: string): Promise<StudentPerformanceStats | null> {
  try {
    ...
    const { data: gradesData, error: gradesError } = await supabase
      .from('grade_data_new')
      .select('student_id, total_score, chinese_score, math_score, english_score, physics_score, chemistry_score, created_at')
      .eq('student_id', studentId)
      .not('total_score', 'is', null)
    ...
    const scores = grades.map(g => g.total_score).filter(score => score != null);
    const avgScore = scores.length > 0
      ? Math.round(scores.reduce((sum, score) => sum + score, 0) / scores.length * 10) / 10
      : 0;
    ...
    const subjectPerformance = subjects.map(subject => {
      const subjectScores = grades.map(g => (g as any)[subject.field])
        .filter(score => score != null && score > 0);

      const average = subjectScores.length > 0
        ? Math.round(subjectScores.reduce((sum, score) => sum + score, 0) / subjectScores.length * 10) / 10
        : 0;
    ...
    const betterScoreCount = classScores.filter(score => score > avgScore).length;
...
export async function calculateWarningStatistics(): Promise<WarningOverallStats> {
  try {
    ...
    const resolvedWarnings = warnings.filter(w => w.status === 'resolved').length;
    const highRiskStudents = warnings.filter(w =>
      w.status === 'active' && (w.details as any)?.severity === 'high'
    ).length;
