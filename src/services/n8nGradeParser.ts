...
class N8nGradeParserService {
  private readonly DEFAULT_WEBHOOK_URL = 'http://localhost:5678/webhook/parse-grade-file';
  ...
  async parseGradeFile(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`n8n工作流请求失败: ${response.status} ${response.statusText}`);
      }
  ...
  async checkHealth(webhookUrl?: string): Promise<boolean> {
    try {
      const testUrl = webhookUrl || this.DEFAULT_WEBHOOK_URL.replace('/parse-grade-file', '/health');
