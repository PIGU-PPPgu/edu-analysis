...
export class knowledgePointService {
  // 计算两个知识点的语义相似度
  static async calculateSimilarity(point1: string, point2: string): Promise<number> {
    try {
      ...
      if (normalizedPoint1.length > 0 && normalizedPoint2.length > 0) {
        const wordOverlap = this.calculateWordOverlap(normalizedPoint1, normalizedPoint2);
   ...
   * 创建合并后的知识点
   */
  private static createMergedKnowledgePoint(points: KnowledgePoint[]): KnowledgePoint {
    // 保留第一个点的ID和创建时间
    ...
    for (const point of points) {
      if (point.description && point.description.length > bestDescription.length) {
    ...
    // 计算平均掌握度
    const avgMasteryLevel = countWithMastery > 0
...
// 从数据库获取作业的知识点
export async function getKnowledgePointsByHomeworkId(homeworkId: string): Promise<KnowledgePoint[]> {
...
export async function updateKnowledgePointEvaluations(
  ...
  try {
    ...
    console.log('开始更新知识点评估:', evaluations.length, '项，submissionId:', submissionId);
    ...
    if (submissionError) {
      ...
      return { success: false, message: `获取提交记录失败: ${submissionError.message}`, results: [] };
    }
    ...
    if (fetchError) {
      ...
      console.log('已有知识点评估:', existingEvaluations?.length || 0, '项', existingEvaluations);
    }
    ...
    for (const evaluation of evaluations) {
      try {
        ...
        console.log(`处理知识点评估 [${knowledgePointId}], 掌握度: ${evaluation.masteryLevel}`);
        
        // 将百分比掌握度转换为字母等级
        const masteryGrade = masteryLevelToGrade(evaluation.masteryLevel);
        console.log(`掌握度${evaluation.masteryLevel}%转换为等级: ${masteryGrade}`);
        
        // 检查是否已有评估记录
        const existingId = existingMap.get(knowledgePointId) || evaluation.evaluationId;
        ...
        if (existingId) {
          ...
          if (error) {
            ...
            if (error.message.includes('duplicate key') || error.message.includes('unique constraint')) {
              ...
              // 获取冲突记录
              const { data: conflictData, error: fetchConflictError } = await supabase
                .from('student_knowledge_mastery')
                .select('id')
                .eq('student_id', studentId)
              ...
              if (!fetchConflictError && conflictData) {
                ...
                const newConflictAssessmentCount = (conflictRecord?.assessment_count || 0) + 1;
...
export async function deleteKnowledgePoint(knowledgePointId: string) {
  try {
    ...
    return { success: false, message: error instanceof Error ? error.message : '删除知识点时发生错误' };
  }
}
...
export async function deleteAllKnowledgePoints(homeworkId: string) {
    ...
    // 过滤出未被引用的知识点ID
    const referencedIds = new Set(evaluations?.map(ref => ref.knowledge_point_id) || []);
    const deleteableIds = knowledgePointIds.filter(id => !referencedIds.has(id));
    ...
    return { success: false, message: error instanceof Error ? error.message : '批量删除知识点时发生错误' };
  }
}
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "你是一位教育专家，擅长从学生作业中提取重要知识点。请从提供的内容中识别出关键知识点，以简洁的方式列出。"
          },
          {
            role: "user",
      ...
      // 解析返回的JSON
      const result = JSON.parse(response.choices[0].message.content || '{"knowledge_points":[]}');
      const extractedPoints = result.knowledge_points || [];
      
      // 将提取的知识点字符串数组转换为KnowledgePoint对象数组进行处理
      const knowledgePointObjects: KnowledgePoint[] = extractedPoints.map((point: string) => ({
        id: '', // 临时ID
        name: point,
        homework_id: '',
        created_at: new Date().toISOString()
      ...
      // 使用语义相似度合并相似知识点
      const mergedPoints = await knowledgePointService.mergeKnowledgePoints(knowledgePointObjects);
...
export function gradeToDescription(grade: string): string {
  switch (grade) {
    case 'A': return '优秀 (完全掌握)';
    case 'B': return '良好 (基本掌握)';
    case 'C': return '中等 (部分掌握)';
    case 'D': return '及格 (勉强掌握)';
    case 'E': return '不及格 (未掌握)';
    default: return '未评估';
  }
} 