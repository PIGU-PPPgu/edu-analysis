...
// HTTP状态码错误映射
const HTTP_ERROR_MAPPINGS: Record<number, { type: ErrorType; severity: ErrorSeverity; userMessage: string }> = {
  400: { type: ErrorType.VALIDATION, severity: ErrorSeverity.MEDIUM, userMessage: '请求参数有误，请检查输入' },
  401: { type: ErrorType.AUTHENTICATION, severity: ErrorSeverity.HIGH, userMessage: '请先登录账户' },
  403: { type: ErrorType.AUTHORIZATION, severity: ErrorSeverity.HIGH, userMessage: '权限不足，无法执行此操作' },
  404: { type: ErrorType.NOT_FOUND, severity: ErrorSeverity.MEDIUM, userMessage: '请求的资源不存在' },
  409: { type: ErrorType.CONFLICT, severity: ErrorSeverity.MEDIUM, userMessage: '数据冲突，请刷新后重试' },
  429: { type: ErrorType.RATE_LIMIT, severity: ErrorSeverity.MEDIUM, userMessage: '操作过于频繁，请稍后重试' },
  500: { type: ErrorType.SERVER, severity: ErrorSeverity.HIGH, userMessage: '服务器错误，请联系管理员' },
  502: { type: ErrorType.NETWORK, severity: ErrorSeverity.HIGH, userMessage: '网络连接异常，请稍后重试' },
  503: { type: ErrorType.SERVER, severity: ErrorSeverity.HIGH, userMessage: '服务暂时不可用，请稍后重试' },
  504: { type: ErrorType.TIMEOUT, severity: ErrorSeverity.MEDIUM, userMessage: '请求超时，请稍后重试' }
};

// Supabase和业务错误映射
const BUSINESS_ERROR_MAPPINGS: Record<string, { type: ErrorType; severity: ErrorSeverity; userMessage: string; suggestions?: string[] }> = {
  // Supabase相关错误
  'PGRST301': { 
  ...
  'FILE_FORMAT_INVALID': { 
    ...
    suggestions: ['使用Excel(.xlsx/.xls)文件', '使用CSV文件', '检查文件是否损坏']
  },
...
class ErrorHandler {
    ...
    try {
      if (error?.response) {
        ...
        standardError = {
          ...
          severity: errorInfo.severity,
          code: error.code,
          message: error.message || error.details || `Business error: ${error.code}`,
          userMessage: errorInfo.userMessage,
          context: {
            ...context,
   ...
   * 创建成功响应
   */
  createSuccessResponse<T>(data: T, message?: string, meta?: Record<string, any>): ApiResponse<T> {
    return {
      success: true,
      data,
...
export const createErrorResponse = (error: any, context?: Record<string, any>): ApiResponse => {
  const standardError = errorHandler.handle(error, context);
