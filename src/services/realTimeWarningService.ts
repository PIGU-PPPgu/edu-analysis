...
export class RealTimeWarningService {
   ...
   */
  async calculateWarningStatistics(filter?: WarningFilter): Promise<WarningStatistics> {
  ...
  private analyzeWarnings(gradesData: any[]): WarningStatistics {
    ...
    students.forEach(student => {
      ...
      if (failingGrades.length >= 2) {
        ...
        riskFactorCounts.set('成绩不及格', (riskFactorCounts.get('成绩不及格') || 0) + 1);
      }
      ...
      if (avgScore < 70) {
        ...
        riskFactorCounts.set('平均分过低', (riskFactorCounts.get('平均分过低') || 0) + 1);
      }
