...
export class UnifiedGradeService {
   ...
   */
  static async getHomeworkScores(
    filters: {
      student_id?: string;
      homework_id?: string;
      status?: string;
   ...
   * 自动选择最合适的数据源
   */
  static async getStudentRecentScores(
    student_id: string,
    limit: number = 10
  ): Promise<UnifiedGradeData[]> {
    try {
      ...
      if (comprehensiveScores.length < limit) {
        ...
        // 转换为统一格式
        const unifiedSubjectScores: UnifiedGradeData[] = subjectScores.map(
          (score) => ({
            id: score.id,
        ...
        return [
          ...this.transformComprehensiveToUnified(comprehensiveScores),
          ...unifiedSubjectScores,
        ]
          .sort(
            (a, b) =>
   ...
   * 转换综合成绩为统一格式 (取总分)
   */
  private static transformComprehensiveToUnified(
    comprehensiveScores: ComprehensiveGradeData[]
  ): UnifiedGradeData[] {
