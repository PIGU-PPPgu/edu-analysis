...
export class AIFieldClassifier {
   ...
   * 智能分类字段类型
   */
  async classifyField(request: FieldInquiryRequest): Promise<FieldClassificationResult> {
   ...
   * 快速分类 - 基于关键词匹配
   */
  private quickClassify(request: FieldInquiryRequest): FieldClassificationResult {
    const { fieldName, userDescription } = request;
    const text = `${fieldName} ${userDescription}`.toLowerCase();
    
    const suggestions: Array<{ type: FieldType; confidence: number; reason: string }> = [];
   ...
   * AI增强分类
   */
  private async aiClassify(request: FieldInquiryRequest): Promise<FieldClassificationResult> {
    ...
    const { data, error } = await supabase.functions.invoke('proxy-ai-request', {
      body: {
        messages: [
          {
            role: 'system',
            content: '你是一个专业的教育数据字段分类专家。根据字段名称和用户描述，判断字段的类型。'
          },
          {
            role: 'user',
            content: prompt
...
可选的字段类型:
${Object.entries(FIELD_TYPE_DESCRIPTIONS).map(([type, desc]) => `- ${type}: ${desc}`).join('\n')}
...
请以JSON格式返回分析结果:
{
   ...
   * 检测科目特定字段
   */
  private detectSubjectField(fieldName: string, userDescription: string): { type: FieldType; confidence: number; reason: string } | null {
    const text = `${fieldName} ${userDescription}`.toLowerCase();
    
    // 科目名称模式
    const subjects = ['语文', '数学', '英语', '物理', '化学', '生物', '政治', '历史', '地理', '道法', '总分'];
    const subjectFound = subjects.find(subject => text.includes(subject.toLowerCase()));
    
    if (subjectFound) {
      // 判断是分数、等级还是排名
      if (text.includes('分数') || text.includes('成绩') || text.includes('得分')) {
        return {
      ...
      } else if (text.includes('校名') || text.includes('级名') || text.includes('年级排名')) {
        return {
   ...
   * 批量分类字段
   */
  async classifyFields(requests: FieldInquiryRequest[]): Promise<Record<string, FieldClassificationResult>> {
