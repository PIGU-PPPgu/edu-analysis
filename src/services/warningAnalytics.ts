...
export function classifyStudentRisk(student: StudentData): {
  ...
  if (student.previousExamAverage && 
  ...
  if (student.examAverage < 60 || student.homeworkCompletionRate < 0.6) {
    factors.push(student.examAverage < 60 ? '考试成绩不及格' : '作业完成率很低');
...
export function analyzeTrend(student: StudentData): {
  ...
  if (student.historicalData.homeworkScores && 
  ...
  if (student.historicalData.participationScores && 
  ...
  // 判断趋势方向和强度
  const trendDirection = weightedSlope > 0.1 ? 'improving' : 
                        weightedSlope < -0.1 ? 'declining' : 'stable';
...
export function generateWarning(student: StudentData): WarningResult {
  ...
  if (riskScore.level === 'high' || classification.riskLevel === 'high' || trend.riskLevel === 'high') {
    finalRiskLevel = 'high';
  } 
  ...
  // 如果只有一个算法返回中等风险，或至少有两个算法返回低风险，则最终结果为低风险
    ...
    riskScore.level === 'medium' || classification.riskLevel === 'medium' || trend.riskLevel === 'medium' ||
    [riskScore.level, classification.riskLevel, trend.riskLevel].filter(
      level => level === 'low'
    ).length >= 2
  ) {
 ...
 * 批量生成学生预警
 */
export function generateBatchWarnings(students: StudentData[]): WarningResult[] {
