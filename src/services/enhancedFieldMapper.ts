...
export const ENHANCED_FIELD_PATTERNS: Record<string, FieldMappingConfig> = {
  student_id: {
    confidence: 0.95,
    alternatives: ['学号', '学生编号', '学生ID', 'ID', '编号', '序号', 'student_no', 'stu_id', '学籍号'],
  ...
  name: {
    confidence: 0.90,
    alternatives: ['姓名', '学生姓名', '名字', '真实姓名', 'name', 'student_name', '学员姓名'],
  ...
  class_name: {
    confidence: 0.85,
    alternatives: ['班级', '班级名称', '所属班级', '班名', 'class', 'class_name', '年级班级'],
  ...
  chinese: {
    ...
    contextClues: ['语文', '中文'],
    validationRules: [
      { pattern: /^\d{1,3}(\.\d{1,2})?$/, dataType: 'number' }
    ]
  },
  ...
  total_score: {
    confidence: 0.85,
    alternatives: ['总分', '总成绩', '合计分数', 'total', 'total_score', '总计'],
...
export class EnhancedFieldMapper {
  ...
  constructor(aiProvider: string = 'openai', confidenceThreshold: number = 0.7) {
   ...
   * 智能字段映射 - 结合规则匹配和AI分析
   */
  async mapFields(headers: string[]): Promise<Record<string, FieldMappingResult>> {
   ...
   * 分析单个字段
   */
  private async analyzeField(header: string, allHeaders: string[]): Promise<FieldMappingResult | null> {
  ...
  private findFuzzyMatch(normalizedHeader: string): FieldMappingResult | null {
    let bestMatch: { field: string; similarity: number; config: FieldMappingConfig } | null = null;
    
    for (const [field, config] of Object.entries(ENHANCED_FIELD_PATTERNS)) {
      for (const alternative of config.alternatives) {
        const normalizedAlt = this.normalizeHeader(alternative);
        const similarity = this.calculateSimilarity(normalizedHeader, normalizedAlt);
    ...
    if (bestMatch) {
      const adjustedConfidence = bestMatch.config.confidence * bestMatch.similarity;
      return {
   ...
   * 上下文分析 - 基于相邻字段推断
   */
  private analyzeContext(header: string, allHeaders: string[]): FieldMappingResult | null {
    const normalizedHeader = this.normalizeHeader(header);
   ...
   * 判断是否需要AI验证
   */
  private shouldUseAiValidation(results: Record<string, FieldMappingResult>): boolean {
   ...
   * 构建AI提示词
   */
  private buildAiPrompt(headers: string[], currentResults: Record<string, FieldMappingResult>): string {
    return `
  ...
  private mergeAiSuggestions(
    ...
    for (const [header, suggestions] of Object.entries(aiSuggestions)) {
      if (suggestions.length > 0) {
        ...
        if (!currentResult || topSuggestion.confidence > currentResult.confidence) {
          mergedResults[header] = {
            ...
            needsManualReview: topSuggestion.confidence < this.confidenceThreshold
          };
        }
      }
    }
 ...
 * 工具函数：快速字段映射
 */
export async function quickFieldMapping(headers: string[]): Promise<Record<string, string>> {
