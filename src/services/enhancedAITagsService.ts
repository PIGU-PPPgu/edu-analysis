...
export interface StudentLearningProfile {
  ...
  rankingAnalysis: {
    ...
    competitivePosition: 'top' | 'upper-middle' | 'middle' | 'lower-middle' | 'bottom';
  };
  
  // 学习模式识别
  learningPatterns: {
    examPerformancePattern: 'consistent' | 'volatile' | 'improving' | 'declining';
    subjectBalance: 'balanced' | 'science-oriented' | 'liberal-oriented' | 'uneven';
...
export class EnhancedAITagsService {
  ...
  async generateTagsForNewStudents(
    ...
    console.log('🎯 [AI标签] 开始为新学生批量生成AI标签，学生数量:', newStudentIds.length);
    ...
    for (let i = 0; i < newStudentIds.length; i += batchSize) {
      ...
      console.log(`📦 [AI标签] 处理批次 ${Math.floor(i / batchSize) + 1}/${Math.ceil(newStudentIds.length / batchSize)}, 学生数: ${batch.length}`);
      ...
      const batchPromises = batch.map(async (studentId) => {
        try {
          ...
          if (!this.isDataSufficient(profile, config.minDataPointsRequired || 1)) {
            result.skipped.push({
              studentId,
   ...
   */
  async analyzeStudentLearningProfile(studentId: string): Promise<StudentLearningProfile> {
   ...
   * 成绩数据分析
   */
  private analyzeGradeData(records: any[]): StudentLearningProfile['gradeAnalysis'] {
    ...
    // 识别优势和劣势学科
    const sortedSubjects = Array.from(subjectAverages.entries())
      .sort(([,a], [,b]) => b - a);
   ...
   * 排名数据分析
   */
  private analyzeRankingData(records: any[]): StudentLearningProfile['rankingAnalysis'] {
    ...
    if (latestRecord) {
      subjects.forEach(subject => {
        ...
        if (latestRecord[rankField]) {
          subjectRankings[this.getSubjectDisplayName(subject)] = latestRecord[rankField];
        }
      });
    }
    
    // 判断竞争位置
    const averageRank = classRankTrend.length > 0
      ? classRankTrend.reduce((sum, rank) => sum + rank, 0) / classRankTrend.length
      : 20;
    
    let competitivePosition: StudentLearningProfile['rankingAnalysis']['competitivePosition'] = 'middle';
   ...
   * 学习模式识别
   */
  private identifyLearningPatterns(records: any[]): StudentLearningProfile['learningPatterns'] {
    const totalScores = records
      .map(r => r.total_score)
      .filter(score => score !== null && score !== undefined);
    
    // 分析考试表现模式
    let examPerformancePattern: StudentLearningProfile['learningPatterns']['examPerformancePattern'] = 'consistent';
    ...
    let scienceAvg = 0, liberalAvg = 0;
    let scienceCount = 0, liberalCount = 0;
    ...
    let subjectBalance: StudentLearningProfile['learningPatterns']['subjectBalance'] = 'balanced';
    const balanceDiff = Math.abs(scienceAvg - liberalAvg);
    
    if (balanceDiff > 15) {
      subjectBalance = scienceAvg > liberalAvg ? 'science-oriented' : 'liberal-oriented';
    } else if (balanceDiff > 25) {
      subjectBalance = 'uneven';
    }
    
    // 分析压力应对（基于考试类型的表现差异）
    let stressResponse: StudentLearningProfile['learningPatterns']['stressResponse'] = 'neutral';
    ...
    if (examTypeScores.size >= 2) {
      const averages = Array.from(examTypeScores.entries())
        .map(([type, scores]) => ({
          type,
      ...
      if (maxAvg - minAvg > 20) {
        ...
        const importantExams = averages.filter(a => 
          a.type.includes('期中') || a.type.includes('期末') || a.type.includes('月考')
        );
        
        if (importantExams.length > 0) {
          const importantAvg = importantExams.reduce((sum, exam) => sum + exam.average, 0) / importantExams.length;
          const overallAvg = averages.reduce((sum, exam) => sum + exam.average, 0) / averages.length;
          
          stressResponse = importantAvg > overallAvg ? 'performs-well' : 'struggles';
        }
      }
    }
   ...
   * 评估数据质量
   */
  private assessDataQuality(records: any[]): StudentLearningProfile['dataQuality'] {
    ...
    // 评估可靠性
    let reliability: StudentLearningProfile['dataQuality']['reliability'] = 'low';
   ...
   * 构建增强的分析上下文
   */
  private buildEnhancedAnalysisContext(profile: StudentLearningProfile): string {
    const ctx = [];
    
    // 成绩分析上下文
    ctx.push(`成绩分析：平均分${profile.gradeAnalysis.overallAverage}分`);
    if (profile.gradeAnalysis.subjectStrengths.length > 0) {
      ctx.push(`优势学科：${profile.gradeAnalysis.subjectStrengths.map(s => `${s.subject}(${s.score}分)`).join(', ')}`);
    }
    if (profile.gradeAnalysis.subjectWeaknesses.length > 0) {
      ctx.push(`薄弱学科：${profile.gradeAnalysis.subjectWeaknesses.map(s => `${s.subject}(${s.score}分)`).join(', ')}`);
    }
    ctx.push(`成绩稳定性：${profile.gradeAnalysis.consistencyScore > 80 ? '很稳定' : profile.gradeAnalysis.consistencyScore > 60 ? '较稳定' : '波动较大'}`);
    ctx.push(`进步趋势：${profile.gradeAnalysis.improvementTrend === 'improving' ? '上升' : profile.gradeAnalysis.improvementTrend === 'declining' ? '下降' : '平稳'}`);
    
    // 排名分析上下文
    if (profile.rankingAnalysis.classRankTrend.length > 0) {
      const latestRank = profile.rankingAnalysis.classRankTrend[profile.rankingAnalysis.classRankTrend.length - 1];
      ctx.push(`班级排名：第${latestRank}名（${profile.rankingAnalysis.competitivePosition}）`);
    }
    ...
    // 数据质量上下文
    ctx.push(`数据基础：${profile.dataQuality.examCount}次考试，${profile.dataQuality.subjectCount}个科目，${profile.dataQuality.timeSpan}`);
   ...
   * 为AI优化成绩格式
   */
  private formatScoresForAI(profile: StudentLearningProfile): Array<{subject: string; score: number; context?: string}> {
   ...
   * 计算标签置信度
   */
  private calculateTagsConfidence(profile: StudentLearningProfile, aiTags: any): number {
   ...
   * 生成Fallback标签（基于规则）
   */
  private generateFallbackTags(profile: StudentLearningProfile): EnhancedAITags {
    ...
    if (tags.personalityTraits.length === 0) tags.personalityTraits.push('认真学习');
   ...
   * 保存AI标签到数据库
   */
  private async saveAITags(studentId: string, tags: EnhancedAITags): Promise<void> {
   ...
   * 判断数据是否充足
   */
  private isDataSufficient(profile: StudentLearningProfile, minRequired: number): boolean {
    return profile.dataQuality.examCount >= minRequired && 
