 ...
 * @returns 相关系数（-1到1之间）
 */
export function calculateCorrelation(data: any[], metricA: string, metricB: string): number {
  // 提取两个指标的数据
  const valuesA = data.map(item => getNestedProperty(item, metricA)).filter(v => v !== undefined && v !== null);
  const valuesB = data.map(item => getNestedProperty(item, metricB)).filter(v => v !== undefined && v !== null);
  ...
  // 计算平均值
  const avgA = valuesA.slice(0, length).reduce((sum, val) => sum + val, 0) / length;
  const avgB = valuesB.slice(0, length).reduce((sum, val) => sum + val, 0) / length;
 ...
 * @returns 交易记录数组
 */
export function convertWarningsToTransactions(warnings: WarningResult[]): string[][] {
  return warnings
...
export function formatRulesAsWarnings(rules: AssociationRule[]): Array<{
  ...
  return rules.map((rule, index) => {
    // 获取可读的条件和结果描述
    const conditions = rule.antecedent.map(code => 
      eventNameMap[code] || formatEventCode(code)
    );
    
    const results = rule.consequent.map(code => 
      eventNameMap[code] || formatEventCode(code)
    );
