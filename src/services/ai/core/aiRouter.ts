...
export class AIRouter {
  ...
  async route(request: RouteRequest): Promise<RouteResult> {
    console.log(
      `🎯 开始路由选择: ${request.modelId}, 策略: ${this.currentStrategy.name}`
    );
    ...
    const result: RouteResult = {
      ...
      selectedModel: this.mapModelToProvider(
        request.modelId,
    ...
    console.log(
      `✅ 路由选择完成: ${result.selectedProvider} (置信度: ${(result.confidence * 100).toFixed(1)}%)`
    );
  ...
  private getAvailableProviders(request: RouteRequest): ProviderHealth[] {
    ...
    return Array.from(this.healthStats.values()).filter((health) => {
      // 基础可用性检查
      if (
        !health.isAvailable ||
        !enabledProviders.includes(health.providerId)
      ) {
        return false;
      }
      ...
      if (request.requirements?.preferredProviders?.length) {
        if (
          !request.requirements.preferredProviders.includes(health.providerId)
        ) {
          return false;
        }
      }
      ...
      if (
        request.requirements?.excludedProviders?.includes(health.providerId)
      ) {
        return false;
      }
      ...
      // 性能要求检查
      if (
        request.requirements?.maxLatency &&
        health.latency > request.requirements.maxLatency
      ) {
        return false;
      }
      ...
      const minSuccessRate =
        this.getProviderPriority(health.providerId)?.conditions
          ?.minSuccessRate || 80;
  ...
  ): Array<ProviderHealth & { score: number }> {
    return providers
      .map((provider) => {
  ...
  private calculatePerformanceScore(provider: ProviderHealth): number {
    ...
    if (provider.latency > 5000)
      latencyScore = 20; // >5s 很慢
    else if (provider.latency > 3000)
      latencyScore = 40; // >3s 慢
    else if (provider.latency > 1000)
      latencyScore = 70; // >1s 一般
  ...
  private selectBestProvider(
    ...
    const topProviders = scoredProviders.slice(
      0,
      Math.min(3, scoredProviders.length)
    );
  ...
  private selectRoundRobin(
    providers: Array<ProviderHealth & { score: number }>
  ): ProviderHealth & { score: number } {
    // 简单轮询实现
  ...
  private generateReasoning(
    provider: ProviderHealth & { score: number },
    request: RouteRequest
  ): string {
  ...
  async updateProviderHealth(
    ...
    // 更新统计信息
    health.latency = this.calculateMovingAverage(
      health.latency,
      result.latency,
      0.3
    );
    ...
    if (result.success) {
      ...
      health.successRate = this.calculateMovingAverage(
        health.successRate,
        100,
        0.1
      );
      ...
      health.errorRate = this.calculateMovingAverage(
        health.errorRate,
        100,
        0.1
      );
    ...
    // 更新可用性状态
    health.isAvailable =
      health.consecutiveFailures < 3 && health.successRate > 50;
  ...
  updateProviderPriority(priority: ProviderPriority): void {
    const index = this.providerPriorities.findIndex(
      (p) => p.providerId === priority.providerId
    );
  ...
  private estimateCost(providerId: string, tokens: number): number {
    ...
    return (
      ((tokens / 1000) * (config.inputTokenCost + config.outputTokenCost)) / 2
    );
  }
  ...
  private mapModelToProvider(
    ...
    return (
      providerMappings[requestedModel] ||
  ...
  private initializeDefaultPriorities(): void {
    ...
    this.providerPriorities.forEach((priority) => {
      if (!this.healthStats.has(priority.providerId)) {
        this.healthStats.set(
          priority.providerId,
          this.createDefaultHealth(priority.providerId)
        );
      }
    });
  }
  ...
  private saveToStorage(): void {
    try {
      localStorage.setItem(
        this.STORAGE_KEYS.HEALTH_STATS,
        JSON.stringify(Array.from(this.healthStats.entries()))
      );
      localStorage.setItem(
        this.STORAGE_KEYS.PROVIDER_PRIORITIES,
        JSON.stringify(this.providerPriorities)
      );
      localStorage.setItem(
        this.STORAGE_KEYS.CURRENT_STRATEGY,
        JSON.stringify(this.currentStrategy)
      );
    } catch (error) {
  ...
  private loadFromStorage(): void {
    try {
      ...
      const prioritiesData = localStorage.getItem(
        this.STORAGE_KEYS.PROVIDER_PRIORITIES
      );
      if (prioritiesData) {
      ...
      const strategyData = localStorage.getItem(
        this.STORAGE_KEYS.CURRENT_STRATEGY
      );
      if (strategyData) {
