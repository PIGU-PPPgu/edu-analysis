 ...
 * 分析学生成绩数据并识别风险
 */
export async function analyzeGradeRisks(
  studentId?: string,
  ...
  try {
    ...
    if (filters?.startDate && filters?.endDate) {
      query = query
        .gte("exam_date", filters.startDate)
        .lte("exam_date", filters.endDate);
    }
          ...
          riskFactors.push({
            ...
            severity: Math.min(
              ...
              Math.max(
                1,
                Math.floor(
                  (recentScores[0] - recentScores[recentScores.length - 1]) / 10
                )
              )
            ),
 ...
 * 分析作业完成情况并识别风险
 */
export async function analyzeHomeworkRisks(
  studentId?: string,
  filters?: DataIntegrationFilters
): Promise<RiskFactor[]> {
  try {
      ...
      if (gradedSubmissions.length >= 3) {
        ...
        if (avgScore < 60) {
          riskFactors.push({
            ...
            lastOccurrence:
              gradedSubmissions[0].submitted_at ||
              gradedSubmissions[0].created_at,
          });
        }
      }
    });
 ...
 * 分析知识点掌握情况并识别风险
 */
export async function analyzeKnowledgeRisks(
  studentId?: string,
  filters?: DataIntegrationFilters
): Promise<RiskFactor[]> {
  try {
 ...
 * 生成学生风险档案
 */
export async function generateStudentRiskProfile(
  studentId: string,
  filters?: DataIntegrationFilters
  ...
  try {
    ...
    // 计算风险分数
    const riskScore = allRisks.reduce(
      (total, risk) => total + risk.severity,
      0
    );
 ...
 * 批量生成预警记录
 */
export async function generateWarningRecords(
  ...
  try {
    ...
    const newWarningRecords = warningRecords.filter((record) => {
      const exists = existingWarnings.find(
        (e) => e.studentId === record.student_id
      )?.exists;
      return !exists;
    });
 ...
 * 完整的数据集成流程
 */
export async function runWarningDataIntegration(
  filters?: DataIntegrationFilters
  ...
  try {
    console.log(
      "🚀 开始预警数据集成流程...",
      filters ? `应用筛选条件: ${JSON.stringify(filters)}` : "无筛选条件"
    );
    ...
    console.log(
      `⚡ 优化模式：仅处理前${processLimit}个学生，每批${batchSize}个并行处理`
    );
    ...
    for (let i = 0; i < studentsToProcess.length; i += batchSize) {
      ...
      console.log(
        `📦 处理第${Math.floor(i / batchSize) + 1}批学生 (${batch.length}个)`
      );

      // 并行处理一批学生
      const batchProfiles = await Promise.allSettled(
        batch.map((student) =>
          generateStudentRiskProfile(student.student_id, filters)
        )
      );
 ...
 * 清理过期的预警记录
 */
export async function cleanupExpiredWarnings(
  daysOld: number = 30
): Promise<number> {
  try {
