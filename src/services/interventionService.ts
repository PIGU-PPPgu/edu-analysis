...
// 获取预警相关的干预计划
export const getInterventionPlans = async (
  warningId?: string
...
export const createInterventionPlan = async (
  plan: Omit<InterventionPlan, "id" | "created_at" | "updated_at">
...
export const updateInterventionPlan = async (
  id: string,
  updates: Partial<InterventionPlan>
): Promise<InterventionPlan | null> => {
  try {
    const { data, error } = await supabase
      .from("intervention_plans")
...
// 添加干预活动
export const addInterventionActivity = async (
  activity: Omit<InterventionActivity, "id">
): Promise<InterventionActivity | null> => {
  try {
    const { data, error } = await supabase
...
// 更新干预活动
export const updateInterventionActivity = async (
  id: string,
  updates: Partial<InterventionActivity>
): Promise<InterventionActivity | null> => {
  try {
    const { data, error } = await supabase
      .from("intervention_activities")
...
// 创建干预评估
export const createInterventionAssessment = async (
  assessment: Omit<InterventionAssessment, "id">
): Promise<InterventionAssessment | null> => {
  try {
    const { data, error } = await supabase
