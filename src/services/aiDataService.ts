...
import { fetchGradeData, fetchGradeDataByClass, fetchGradeDataBySubject, calculateGradeStatistics } from '@/api/gradeDataAPI';
...
export async function getAIDataSummary(): Promise<AIDataSummary> {
  try {
    ...
    if (recentExamsLimit.length > 0) {
      // 获取所有科目的数据
      const subjectsMap = new Map<string, { scores: number[], counts: number[] }>();
      ...
      for (const exam of recentExamsLimit) {
        if (exam.subject) {
          try {
            ...
            const gradeArray = Array.isArray(gradeDataResponse.data) ? gradeDataResponse.data : [];
      ...
      subjectsMap.forEach((data, subject) => {
        const averageScore = data.scores.length > 0 
          ...
          : 0;
        const participantCount = data.counts.length > 0 
          ? Math.round(data.counts.reduce((a, b) => a + b, 0) / data.counts.length)
          : 0;
        ...
        const passRate = averageScore > 0 ? Math.min((averageScore / 100) * 100, 100) : 0;
    ...
    const totalStudents = classOverview.reduce((sum, cls) => sum + cls.totalStudents, 0);
    const overallAverageScore = classOverview.length > 0
      ? classOverview.reduce((sum, cls) => sum + cls.averageScore, 0) / classOverview.length
      : 0;
 ...
 * @param examId 考试ID
 */
export async function getExamDetailAnalysis(examId: string): Promise<ExamDetailAnalysis> {
  try {
    ...
    const subjectBreakdown = Array.from(subjectMap.entries()).map(([subject, records]) => {
      const scores = records.map(r => r.total_score || r.score).filter(s => s !== null && s !== undefined);
      const totalScore = Math.max(...records.map(r => r.subject_total_score || 100).filter(s => s !== null));
      ...
      return {
        ...
        passRate: scores.length > 0 ? Math.round((scores.filter(s => s >= passThreshold).length / scores.length) * 100 * 100) / 100 : 0
      };
    });
    ...
    const classPerformance = Array.from(classMap.entries()).map(([className, records]) => {
      const scores = records.map(r => r.total_score || r.score).filter(s => s !== null && s !== undefined);
      const totalScore = Math.max(...records.map(r => r.subject_total_score || 100).filter(s => s !== null));
      const passThreshold = totalScore * 0.6;
    ...
    // 排序找出前5名和后5名
    const sortedStudents = allStudents.sort((a, b) => b.totalScore - a.totalScore);
...
export function getFrontendContext(): FrontendContextData {
  try {
    ...
    return {
      ...
      visibleData: { studentsInView: 0, chartsDisplayed: [], currentAnalysisMode: 'unknown' },
      userActions: { lastAction: 'unknown', timestamp: new Date().toISOString(), context: 'unknown' }
    };
  }
}
...
function getFrontendAnalysisMode(): string {
  try {
    ...
    // 从活跃的标签页获取
    const activeTab = document.querySelector('.active-tab, .tab-active, [aria-selected="true"]');
    if (activeTab) {
...
export async function getRelevantDataForQuestion(
  ...
  try {
    ...
    // 判断是否询问特定考试
    if (examId || question.includes('这次考试') || question.includes('本次考试') || question.includes('这场考试')) {
      if (!examId) {
    ...
    if (error.message?.includes('数据库表尚未创建') || error.message?.includes('does not exist')) {
      suggestion = '数据库尚未初始化，请先导入考试成绩数据。您可以通过"导入成绩"功能上传CSV文件。';
...
\n`;
  ...
  if (dataType === 'summary') {
    ...
    // 检查是否为演示数据
    const isDemoData = summary.recentExams.some(exam => exam.id.startsWith('demo-'));
