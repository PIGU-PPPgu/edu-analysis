...
export async function getBatchClassStatistics(
  classNames: string[]
): Promise<Map<string, ClassStatistics>> {
  ...
  try {
    ...
    for (const className of classNames) {
      const classStats: ClassStatistics = {
        ...
        overall_stats:
          overallStats.get(className)?.overall_stats ||
          getDefaultOverallStats(),
        subject_stats: subjectStats.get(className) || [],
        score_distribution:
          distributionData.get(className) || getDefaultDistribution(),
        time_trends: trendData.get(className) || [],
        student_groups:
          studentGroups.get(className) || getDefaultStudentGroups(),
      };
 ...
 * 获取班级对比分析数据
 */
export async function getClassComparison(options: {
  ...
  try {
    ...
    // 整体对比数据
    const classesData = Array.from(statsMap.entries()).map(
      ([className, stats]) => ({
        class_name: className,
    ...
    for (const subject of subjects) {
      const subjectData = Array.from(statsMap.entries())
        .map(([className, stats]) => {
          const subjectStats = stats.subject_stats.find(
            (s) => s.subject === subject
          );
          return subjectStats
            ? {
      ...
      if (subjectData.length > 0) {
        ...
        subjectData.forEach((cls, index) => {
          cls.rank = index + 1;
          cls.percentile =
            ((subjectData.length - index) / subjectData.length) * 100;
        });
 ...
 * 批量获取班级整体统计数据
 */
async function getBatchOverallStats(
  classNames: string[]
): Promise<
  Map<
    string,
    { total_students: number; overall_stats: ClassStatistics["overall_stats"] }
  >
> {
 ...
 * 批量获取班级学科统计数据
 */
async function getBatchSubjectStats(
  classNames: string[]
): Promise<Map<string, ClassStatistics["subject_stats"]>> {
 ...
 * 批量获取班级分数分布数据
 */
async function getBatchDistributionData(
  classNames: string[]
): Promise<Map<string, ClassStatistics["score_distribution"]>> {
  ...
  for (const [className, distribution] of Object.entries(groupedData)) {
    statsMap.set(
      className,
      distribution as ClassStatistics["score_distribution"]
    );
  }
 ...
 * 批量获取班级时间趋势数据
 */
async function getBatchTrendData(
  classNames: string[]
): Promise<Map<string, ClassStatistics["time_trends"]>> {
 ...
 * 批量获取班级学生分组数据
 */
async function getBatchStudentGroups(
  classNames: string[]
): Promise<Map<string, ClassStatistics["student_groups"]>> {
  ...
  const groupedData = data.reduce((acc: any, item: any) => {
    ...
    const student = {
      ...
      subjects_below_60: item.subjects_below_60
        ? item.subjects_below_60.split(",")
        : [],
      improvement_score: parseFloat(item.improvement_score) || 0,
      improvement_subjects: item.improvement_subjects
        ? item.improvement_subjects.split(",")
        : [],
    };
