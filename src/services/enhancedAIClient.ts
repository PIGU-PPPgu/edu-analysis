...
export class EnhancedAIClient {
        ...
        create: async (params: any) => {
          try {
            ...
            if (this.provider.authType === "bearer") {
              ...
              this.provider.authType === "header" &&
              this.provider.authKey
            ) {
    ...
    if (error.response) {
      ...
      if (this.provider.id === "deepseek") {
        if (errorMessage === "Model Not Exist") {
          const availableModels = this.provider.models
            .map((m) => m.id)
            .join(", ");
          console.error(
            `DeepSeek API错误: 模型 "${this.model}" 不存在。请尝试以下模型: ${availableModels}`
          );
          console.error(
            "提示: 最新的DeepSeek模型ID可能使用 deepseek-chat-v1 或 deepseek-coder-v1 格式"
          );
        } else if (error.response.status === 400) {
          console.error(
            "DeepSeek API错误: 请检查模型名称和参数格式。确保stream参数是布尔值而非字符串。"
          );
          console.error("当前使用的模型:", this.model);
          console.error(
            "可能的原因: 1) 模型ID不正确 2) API密钥无效 3) 请求格式错误"
          );
        }
      }
  ...
  getProviderInfo() {
    return {
      ...
      modelName:
        this.provider.models.find((m) => m.id === this.model)?.name ||
        this.model,
    };
  }
