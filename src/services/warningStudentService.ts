 ...
 * @returns 预警学生数据数组
 */
export const getActiveWarningStudents = async (limit: number = 6): Promise<WarningStudentData[]> => {
  try {
    // 查询活跃的预警记录，包含学生信息
    ...
    // 按学生分组统计预警信息
    const studentWarnings = warningRecords.reduce((acc, record) => {
      ...
      if (currentSeverity === 'high' || 
          (currentSeverity === 'medium' && acc[studentId].highestSeverity === 'low')) {
    ...
    // 计算平均成绩
    const avgScores = (gradeData || []).reduce((acc, grade) => {
    ...
    const warningStudents: WarningStudentData[] = (studentsData || [])
      .map(student => {
        ...
        const averageScore = avgScoreData ? avgScoreData.total / avgScoreData.count : undefined;
