...
class EnhancedStudentMatcher {
  ...
  async batchMatchStudents(students: StudentInfo[]): Promise<MatchResult[]> {
    ...
    for (let i = 0; i < students.length; i += batchSize) {
      ...
      console.log(`📦 处理批次 ${Math.floor(i / batchSize) + 1}/${Math.ceil(students.length / batchSize)}`);
    ...
    console.log(`✅ 批量匹配完成，成功匹配 ${results.filter(r => r.matchedStudent).length}/${students.length} 个学生`);
   ...
   * 执行实际的匹配逻辑
   */
  private async performMatching(studentInfo: StudentInfo): Promise<MatchResult> {
    // 1. 精确学号匹配（最高优先级）
    if (studentInfo.student_id && studentInfo.student_id.trim()) {
      const exactIdResult = await this.matchByStudentId(studentInfo.student_id.trim());
    ...
    // 4. 模糊姓名匹配（相似度匹配）
    if (studentInfo.name && studentInfo.name.trim()) {
      const fuzzyNameResult = await this.fuzzyMatchByName(studentInfo.name.trim());
   ...
   * 通过学号匹配
   */
  private async matchByStudentId(studentId: string): Promise<Partial<MatchResult>> {
    try {
   ...
   * 通过姓名和班级匹配
   */
  private async matchByNameAndClass(name: string, className: string): Promise<Partial<MatchResult>> {
    try {
      const { data, error } = await supabase
        .from('students')
        .select('*')
  ...
  private async fuzzyMatchByName(name: string): Promise<Partial<MatchResult>> {
    try {
      ...
      // 计算字符串相似度
      const candidates = data.map(student => ({
        student,
