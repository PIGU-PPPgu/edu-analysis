...
export class IntelligentPortraitService {
   ...
   * 基于成绩数据生成学生画像
   */
  async generateStudentPortrait(
    studentId: string
  ): Promise<StudentPortraitAnalysis | null> {
    try {
      ...
      const learningCharacteristics =
        this.identifyLearningCharacteristics(gradeData);
   ...
   * 生成小组画像
   */
  async generateGroupPortrait(
    groupId: string
  ): Promise<GroupPortraitAnalysis | null> {
    try {
      ...
      const academicComposition =
        this.analyzeGroupAcademicComposition(validPortraits);

      // 4. 分析协作特征
      const collaborationProfile =
        this.analyzeGroupCollaboration(validPortraits);

      // 5. 评估小组动态
      const groupDynamics = this.assessGroupDynamics(
        validPortraits,
        academicComposition
      );
      ...
      logInfo("小组画像生成完成", {
        groupId,
        memberCount: validPortraits.length,
      });
   ...
   * 生成班级画像
   */
  async generateClassPortrait(
    className: string
  ): Promise<ClassPortraitAnalysis | null> {
    try {
      logInfo("生成班级画像", { className });

      // 1. 获取班级基础统计（复用现有API）
      const { data: basicStats, error: statsError } = await supabase.rpc(
      ...
      const classCharacteristics =
        this.analyzeClassCharacteristics(studentPortraits);
   ...
   * 分析学术表现
   */
  private analyzeAcademicPerformance(
    gradeData: any[]
  ): StudentPortraitAnalysis["academic_profile"] {
    ...
    const subjectStrengths = Object.entries(subjectScores)
      .map(([subject, scores]) => {
        ...
        return {
          ...
          improvement_trend: this.calculateTrend(scores) as
            | "improving"
            | "stable"
            | "declining",
        };
   ...
   * 识别学习特征
   */
  private identifyLearningCharacteristics(
    gradeData: any[]
  ): StudentPortraitAnalysis["learning_characteristics"] {
    // 基于成绩模式推断学习类型
    const scoreVariance = this.calculateVariance(
      gradeData.map((d) => d.total_score || 0)
    );
    ...
    return {
      ...
      collaboration_style: this.inferCollaborationStyle(
        gradeData,
        scoreVariance
      ),
    };
  ...
  private async generateAIInsights(
    ...
    // 基于数据模式生成洞察（简化版AI逻辑）
    const personalityTraits = this.inferPersonalityTraits(
      academicProfile,
      learningCharacteristics
    );
    const learningStrategies = this.suggestLearningStrategies(
      academicProfile,
      learningCharacteristics
    );
    const potentialRoles = this.identifyPotentialRoles(
      academicProfile,
      learningCharacteristics
    );
    const developmentSuggestions =
      this.generateDevelopmentSuggestions(academicProfile);
   ...
   * 获取班级学生画像
   */
  private async getClassStudentPortraits(
    className: string
  ): Promise<StudentPortraitAnalysis[]> {
  ...
  private createBalancedGroups(
    ...
    for (let i = 0; i < groupCount; i++) {
      groups.push({
        ...
        group_balance: {
          academic_balance: 0,
          personality_balance: 0,
          skill_complementarity: 0,
        },
  ...
  // 基于真实成绩数据的推理方法
  private inferLearningType(
    subjectBalance: any
  ): StudentPortraitAnalysis["learning_characteristics"]["learning_type"] {
    // 基于各科目成绩分布推断学习类型
    ...
    const visualScores = subjects
      .filter((s) => visualSubjects.includes(s))
      .map((s) => subjectBalance[s]?.average || 0);
    const logicalScores = subjects
      .filter((s) => logicalSubjects.includes(s))
      .map((s) => subjectBalance[s]?.average || 0);
    ...
    const visualAvg =
      visualScores.length > 0
        ...
        : 0;
    const logicalAvg =
      logicalScores.length > 0
        ? logicalScores.reduce((a, b) => a + b, 0) / logicalScores.length
        : 0;
  ...
  private inferConcentrationPattern(
    gradeData: any[]
  ): StudentPortraitAnalysis["learning_characteristics"]["concentration_pattern"] {
    // 基于成绩稳定性推断专注度模式
  ...
  private inferDifficultyPreference(
    gradeData: any[]
  ): StudentPortraitAnalysis["learning_characteristics"]["difficulty_preference"] {
    // 基于成绩平均水平推断难度偏好
    const avgScore =
      gradeData.reduce((sum, d) => sum + (d.total_score || 0), 0) /
      gradeData.length;
  ...
  private inferCollaborationStyle(
    gradeData: any[],
    scoreVariance: number
  ): StudentPortraitAnalysis["learning_characteristics"]["collaboration_style"] {
  ...
  private inferPersonalityTraits(
    ...
    if (learningCharacteristics.collaboration_style === "collaborative")
      traits.push("善于合作");
    return traits;
  }

  private suggestLearningStrategies(
    academicProfile: any,
    learningCharacteristics: any
  ): string[] {
    const strategies = [];
    if (academicProfile.subject_weaknesses.length > 0) {
      strategies.push(
        `重点加强${academicProfile.subject_weaknesses[0]?.subject}学科`
      );
    }
  ...
  private identifyPotentialRoles(
    academicProfile: any,
    learningCharacteristics: any
  ): string[] {
    const roles = [];
  ...
  private generateDevelopmentSuggestions(academicProfile: any): string[] {
    ...
    if (academicProfile.subject_weaknesses.length > 0) {
      suggestions.push(
        `建议针对${academicProfile.subject_weaknesses[0]?.subject}制定专项提升计划`
      );
    }
    return suggestions;
  }
  ...
  // 数据库操作方法
  private async saveStudentPortrait(
    studentId: string,
    portrait: StudentPortraitAnalysis
  ): Promise<void> {
    try {
  ...
  private async getStudentPortraitFromDB(
    studentId: string
  ): Promise<StudentPortraitAnalysis | null> {
    // 尝试从缓存或数据库获取已生成的画像
    try {
  ...
  private calculateDataQualityScore(gradeData: any[]): number {
    ...
    const validFields = gradeData.reduce((count, record) => {
      return (
        count +
        Object.values(record).filter((val) => val !== null && val !== undefined)
          .length
      );
    }, 0);
  ...
  // 小组分配算法实现
  private createMixedAbilityGroups(
    students: StudentPortraitAnalysis[],
    groupCount: number
  ): GroupAllocationResult[] {
  ...
  private createHomogeneousGroups(
    students: StudentPortraitAnalysis[],
    groupCount: number
  ): GroupAllocationResult[] {
    // 同质化分组：相似能力的学生分在一组
      ...
      groups.push({
        ...
        predicted_performance:
          groupStudents.reduce(
            (sum, s) => sum + s.academic_profile.overall_average,
            0
  ...
  private optimizeGroupAllocation(
    groups: GroupAllocationResult[],
    students: StudentPortraitAnalysis[]
  ): GroupAllocationResult[] {
    // 基于实际数据优化分组配置
    groups.forEach((group) => {
      ...
      group.group_balance.academic_balance = Math.max(0, 100 - variance);
      group.group_balance.skill_complementarity = Math.min(
        100,
        avgScore + (100 - variance)
      );
  ...
  private async saveGroupAllocations(
    className: string,
    groups: GroupAllocationResult[]
  ): Promise<void> {
    // 保存分组结果到数据库
  ...
  private analyzeGroupAcademicComposition(
    portraits: StudentPortraitAnalysis[]
  ): GroupPortraitAnalysis["academic_composition"] {
    const avgPerformance =
      portraits.reduce(
        (sum, p) => sum + p.academic_profile.overall_average,
        0
      ) / portraits.length;
  ...
  private analyzeGroupCollaboration(
    portraits: StudentPortraitAnalysis[]
  ): GroupPortraitAnalysis["collaboration_profile"] {
    return {
  ...
  private assessGroupDynamics(
    portraits: StudentPortraitAnalysis[],
    academicComp: any
  ): GroupPortraitAnalysis["group_dynamics"] {
    return {
      cohesion_score: 80,
  ...
  private async saveGroupPortrait(
    groupId: string,
    portrait: GroupPortraitAnalysis
  ): Promise<void> {
    logInfo("保存小组画像", { groupId });
  }

  private analyzeClassCharacteristics(
    portraits: StudentPortraitAnalysis[]
  ): ClassPortraitAnalysis["class_characteristics"] {
    const scores = portraits.map((p) => p.academic_profile.overall_average);
  ...
  private async analyzeClassGroups(
    className: string,
    groups: any[]
  ): Promise<ClassPortraitAnalysis["group_analysis"]> {
    return {
  ...
  private generateTeachingRecommendations(
    ...
    if (characteristics.academic_distribution.needs_support > 3) {
      recommendations.differentiated_instruction.push(
        "建议为基础薄弱学生提供额外辅导"
      );
    }

    if (characteristics.academic_distribution.high_achievers > 5) {
      recommendations.group_activity_suggestions.push(
        "可以让优秀学生担任小组长，带动其他同学"
      );
    }
  ...
  private async saveClassPortrait(
    className: string,
    portrait: ClassPortraitAnalysis
  ): Promise<void> {
    logInfo("保存班级画像", { className });
  }

  private assignGroupRole(
    student: StudentPortraitAnalysis,
    group: GroupAllocationResult
  ): "leader" | "collaborator" | "supporter" | "specialist" {
    if (student.academic_profile.overall_average > 85) return "leader";
    if (
      student.learning_characteristics.collaboration_style === "collaborative"
    )
      return "collaborator";
    if (student.academic_profile.subject_strengths.length > 2)
      return "specialist";
    return "supporter";
  }
  ...
  private calculateGroupBalance(
    group: GroupAllocationResult,
    allStudents: StudentPortraitAnalysis[]
  ): GroupAllocationResult["group_balance"] {
    return {
      academic_balance: 85,
  ...
  private predictGroupPerformance(
    ...
    const memberPortraits = allStudents.filter((s) =>
      memberIds.includes(s.student_id)
    );
    const avgScore =
      memberPortraits.reduce(
        (sum, p) => sum + p.academic_profile.overall_average,
        0
      ) / memberPortraits.length;
