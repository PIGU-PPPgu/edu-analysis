...
export class ExamDuplicateChecker {
    ...
    try {
      ...
      if (exactMatch) {
        return {
          ...
          conflictType: 'exact',
          similarity: 1.0,
          suggestions: await this.generateSuggestions(examInfo, exactMatch, 'exact')
        };
      }
      ...
      if (similarMatch.exam && similarMatch.similarity > 0.8) {
        return {
          ...
          suggestions: await this.generateSuggestions(examInfo, similarMatch.exam, 'similar')
        };
      }
  ...
  async resolveDuplicate(
    ...
    switch (this.strategy) {
      case 'ask_user':
        return await this.handleUserChoice(examInfo, duplicateResult, userChoice);
      ...
      case 'auto_replace':
        return await this.replaceExisting(examInfo, duplicateResult.existingExam);
   ...
   * 查找精确匹配的考试
   */
  private async findExactMatch(examInfo: ExamInfo): Promise<ExistingExam | null> {
   ...
   * 查找相似的考试
   */
  private async findSimilarMatch(examInfo: ExamInfo): Promise<{ exam: ExistingExam | null; similarity: number }> {
    // 获取时间范围内的考试（前后7天）
  ...
  private calculateSimilarity(examInfo: ExamInfo, existingExam: any): number {
    ...
    // 标题相似度 (权重: 40%)
    const titleSimilarity = this.calculateStringSimilarity(examInfo.title, existingExam.title);
    ...
    // 日期接近度 (权重: 20%)
    const dateDiff = Math.abs(new Date(examInfo.date).getTime() - new Date(existingExam.date).getTime());
   ...
   * 合并到现有考试
   */
  private async mergeToExisting(existingExam: ExistingExam): Promise<{ examId: string; action: string; message: string }> {
    return {
   ...
   * 自动重命名创建
   */
  private async createWithAutoRename(examInfo: ExamInfo): Promise<{ examId: string; action: string; message: string }> {
    let index = 1;
