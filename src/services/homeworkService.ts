...
export async function gradeHomework(data: {
  ...
  try {
    ...
    if (!studentId || !homeworkId) {
      console.error(
        `最终检查失败 - 缺少参数：学生ID: ${studentId}, 作业ID: ${homeworkId}`
      );
      return {
        success: false,
    ...
    if (!isValidUuid) {
      ...
      console.warn(
        `最终检查 - 学生ID不是有效的UUID格式: ${studentId}，尝试通过学号查找...`
      );
    ...
    if (isTemporarySubmission) {
      ...
      if (existingSubmission && existingSubmission.id) {
        console.log(
          `已找到该学生的现有提交记录 ID: ${existingSubmission.id}，将更新而非创建`
        );
    ...
    if (
      data.knowledgePointEvaluations &&
      data.knowledgePointEvaluations.length > 0
    ) {
      try {
        console.log(
          "处理知识点评估数据:",
          data.knowledgePointEvaluations.length,
          "项，提交ID:",
          finalSubmissionId
        );
        // 知识点评估服务已经是静态导入的
    ...
    if (knowledgePointsSuccessfullyAssessed) {
      console.log(
        "更新 knowledge_points_assessed 标志为 true for:",
        finalSubmissionId
      );
      ...
      if (updateKpFlagError) {
        console.error(
          "更新 knowledge_points_assessed 标志失败:",
          updateKpFlagError
        );
...
export async function deleteHomework(
  ...
  try {
    ...
    if (force && submissions && submissions.length > 0) {
      ...
      console.log(
        `强制删除作业，将删除 ${submissions.length} 条提交记录及关联数据`
      );
      ...
      try {
        ...
        if (deleteKnowledgeMasteryError1) {
          console.warn(
            "删除作业相关的knowledge_mastery记录失败:",
            deleteKnowledgeMasteryError1
          );
        }
        ...
        if (deleteKnowledgeMasteryError2) {
          console.warn(
            "删除submission相关的knowledge_mastery记录失败:",
            deleteKnowledgeMasteryError2
          );
        }
      } catch (err) {
      ...
      try {
        ...
        if (deleteSkpError) {
          console.warn(
            "删除submission_knowledge_points数据失败:",
            deleteSkpError
          );
        }
      } catch (err) {
    ...
    if (error) {
      ...
      if (error.code === "23503") {
        ...
        toast.error(
          `删除作业失败: 该作业正被其他记录引用（外键约束）。错误详情: ${error.message}`,
          {
            duration: 8000,
          }
        );
    ...
    // 6. 尝试删除作业相关文件（如果有）
    try {
      const { supabase: supabaseStorage } = await import(
    ...
    toast.success(successMessage, {
      description: force
        ? `已删除作业及相关的${submissions?.length || 0}条提交记录`
        : undefined,
      duration: 5000,
    });
