...
async function initMappingCache() {
  ...
  try {
    ...
    mappings?.forEach(mapping => {
      ...
      reverseMappingCache!.set(mapping.grade_table_id, mapping.student_table_id)
    })
 ...
 * 获取学生表ID对应的成绩表ID
 */
export async function getGradeTableId(studentTableId: string): Promise<string | null> {
 ...
 * 获取成绩表ID对应的学生表ID
 */
export async function getStudentTableId(gradeTableId: string): Promise<string | null> {
  await initMappingCache()
 ...
 * 批量获取学生表ID对应的成绩表ID
 */
export async function batchGetGradeTableIds(studentTableIds: string[]): Promise<Map<string, string>> {
  await initMappingCache()
 ...
 * 批量获取成绩表ID对应的学生表ID
 */
export async function batchGetStudentTableIds(gradeTableIds: string[]): Promise<Map<string, string>> {
  await initMappingCache()
  const result = new Map<string, string>()
...
export async function validateMapping() {
  ...
  const validCount = validationResults.filter(r =>
    r.studentExists && r.gradeExists && r.nameMatch
  ).length
