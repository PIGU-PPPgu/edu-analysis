...
const ENHANCED_FIELD_RULES = {
  // 学生标识
  student_id: [
    '学号', '考号', '学生号', '学生编号', '准考证号', 'student_id', 'id', '编号',
    '学籍号', '考生号', '序号', 'no', 'number', '学生学号'
  ],
  
  // 学生姓名
  name: [
    '姓名', '学生姓名', 'name', 'student_name', '名字', '学生', 
    '考生姓名', '考生', 'fullname', 'full_name'
  ],
  
  // 班级信息 - 扩展识别模式
  class_name: [
    '班级', '班级名称', '现班', '行政班级', '教学班', '所在班级', 'class', 'class_name',
    '班次', '班别', '年级班级', '分班', '班组', '组别', '学习小组', 'classroom',
    '班', '年班', '级班', 'grade_class', '届', '年级', '班号'
  ],
  
  // 成绩相关
  score: [
    '成绩', '分数', '得分', 'score', '总分', '总成绩', '卷面分', '实际得分',
    '原始分', '标准分', 'total_score', '合计', '总计'
  ],
  ...
  politics_score: ['政治', '道法', '思政', '道德与法治', 'politics', '思想品德'],
  history_score: ['历史', '历史成绩', '历史分数', 'history'],
  
  // 排名信息 - 关键修复点
  rank_in_class: [
    '班排名', '班级排名', '班内排名', '班排', '班次排名', 'class_rank',
    '本班排名', '班排序', '班位次', '班级名次', '年级排名(班)', '班内名次'
  ],
  
  rank_in_grade: [
    '年级排名', '级排名', '年排', '全年级排名', 'grade_rank', '级排',
    '年级名次', '级内排名', '年级位次', '全级排名', '学校排名', '校排名'
  ],
  
  rank_in_school: [
    '校排名', '学校排名', '全校排名', 'school_rank', '总排名',
    '全校名次', '学校位次', '校内排名'
  ],
  
  // 等级信息
  grade_level: [
    '等级', '级别', '层次', 'grade', 'level', '评级', '档次',
    '水平', '标准', '总等级', '综合等级'
  ],
...
function performRuleBasedMapping(headers: string[]): AIParsingResult {
  ...
  headers.forEach(header => {
    ...
    // 遍历所有字段规则
    for (const [standardField, patterns] of Object.entries(ENHANCED_FIELD_RULES)) {
...
function inferDataType(standardField: string, originalHeader: string): string {
  ...
  if (standardField.includes('date') || originalHeader.includes('时间') || originalHeader.includes('日期')) {
    return 'date';
  }
 ...
 */
async function performAIMapping(headers: string[], sampleData: any[]): Promise<AIParsingResult | null> {
  try {
    ...
    const { data, error } = await supabase.functions.invoke('proxy-ai-request', {
      body: {
        ...
        messages: [
          {
            role: 'system',
            content: '你是一个专业的教育数据分析助手，专门处理学生成绩数据的字段映射。'
          },
          {
            role: 'user',
            content: prompt
...
export async function enhanceFileParsing(
  ...
  if (Object.keys(validatedResult.mappings).length > 0) {
    toast.success(`成功识别 ${Object.keys(validatedResult.mappings).length} 个字段`, {
...
function performFuzzyMapping(headers: string[]): Record<string, string> {
  ...
  headers.forEach(header => {
    ...
    for (const [standardField, patterns] of Object.entries(ENHANCED_FIELD_RULES)) {
      for (const pattern of patterns) {
        const distance = levenshteinDistance(lowerHeader, pattern.toLowerCase());
    ...
    if (bestField && !fuzzyMappings[header]) {
      ...
      console.log(`🔍 模糊匹配: "${header}" -> "${bestField}" (相似: "${bestMatch}")`);
    }
  });
 ...
 * 验证和优化映射结果
 */
function validateMappings(result: AIParsingResult, headers: string[]): AIParsingResult {
...
export async function saveFieldMappings(
  ...
  try {
    const fieldMappingRecords = Object.entries(mappings).map(([original, mapped]) => ({
      batch_id: batchId,
