...
function calculateCategoryChanges(current: Record<string, number>, previous: Record<string, number>): Record<string, number> {
...
function calculateScopeChanges(current: Record<string, number>, previous: Record<string, number>): Record<string, number> {
  const changes: Record<string, number> = {};
  const allScopes = new Set([...Object.keys(current || {}), ...Object.keys(previous || {})]);
...
export async function getWarningDataForPeriod(startDate: Date, endDate: Date): Promise<Omit<WarningData, 'period' | 'startDate' | 'endDate'>> {
  ...
  const [studentsRes, warningsRes] = await Promise.all([
    ...
    supabase
      ...
      .select('student_id, status, created_at, warning_rules!inner(severity, name)', { count: 'exact' })
      .in('status', ['active', 'resolved', 'dismissed']) // 包含所有状态
      .gte('created_at', startDateStr)
      .lte('created_at', endDateStr),
  ]);

  if (studentsRes.error) throw new Error(`获取学生数据失败: ${studentsRes.error.message}`);
  if (warningsRes.error) throw new Error(`获取预警记录失败: ${warningsRes.error.message}`);
  ...
  const riskRatio = totalStudents > 0 ? Number((studentsAtRisk / totalStudents * 100).toFixed(1)) : 0;
  
  const highRiskStudentIds = new Set(
    warnings.filter(w => w.warning_rules?.severity === 'high').map(w => w.student_id)
  );
  ...
  for (const warning of warnings) {
    ...
    if (rule) {
      if (rule.severity) riskDistribution[rule.severity] = (riskDistribution[rule.severity] || 0) + 1;
      if (rule.name) categoryDistribution[rule.name] = (categoryDistribution[rule.name] || 0) + 1;
      if (rule.scope) scopeDistribution[rule.scope] = (scopeDistribution[rule.scope] || 0) + 1;
    }
  }
...
export async function getWarningHistoryComparison(timeRange: string): Promise<HistoryComparison> {
  return warningAnalysisCache.getHistoryComparison(async () => {
    ...
    const previousPeriodData = await getWarningDataForPeriod(previousStartDate, previousEndDate);
    
    const comparison: ComparisonMetrics = {
        studentsAtRiskChange: calculatePercentageChange(currentPeriodData.studentsAtRisk, previousPeriodData.studentsAtRisk),
        riskRatioChange: Number((currentPeriodData.riskRatio - previousPeriodData.riskRatio).toFixed(1)),
        highRiskStudentsChange: calculatePercentageChange(currentPeriodData.highRiskStudents, previousPeriodData.highRiskStudents),
        trends: {
          overall: getTrendDirection(currentPeriodData.studentsAtRisk, previousPeriodData.studentsAtRisk),
          riskLevel: getTrendDirection(currentPeriodData.highRiskStudents, previousPeriodData.highRiskStudents),
          resolution: getTrendDirection(currentPeriodData.resolvedIssues, previousPeriodData.resolvedIssues)
        },
        categoryChanges: calculateCategoryChanges(currentPeriodData.categoryDistribution, previousPeriodData.categoryDistribution),
        scopeChanges: calculateScopeChanges(currentPeriodData.scopeDistribution, previousPeriodData.scopeDistribution)
    };
...
export async function getWarningTrendData(timeRange: string): Promise<WarningTrendData[]> {
    return warningAnalysisCache.getHistoryComparison(async () => {
        ...
        const trendDataPromises = points.map(async (point) => {
            ...
            const totalWarnings = (periodData.newIssues || 0) + (periodData.resolvedIssues || 0);
            ...
            return {
                ...
                studentsAtRisk: isNaN(periodData.studentsAtRisk) ? 0 : periodData.studentsAtRisk,
                resolvedIssues: isNaN(periodData.resolvedIssues) ? 0 : periodData.resolvedIssues,
                ...
                gradeRelated: isNaN(totalWarnings * 0.4) ? 0 : Math.round(totalWarnings * 0.4),
                behaviorRelated: isNaN(totalWarnings * 0.3) ? 0 : Math.round(totalWarnings * 0.3),
                attendanceRelated: isNaN(totalWarnings * 0.3) ? 0 : Math.round(totalWarnings * 0.3),
                progressRate: isNaN(periodData.resolvedIssues / totalWarnings * 100) ? 0 : 
                             Math.round((periodData.resolvedIssues / (totalWarnings || 1)) * 100),
