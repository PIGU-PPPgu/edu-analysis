...
export class WarningSystemIntegrationTester {
  ...
  private async prepareTestData(testDataCount: number): Promise<void> {
    await this.runTest('准备测试数据', async () => {
      // 生成测试学生
      this.testData.students = TestDataGenerator.generateTestStudents(testDataCount);
      ...
      if (studentsError) throw new Error(`插入测试学生失败: ${studentsError.message}`);
      ...
      if (gradesError) throw new Error(`插入测试成绩失败: ${gradesError.message}`);
  ...
  private async testEdgeFunctions(): Promise<void> {
    await this.runTest('Edge Functions执行测试', async () => {
      ...
      if (!data.success) throw new Error(`Edge Function执行失败: ${data.error}`);
  ...
  private async testAutoTriggerMechanism(): Promise<void> {
    await this.runTest('自动触发机制测试', async () => {
      ...
      if (insertError) throw new Error(`插入触发成绩失败: ${insertError.message}`);
      ...
      if (warningsError) throw new Error(`查询预警记录失败: ${warningsError.message}`);
  ...
  private async testRealTimeNotifications(): Promise<void> {
    await this.runTest('实时通知系统测试', async () => {
      ...
      if (templatesError) throw new Error(`查询通知模板失败: ${templatesError.message}`);
  ...
  private async testTrackingSystem(): Promise<void> {
    await this.runTest('追踪系统测试', async () => {
      ...
      if (execution.execution_status === 'pending') throw new Error('执行状态未更新');
  ...
  private async testPerformanceOptimizations(): Promise<void> {
    await this.runTest('性能优化测试', async () => {
      ...
      const { data: stats, error } = await supabase.rpc('get_warning_statistics_optimized', {
        time_range_days: 30
      });
      ...
      // 验证返回数据结构
      const stat = stats[0];
      const requiredFields = ['total_students', 'warning_students', 'total_warnings'];
      for (const field of requiredFields) {
        if (stat[field] === undefined) throw new Error(`统计结果缺少字段: ${field}`);
      }

      // 测试执行统计查询
      const { data: execStats, error: execError } = await supabase.rpc('get_execution_statistics', {
  ...
  private async testDataConsistency(): Promise<void> {
    await this.runTest('数据一致性验证', async () => {
      ...
      const { data: warnings, error: warningsError } = await supabase
        .from('warning_records')
        .select('*')
        .in('student_id', this.testData.students.map(s => s.student_id));
      ...
      // 验证数据关联性
      if (warnings && warnings.length > 0 && executions && executions.length > 0) {
        const warningCount = warnings.length;
        const executionWarningCount = executions.reduce((sum, exec) => sum + exec.generated_warnings, 0);
        
        // 允许一定的数据延迟和差异
        if (Math.abs(warningCount - executionWarningCount) > warningCount * 0.5) {
          throw new Error(`数据不一致: 预警记录${warningCount}条, 执行记录显示${executionWarningCount}条`);
        }
      }
    ...
    await this.runTest('清理测试数据', async () => {
      ...
      try {
        ...
        const { error: warningsError } = await supabase
          ...
          .delete()
          .in('student_id', this.testData.students.map(s => s.student_id));
        if (warningsError) cleanupErrors.push(`清理预警记录: ${warningsError.message}`);
        ...
        const { error: gradesError } = await supabase
          ...
          .delete()
          .in('student_id', this.testData.students.map(s => s.student_id));
        if (gradesError) cleanupErrors.push(`清理成绩数据: ${gradesError.message}`);
        ...
        const { error: studentsError } = await supabase
          ...
          .delete()
          .in('student_id', this.testData.students.map(s => s.student_id));
        if (studentsError) cleanupErrors.push(`清理学生数据: ${studentsError.message}`);
        ...
        if (this.testData.ruleId) {
          ...
          if (ruleError) cleanupErrors.push(`清理测试规则: ${ruleError.message}`);
        }
  ...
  private generateTestReport(): IntegrationTestReport {
    ...
    const systemStatus = {
      ...
      databaseStatus: this.getComponentStatus(['系统基础设施检查', '数据一致性验证']),
  ...
  // 获取组件状态
  private getComponentStatus(testNames: string[]): 'healthy' | 'unhealthy' | 'unknown' {
    ...
    const failedCount = relevantTests.filter(r => r.status === 'failed').length;
