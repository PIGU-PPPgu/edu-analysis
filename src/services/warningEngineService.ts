 ...
 * 执行所有预警规则
 */
export async function executeWarningRules(trigger?: string): Promise<WarningEngineResult | null> {
  try {
    // 调用 Edge Function
  ...
  try {
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116' && error.code !== '42P01') {
        console.warn('[WarningEngineService] 查询warning_executions失败:', error.message);
      } else {
        recentExecution = data;
      }
    } catch (tableError: any) {
      console.warn('[WarningEngineService] warning_executions表不存在，跳过查询');
    }
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116' && error.code !== '42P01') {
        console.warn('[WarningEngineService] 查询warning_engine_stats失败:', error.message);
      } else {
        todayStats = data;
      }
    } catch (tableError: any) {
      console.warn('[WarningEngineService] warning_engine_stats表不存在，跳过查询');
    }
    ...
    try {
      ...
      if (error && error.code !== 'PGRST116' && error.code !== '42P01') {
        console.warn('[WarningEngineService] 查询running executions失败:', error.message);
      } else {
        runningExecutions = data || [];
      }
    } catch (tableError: any) {
      console.warn('[WarningEngineService] warning_executions表不存在，跳过运行状态检查');
    }
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116' || error.code === '42P01') {
        console.warn('[WarningEngineService] warning_execution_summary表不存在，返回空历史记录');
        return [];
      }
...
export async function getWarningEngineStats(days: number = 7) {
  try {
    ...
    if (error) {
      // 如果表不存在，返回空数组而不报错
      if (error.code === 'PGRST116' || error.code === '42P01') {
        console.warn('[WarningEngineService] warning_engine_stats表不存在，返回空统计数据');
        return [];
      }
 ...
 * 取消运行中的执行
 */
export async function cancelWarningExecution(executionId: string, reason?: string): Promise<boolean> {
  try {
    ...
    if (error) {
      ...
      if (error.code === 'PGRST116' || error.code === '42P01') {
        console.warn('[WarningEngineService] warning_executions表不存在，取消操作跳过');
  ...
  try {
    ...
    if (results[2].status === 'fulfilled' && !results[2].value.error) {
      ...
      if (error.code !== 'PGRST116' && error.code !== '42P01') {
        ...
        console.warn('[WarningEngineService] warning_execution_results表不存在');
      }
    }
