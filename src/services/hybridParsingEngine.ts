...
class HybridParsingEngine {
   ...
   * 主要解析入口 - 智能选择解析策略
   */
  async parseHeaders(headers: string[], sampleData?: any[]): Promise<ParsingResult> {
   ...
   * 快速算法解析 - 标准模式数据
   */
  private async fastAlgorithmParsing(headers: string[]): Promise<ParsingResult> {
   ...
   * 标准混合解析 - AI辅助算法
   */
  private async standardHybridParsing(headers: string[], sampleData?: any[]): Promise<ParsingResult> {
    // 先用算法快速识别明确的字段
    const algorithmFields = headers.map(h => this.algorithmMatch(h)).filter(Boolean) as ParsedField[];
    const unmatched = headers.filter(h => !algorithmFields.some(f => f.originalName === h));
    
    // 只对未匹配的字段使用AI
    const aiFields = unmatched.length > 0 ? await this.smartAIAnalysis(unmatched, sampleData) : [];
   ...
   * 全面AI解析 - 复杂数据处理
   */
  private async comprehensiveAIParsing(headers: string[], sampleData?: any[]): Promise<ParsingResult> {
    // 使用AI分析所有字段，但仍然保留算法验证
    const aiResult = await this.smartAIAnalysis(headers, sampleData);
    const algorithmResult = headers.map(h => this.algorithmMatch(h)).filter(Boolean) as ParsedField[];
      ...
      for (const pattern of patterns) {
        if (pattern.test(lowerHeader)) {
          ...
          return {
            ...
            confidence: 0.9,
            method: 'algorithm',
            dataType: fieldType === 'score' ? 'number' : fieldType === 'grade' ? 'grade' : 'rank'
          };
        }
      }
    }
   ...
   * 智能AI分析 - 只在必要时调用
   */
  private async smartAIAnalysis(headers: string[], sampleData?: any[]): Promise<ParsedField[]> {
    // 这里集成现有的AI服务，但加入性能优化
    try {
      ...
      return headers.map(h => this.algorithmMatch(h)).filter(Boolean) as ParsedField[];
    }
  }
   ...
   * 结果融合 - 多重验证提升置信度
   */
  private fuseResults(aiResults: ParsedField[], algorithmResults: ParsedField[]): ParsedField[] {
    const fused: ParsedField[] = [];
    
    for (const aiField of aiResults) {
      const algorithmField = algorithmResults.find(f => f.originalName === aiField.originalName);
      ...
      if (algorithmField) {
        ...
        if (algorithmField.mappedName === aiField.mappedName) {
          fused.push({
            ...aiField,
            confidence: Math.min(0.98, (aiField.confidence + algorithmField.confidence) / 2 + 0.1),
            method: 'hybrid'
          });
        } else {
          // 结果不一致 - 选择置信度更高的
          fused.push(aiField.confidence > algorithmField.confidence ? aiField : algorithmField);
        }
      } else {
        // 只有AI识别到的字段
        fused.push(aiField);
    ...
    for (const algorithmField of algorithmResults) {
      if (!aiResults.some(f => f.originalName === algorithmField.originalName)) {
   ...
   * 数据复杂度分析 - 选择合适的解析策略
   */
  private analyzeComplexity(headers: string[], sampleData?: any[]): 'simple' | 'medium' | 'complex' {
    // 快速启发式判断
    const standardFieldCount = headers.filter(h => this.algorithmMatch(h)).length;
   ...
   * AI服务调用（占位符 - 实际项目中实现）
   */
  private async callAIService(headers: string[], sampleData?: any[]): Promise<any> {
    // 这里应该调用实际的AI服务
    // 为了示例，返回模拟数据
    return {
