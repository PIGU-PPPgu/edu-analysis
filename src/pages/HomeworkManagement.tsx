...
import { PlusCircle, FileText, Search, BarChart3 } from "lucide-react";
// import { HomeworkAnalysisDashboard } from "@/components/analysis"; // 已删除
import {
  Card,
  CardContent,
  CardDescription,
        ...
        for (const homework of homeworks) {
          try {
            ...
            if (result.success && Array.isArray(result.submissions)) {
              ...
              pendingCount += result.submissions.filter(
                (sub) => sub.status === "submitted"
              ).length;
            } else if (!result.success) {
              // 如果获取失败，可以选择记录错误或跳过
              console.warn(
                `获取作业 ${homework.id} 的提交数据失败:`,
                result.error
              );
            }
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {loading ? "..." : stats.overdueHomeworks}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    已截止的作业数量
                  </p>
