...
import {
  ...
  parseExcel,
  isBinaryContent,
  enhancedGenerateInitialMappings,
...
export const useFileProcessing = () => {
  ...
  const processFile = async (
    file: File,
    isAIEnhanced: boolean = true
  ): Promise<ParsedData | null> => {
    console.log(
      "[useFileProcessing] 开始处理文件:",
      file.name,
      "AI增强:",
      isAIEnhanced
    );
    ...
    try {
      ...
      if (
        parseResult.metadata.detectedStructure === "wide" &&
        shouldSkipMapping
      ) {
        ...
        console.log(
          `[useFileProcessing] 宽表转换完成，共生成 ${finalData.length} 条记录`
        );
        ...
        console.log(
          "[useFileProcessing] ✅ 智能解析自动转换完成，autoProcessed=true，应该跳过字段映射步骤"
        );
      ...
      const result: ParsedData = {
        ...
        detectedFormat: parseResult.metadata.fileType,
        confidence: parseResult.metadata.confidence,
        fieldMappings: shouldSkipMapping
          ? {}
          : parseResult.metadata.suggestedMappings, // 高置信度时返回空映射
        intelligentParseResult: {
          success: true,
          data: finalData,
          metadata: {
      ...
      if (shouldSkipMapping) {
        console.log(
          "[useFileProcessing] 智能解析置信度足够高，跳过字段映射步骤"
        );
        toast.success("智能解析成功", {
        ...
        console.log(
          "[useFileProcessing] 智能解析置信度较低，需要手动确认字段映射"
        );
        toast.info("需要确认字段映射", {
      ...
      try {
        ...
        // 生成传统字段映射
        const fieldMappings = enhancedGenerateInitialMappings(
          headers,
