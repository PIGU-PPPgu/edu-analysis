 ...
 */
export function useCachedQuery<T = any>(options: CachedQueryOptions<T>): CachedQueryResult<T> {
  const {
    cacheKey,
    queryFn,
  ...
  const [cacheInfo, setCacheInfo] = useState<CachedQueryResult<T>['cacheInfo']>(null);
  ...
  const checkServerCache = async (cachedVersion?: number): Promise<boolean> => {
    ...
    try {
      ...
      // 如果提供了版本号，检查是否匹配
      if (cachedVersion !== undefined && serverCacheInfo.version !== cachedVersion) {
        logDebug('缓存版本不匹配', { cachedVersion, serverVersion: serverCacheInfo.version });
        return false;
      }
      ...
      setCacheInfo({
        ...
        expires: new Date(serverCacheInfo.last_updated).getTime() + (serverCacheInfo.expiry_seconds * 1000),
  ...
  const executeQuery = async () => {
    ...
    try {
      ...
      logDebug('查询执行成功，已更新缓存', { result, cacheInfo: clientCache[cacheKey] });
    } catch (e: any) {
  ...
  const clearCache = () => {
    ...
    // 如果使用服务器缓存，也尝试使其失效
    if (useServerCache) {
      client.rpc('invalidate_cache', { p_cache_key: cacheKey })
