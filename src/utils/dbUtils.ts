...
export const db = {
  ...
  async getWarningStatistics() {
    try {
      ...
      const highRisk = warningStudents.filter(
        (s) => s.risk_level === "high"
      ).length;
      const mediumRisk = warningStudents.filter(
        (s) => s.risk_level === "medium"
      ).length;
      const lowRisk = warningStudents.filter(
        (s) => s.risk_level === "low"
      ).length;
  ...
  async getRiskFactors() {
    try {
      // 获取最近一次考试的分数
      const { data: latestGradesData, error: latestGradesError } =
        await supabase
          .from("grades")
          .select("score")
      ...
      return [
        {
          ...
          value: Math.min(
            100,
            scoreRiskFactor + Math.round(Math.random() * 20)
          ),
        },
        {
          factor: "作业完成",
          value: Math.min(
            100,
            scoreRiskFactor - Math.round(Math.random() * 15)
          ),
        },
        { factor: "考试成绩", value: scoreRiskFactor },
        {
          ...
          value: Math.min(
            100,
            scoreRiskFactor - Math.round(Math.random() * 10)
          ),
        },
        {
          factor: "学习态度",
    ...
    try {
      ...
      for (const item of data) {
        try {
          ...
          results.errors.push(
            `学生 ${item.name}(${item.studentId}) 的 ${item.subject} 成绩保存失败: ${error.message}`
          );
        }
      }
      ...
      for (const student of students) {
        ...
        studentGrades.forEach((grade) => {
          if (!subjectAvgScores[grade.subject]) {
            ...
            // 简单平均
            subjectAvgScores[grade.subject] =
              (subjectAvgScores[grade.subject] + grade.score) / 2;
          }
        ...
        // 计算总平均分
        const avgScore =
          allSubjectsScores.reduce((sum, score) => sum + score, 0) /
          allSubjectsScores.length;
        ...
        const attendanceRate = Math.min(
          100,
          avgScore + (Math.random() * 20 - 10)
        ); // 模拟出勤率，基于平均分略有波动
        const homeworkRate = Math.min(
          100,
          avgScore + (Math.random() * 15 - 7.5)
        ); // 模拟作业完成率
        const participationRate = Math.min(
          100,
          avgScore + (Math.random() * 25 - 12.5)
        ); // 模拟课堂参与度
