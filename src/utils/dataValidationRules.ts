...
export const VALIDATION_RULES: ValidationRule[] = [
  ...
  },
  {
    ...
    field: "name",
    condition: (value) =>
      !value || /^[\u4e00-\u9fa5a-zA-Z\s]{2,20}$/.test(String(value)),
  ...
  // 分数范围校验
  ...Object.entries(SUBJECT_CONFIGS).map(
    ([subject, config]) =>
      ({
        ...
        condition: (value: any) =>
          value === null ||
          value === undefined ||
          (Number(value) >= 0 && Number(value) <= config.maxScore),
  ...
  // 等级格式校验
  ...Object.keys(SUBJECT_CONFIGS).map(
    (subject) =>
      ({
        ...
        condition: (value: any) =>
          !value ||
          GRADE_CONFIGS.validGrades.includes(String(value).toUpperCase()),
  ...
  // 排名范围校验
  {
    ...
    condition: (value) =>
      ...
      (Number.isInteger(Number(value)) &&
        Number(value) > 0 &&
        Number(value) <= RANK_CONFIGS.classRank.max),
  ...
  },
  {
    ...
    condition: (value) =>
      ...
      (Number.isInteger(Number(value)) &&
        Number(value) > 0 &&
        Number(value) <= RANK_CONFIGS.gradeRank.max),
  ...
  // 逻辑校验规则
  {
    ...
    condition: (value, record) => {
      ...
      const calculatedTotal = subjectScores.reduce(
        (sum, score) => sum + score,
        0
      );
        ...
        if (score !== null && score !== undefined && grade) {
          ...
          if (gradeRange) {
            const normalizedScore =
              (Number(score) / SUBJECT_CONFIGS[subject].maxScore) * 100;
            if (
              normalizedScore < gradeRange.min ||
              normalizedScore > gradeRange.max
            ) {
              return false;
            }
          }
  ...
  // 业务规则校验
  {
    ...
    condition: (value, record, allRecords) => {
      ...
      const duplicates = allRecords.filter(
        (r) => r.student_id === value && r.exam_title === record.exam_title
      );
  ...
  },
  {
    ...
    condition: (value, record, allRecords) => {
      if (
        !record.total_rank_in_class ||
        !record.class_name ||
        !record.exam_title
      )
        return true;

      const sameClassRecords = allRecords.filter(
        (r) =>
...
export function getValidationRules(field?: string): ValidationRule[] {
  ...
  return VALIDATION_RULES.filter(
    (rule) => rule.enabled && (!rule.field || rule.field === field)
  );
}
...
export function calculateDataQualityScore(
  ...
  for (const [level, config] of Object.entries(
    QUALITY_THRESHOLDS.qualityLevels
  )) {
    if (score >= config.min) {
      return {
