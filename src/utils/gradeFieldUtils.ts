 ...
 * @returns 班级排名
 */
export function getSubjectClassRank(
  record: GradeRecord,
  subject: Subject | string
): number {
  const fieldMapping = SUBJECT_FIELD_MAPPING[subject as Subject];
 ...
 * @returns 学校排名
 */
export function getSubjectSchoolRank(
  record: GradeRecord,
  subject: Subject | string
): number {
  const fieldMapping = SUBJECT_FIELD_MAPPING[subject as Subject];
 ...
 * @returns 年级排名
 */
export function getSubjectGradeRank(
  record: GradeRecord,
  subject: Subject | string
): number {
  const fieldMapping = SUBJECT_FIELD_MAPPING[subject as Subject];
 ...
 * @returns 是否包含该科目数据
 */
export function hasSubjectData(
  record: GradeRecord,
  subject: Subject | string
): boolean {
  // 检查通用字段
 ...
 * @returns 标准化的成绩记录
 */
export function createStandardGradeRecord(
  rawRecord: any,
  subject: Subject | string
): GradeRecord {
  return {
 ...
 * 获取多个班级的数据
 */
export function getMultipleClassesData(
  data: GradeRecord[],
  classNames: string[]
): GradeRecord[] {
