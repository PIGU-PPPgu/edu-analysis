...
const ERROR_PATTERNS = {
  ...
  file_format_unsupported: {
    pattern:
      /(unsupported.*format|invalid.*file.*type|format.*not.*supported)/i,
    getFriendlyError: (context: ErrorContext): FriendlyError => ({
      title: "文件格式不支持",
  ...
  file_corrupted: {
    pattern:
      /(corrupted|damaged|cannot.*read|parse.*error|invalid.*structure)/i,
    getFriendlyError: (context: ErrorContext): FriendlyError => ({
      title: "文件可能已损坏",
      message: "无法正确读取文件内容",
      solutions: [
  ...
  required_field_missing: {
    pattern:
      /(required.*field.*missing|姓名.*not.*found|name.*field.*required)/i,
    getFriendlyError: (context: ErrorContext): FriendlyError => ({
      title: "缺少必需的字段",
  ...
  missing_student_info: {
    pattern:
      /(missing.*student.*info|学生.*信息.*不完整|incomplete.*student.*data)/i,
    getFriendlyError: (context: ErrorContext): FriendlyError => ({
      title: "学生信息不完整",
 ...
 * 生成通用的用户友好错误信息
 */
function getGenericFriendlyError(
  errorMessage: string,
  context: ErrorContext
): FriendlyError {
...
export class ErrorRecoveryGuide {
  ...
  static getPreventionTips(context: ErrorContext): string[] {
    ...
    if (context.operation === "data_validation") {
      tips.push("分数使用数字格式", "学生姓名完整且无重复", "科目列名规范统一");
    }

    if (context.fileSize && context.fileSize > 5 * 1024 * 1024) {
      // > 5MB
      tips.push("大文件建议分批导入", "删除不必要的工作表", "优化文件大小");
    }
...
export class AutoErrorFixer {
  static async fixMissingHeaders(data: any[][]): Promise<any[][]> {
    ...
    const looksLikeData = firstRow.some(
      (cell) =>
        typeof cell === "number" ||
...
export class ErrorAnalytics {
  ...
  static getErrorSummary(): {
    ...
    this.errorLog.forEach((log) => {
      byOperation[log.context.operation] =
        (byOperation[log.context.operation] || 0) + 1;
      bySeverity[log.error.severity] =
        (bySeverity[log.error.severity] || 0) + 1;
    });
