...
class RequestCache {
   ...
   */
  async get<T>(key: string, fetchFunction: () => Promise<T>, ttlMs: number = 60 * 1000): Promise<T> {
    const now = Date.now();
    ...
    if (this.cache[key] && this.cache[key].expiry > now) {
      if (this.debug) {
        console.log(`[Cache] 命中: ${key}，过期于 ${new Date(this.cache[key].expiry)}`);
      }
    ...
    try {
      ...
      if (this.debug) {
        console.log(`[Cache] 已缓存: ${key}，过期于 ${new Date(this.cache[key].expiry)}`);
      }
   ...
   * @param ttl 缓存有效期（毫秒），默认5分钟
   */
  async update<T>(key: string, fetchFunction: () => Promise<T>, ttlMs: number = 60 * 1000): Promise<T> {
    if (this.debug) {
    ...
    try {
      ...
      if (this.debug) {
        console.log(`[Cache] 已更新: ${key}，过期于 ${new Date(this.cache[key].expiry)}`);
      }
  ...
  set<T>(key: string, value: T, ttlMs: number = 60 * 1000): void {
    ...
    if (this.debug) {
      console.log(`[Cache] 已手动设置: ${key}，过期于 ${new Date(this.cache[key].expiry)}`);
    }
  }
   ...
   * @param fetchFunction 获取数据的函数
   */
  async refreshKeyAndPage(key: string, fetchFunction: () => Promise<any>): Promise<void> {
    try {
      // 先清除特定键缓存
      this.invalidate(key);
