...
export class FileImportFixer {
  ...
  async testFieldMapping(
    ...
    try {
      ...
      // 生成字段映射
      const mappingResult = await enhancedGenerateInitialMappings(
        parsedData,
        fileName
      );
      ...
      console.log("✅ 字段映射生成完成:", {
        ...
        standardFieldsCount: Object.keys(mappingResult.standardFields || {})
          .length,
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "字段映射过程中出现未知错误",
      };
    }
  }
  ...
  async diagnoseFileImport(file: File): Promise<{
    ...
    if (!parseTest.success) {
      ...
      fixes.push(
        "检查文件格式和完整性，确保使用支持的文件类型(.xlsx, .xls, .csv)"
      );
    }

    // 2. 测试字段映射 (如果解析成功)
    if (parseTest.success && parseTest.data) {
      const mappingTest = await this.testFieldMapping(
        parseTest.data,
        file.name
      );
      ...
      if (!mappingTest.success) {
        ...
        fixes.push(
          "检查Excel/CSV文件的表头是否正确，确保包含学号、姓名等基本字段"
        );
      }
    }
  ...
  showRepairSuggestions(issues: string[], fixes: string[]): void {
    ...
    // 显示用户友好的提示
    toast.error(
      `发现 ${issues.length} 个问题需要修复，请查看控制台获取详细信息`
    );
  }
}
