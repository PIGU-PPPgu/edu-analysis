  ...
  headers.forEach((header, index) => {
    if (!mappings[header] && index in commonPosition) {
      const potentialField =
        commonPosition[index as keyof typeof commonPosition];
...
export const parseExcel = async (buffer: ArrayBuffer) => {
  try {
    console.log(
      "[parseExcel] 开始解析Excel文件，buffer大小:",
      buffer.byteLength
    );
...
// 增强AI识别力的启发式规则
export const intelligentFieldDetection = (
  headers: string[],
  sampleData: any[]
): Record<string, string> => {
  const mappings: Record<string, string> = {};
  ...
  headers.forEach((header) => {
    ...
    if (
      ...
      // 学号通常是数字或固定模式的字符串
      const isIdLike = samples.every(
        (s) => /^\d+$/.test(s) || /^[A-Za-z]\d+$/.test(s)
      );
      if (isIdLike) {
    ...
    if (
      ...
      const isNameLike = samples.every(
        (s) => /^[\u4e00-\u9fa5]{2,4}$/.test(s) || !/\d/.test(s)
      );
      if (isNameLike) {
    ...
    if (
      ...
      // 班级通常包含"班"字或有特定格式
      const isClassLike = Array.from(uniqueValues).some(
        (v) =>
          String(v).includes("班") ||
          /^\d+级\d+班$/.test(String(v)) ||
          /^[高初]\d+[班\(\d+\)]/.test(String(v))
      );
  ...
  if (!matchedFields.has("studentId") || !matchedFields.has("name")) {
    ...
    for (let i = 0; i < Math.min(headers.length, 2); i++) {
      ...
      if (i === 1 && !matchedFields.has("name")) {
        // 名字通常是2-4个中文字符
        const isNameLike = samples.every((s) =>
          /^[\u4e00-\u9fa5]{2,4}$/.test(s)
        );
        if (isNameLike) {
          mappings[header] = "name";
...
// 增强文件解析函数，使用AI增强功能
export const enhancedGenerateInitialMappings = (
  headers: string[],
  sampleData: any[]
): Record<string, string> => {
  console.log("[enhancedGenerateInitialMappings] 开始字段映射，表头:", headers);
  console.log(
    "[enhancedGenerateInitialMappings] 样本数据:",
  ...
  if (sampleData && sampleData.length > 0) {
    ...
    console.log(
      "[enhancedGenerateInitialMappings] 智能映射结果:",
      intelligentMappings
    );
    ...
    headers.forEach((header) => {
      ...
      // 如果是高优先级字段且未被使用，保留
      if (
        priorityFields.includes(mapping) &&
        !usedStandardFields.has(mapping)
      ) {
    ...
    console.log(
      "[enhancedGenerateInitialMappings] 清理后的映射结果:",
      cleanedMappings
    );
  ...
  console.log(
    "[enhancedGenerateInitialMappings] 使用基础映射结果:",
    basicMappings
  );
