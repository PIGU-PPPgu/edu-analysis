...
const generateTestData = (count: number) => {
  const data = [];
  const subjects = ['语文', '数学', '英语', '物理', '化学', '道法', '历史', '生物', '地理'];
  ...
  for (let i = 0; i < count; i++) {
    data.push({
      ...
      exam_date: new Date(2024, i % 12, (i % 28) + 1).toISOString().split('T')[0],
...
export const VirtualTablePerformanceTest: React.FC = () => {
  ...
  const headers = [
    'student_id', 'name', 'class_name', 'exam_title', 'exam_date',
    'subject', 'score', 'total_score', 'grade', 'rank_in_class'
  ];

  // 性能测试函数
  const runPerformanceTest = async (dataSize: number, useVirtualization: boolean) => {
    setCurrentTest(`测试 ${dataSize} 条数据 (${useVirtualization ? '虚拟化' : '非虚拟化'})`);
                ...
                <table className="w-full border-collapse border border-gray-300">
                  <thead>
                    <tr className="bg-gray-50">
                      <th className="border border-gray-300 px-4 py-2">数据量</th>
                      <th className="border border-gray-300 px-4 py-2">虚拟化</th>
                      <th className="border border-gray-300 px-4 py-2">渲染时间(ms)</th>
                      <th className="border border-gray-300 px-4 py-2">滚动性能</th>
                      <th className="border border-gray-300 px-4 py-2">内存使用</th>
                      <th className="border border-gray-300 px-4 py-2">性能等级</th>
                    </tr>
                  </thead>
                  <tbody>
                    {testResults.map((result, index) => (
                      <tr key={index}>
                        <td className="border border-gray-300 px-4 py-2">{result.dataSize}</td>
                        <td className="border border-gray-300 px-4 py-2">
                          <Badge variant={result.virtualized ? "default" : "secondary"}>
                            {result.virtualized ? '是' : '否'}
                          </Badge>
                        </td>
                        <td className="border border-gray-300 px-4 py-2">
                          <span className={result.renderTime > 100 ? 'text-red-600' : 'text-green-600'}>
                        ...
                        <td className="border border-gray-300 px-4 py-2">
                          <Badge variant={
                            result.renderTime < 50 ? "default" : 
                            result.renderTime < 100 ? "secondary" : "destructive"
                          }>
                            {result.renderTime < 50 ? '优秀' : 
                             result.renderTime < 100 ? '良好' : '需优化'}
                          </Badge>
                        </td>
                      </tr>
                    ))}
