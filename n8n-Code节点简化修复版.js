...
for (const dataItem of inputData) {
  try {
    console.log('处理项目:', JSON.stringify(dataItem, null, 2));
    ...
    // 检查各种可能的文件数据源
    if (dataItem.binary && dataItem.binary.data) {
      fileData = dataItem.binary.data.data;
      fileName = dataItem.binary.data.fileName || 'unknown.xlsx';
      console.log('从binary.data获取文件');
    } else if (dataItem.json && dataItem.json.body && dataItem.json.body.file) {
      fileData = dataItem.json.body.file;
      fileName = dataItem.json.body.fileName || 'unknown.xlsx';
      console.log('从json.body.file获取文件');
    } else if (dataItem.json && dataItem.json.file) {
      fileData = dataItem.json.file;
      fileName = dataItem.json.fileName || 'unknown.xlsx';
      console.log('从json.file获取文件');
    } else if (dataItem.json && dataItem.json.data) {
      fileData = dataItem.json.data;
      fileName = dataItem.json.fileName || 'unknown.xlsx';
