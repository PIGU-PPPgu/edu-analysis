...
const DataValidator: React.FC<DataValidatorProps> = ({
  ...
  const performValidation = async () => {
    ...
    try {
      ...
      const validData = processedData.filter((_, index) => {
        const hasErrors = result.errors.some(error => error.row === index + 1);
  ...
  const validateData = async (): Promise<ValidationResult> => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // 验证等级字段
        if (mappedRow.original_grade && typeof mappedRow.original_grade === 'string') {
          const grade = mappedRow.original_grade.trim().toUpperCase();
          const validGrades = ['A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D', 'E', 'F'];
      ...
      // 检查最大错误数限制
      if (validationConfig.maxErrors > 0 && errors.length >= validationConfig.maxErrors) {
        warnings.push({
          row: rowNumber,
          field: 'system',
          value: null,
  ...
  const getRowValidationStatus = (rowIndex: number) => {
    ...
    const hasError = validationResult.errors.some(error => error.row === rowIndex);
    const hasWarning = validationResult.warnings.some(warning => warning.row === rowIndex);
  ...
  const getFieldValidationStatus = (rowIndex: number, field: string) => {
    ...
    const error = validationResult.errors.find(e => e.row === rowIndex && e.field === field);
    const warning = validationResult.warnings.find(w => w.row === rowIndex && w.field === field);
  ...
  const validationStats = useMemo(() => {
    ...
    return {
      validPercentage: totalRows > 0 ? Math.round((summary.validRows / totalRows) * 100) : 0,
      errorPercentage: totalRows > 0 ? Math.round((summary.errorRows / totalRows) * 100) : 0,
      warningPercentage: totalRows > 0 ? Math.round((summary.warningRows / totalRows) * 100) : 0,
      ...summary
    };
  }, [validationResult]);
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => 
                  setValidationConfig(prev => ({ ...prev, strictMode: checked }))
                }
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => 
                  setValidationConfig(prev => ({ ...prev, skipInvalidRows: checked }))
                }
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => 
                  setValidationConfig(prev => ({ ...prev, validateDuplicates: checked }))
                }
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => 
                  setValidationConfig(prev => ({ ...prev, validateStudentMatch: checked }))
                }
              />
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => 
                  setValidationConfig(prev => ({ ...prev, requireScores: checked }))
                }
              />
        ...
        {validationStats && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{validationStats.validRows}</p>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{validationStats.errorRows}</p>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{validationStats.warningRows}</p>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{validationStats.totalRows}</p>
            ...
            <div className="space-y-2">
              ...
              <Progress value={validationStats.validPercentage} className="h-2" />
            </div>
              ...
              <AlertDescription>
                {validationResult?.isValid ? (
                  ...
                  <span className="text-red-700">
                    发现 {validationResult?.errors.length} 个错误，需要修复后才能导入
                  </span>
                )}
              </AlertDescription>
            </Alert>
              ...
              <div className="border rounded-lg">
                <ScrollArea className="h-96">
                  <Table>
                    ...
                    <TableBody>
                      {Object.entries(studentMatches).map(([key, match], index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <Badge variant={
                              ...
                              match.matchType === 'new' ? 'outline' :
                              'destructive'
                            } className="text-xs">
                              {match.matchType === 'exact' ? '精确匹配' :
                               match.matchType === 'fuzzy' ? '模糊匹配' :
                               match.matchType === 'new' ? '新学生' :
                               '冲突'}
                            </Badge>
                          </TableCell>
                          <TableCell>
