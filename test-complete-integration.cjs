...
class TestDataGenerator {
   ...
   * 生成成绩数据
   */
  generateGradeData(students, examTitle = '期中考试') {
    const gradeData = [];
    
    students.forEach(student => {
      this.subjects.forEach(subject => {
        const baseScore = Math.floor(Math.random() * 40) + 60; // 60-100分
...
async function testDatabaseIntegration() {
  ...
  try {
    ...
    // 测试表结构
    const tables = ['students', 'exams', 'grade_data'];
...
async function main() {
  ...
  try {
    // 1. 测试数据库集成
    const dbIntegrationOk = await testDatabaseIntegration();
    if (!dbIntegrationOk) {
      console.error('❌ 数据库集成测试失败，终止测试');
      process.exit(1);
    }
    ...
    if (report.successRate >= 0.8) {
      console.log('🎉 集成测试通过！智能匹配系统运行正常');
    } else {
      console.log('⚠️  集成测试部分失败，需要进一步优化');
    }
    console.log('='.repeat(60));
