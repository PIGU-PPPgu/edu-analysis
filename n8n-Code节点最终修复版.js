// n8n Code节点最终修复版 - 解决json属性必须是对象的问题
// 返回包含records数组的对象，而不是直接返回数组
...
try {
  ...
  // 🎯 关键修复：返回包含records数组的对象
  // json属性必须是对象，不能是数组
  return [{
    json: {
      records: records,        // 记录数组放在records属性中
      exam_info: examInfo,     // 考试信息
      total_count: records.length,  // 总记录数
      success: true,           // 成功标志
      message: `成功解析${records.length}条记录`
    }
  }];
