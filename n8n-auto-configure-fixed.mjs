...
/**
 * n8n Information Extractor 自动配置脚本 (修复版本)
 * 通过API自动添加71个字段属性
 */
 ...
 * 激活工作流
 */
async function activateWorkflow(apiConfig) {
  try {
    console.log('🔄 激活工作流...');
    ...
    console.error('❌ 激活工作流失败:', error.message);
    if (error.response) {
      console.error('响应状态:', error.response.status);
      console.error('响应数据:', JSON.stringify(error.response.data, null, 2));
    }
...
async function main() {
  ...
  try {
    ...
    console.log('当前状态:', workflow.active ? '激活' : '未激活');
    ...
    // 5. 激活工作流
    if (!workflow.active) {
      await activateWorkflow(apiConfig);
    }
    ...
    console.log('   - 工作流状态: 已激活');
    console.log('');
    console.log('✅ 配置完成，可以开始测试！');
    console.log('🔗 n8n界面: http://localhost:5678');
    console.log('🔗 Webhook URL: http://localhost:5678/webhook/083f9843-c404-4c8f-8210-e64563608f57');
    console.log('');
    console.log('🧪 测试建议:');
    console.log('   1. 在n8n界面中验证所有71个字段都已配置');
    console.log('   2. 使用测试CSV数据验证解析功能');
    console.log('   3. 确认Webhook响应配置正确');
    console.log('   4. 测试完整的数据流：CSV → 解析 → Supabase');
